...
public class LeaderboardUI : BaseLeaderboard
{
	...
	{
		var enableFriendLeaderboards = DIContainerBalancing.GameConstantsBalancingDataProvider.EnableFriendLeaderboards;
	...
	private IEnumerator EnterCoroutine()
	{
		...
		var uiPanelOffset = GetStandardUiPanelOffset();
	...
	{
		if (m_EventModel != null && m_CheaterBoardLabelRoot != null)
		{
	...
	private List<Leaderboard.Score> GetRankedPlayers(bool alsoGetZero = false, bool enemyTeam = false)
	{
		...
		{
			return enemyTeam
				? m_EventModel.GetRankedPlayersEnemys(alsoGetZero)
				: m_EventModel.GetRankedPlayers(alsoGetZero);
		}
	...
	private IEnumerator SetupLeagueBlinds(bool enemyTeam)
	{
		...
		var players = GetRankedPlayers(true, enemyTeam);
		for (var rank = 0; rank < players.Count; rank++)
		{
			...
			{
				oppInfo2.SetDefault((int)score.GetPoints(), rank + 1, (int)score.GetPoints() > 0 ? GetStarRatingForRank(rank + 1) : 0, false, false, true);
			...
			{
				oppInfo2.SetDefault((int)score.GetPoints(), rank + 1, (int)score.GetPoints() > 0 ? GetStarRatingForRank(rank + 1) : 0, false, enemyTeam);
	...
	private IEnumerator SetupFriendBlinds(int pageNum)
	{
		...
		// yes the null assignments are supposed to be there
		...
		if (m_maxPages > 0)
		{
			var pageText = m_currentPage + 1 + "/" + (m_maxPages + 1);
		...
		for (var rank = pageNum * 15; rank < (pageNum + 1) * 15 && sortedFriends.Count > rank && sortedFriends.ContainsKey(rank); rank++)
		{
			...
			if (friend == null)
			{
				...
				var playerscore2 = 0;
				playerscore2 = (int)player.CurrentEventManagerGameData.Data.CurrentScore;
				oppInfo2.SetDefault(playerscore2, rank + 1, playerscore2 > 0 ? GetStarRatingForRank(rank + 1) : 0, false, true, true);
			...
			else
			{
				...
				{
					oppInfo2.SetDefault(0, rank + 1, 0, false, true);
				}
				else
				{
					oppInfo2.SetDefault(friendScores[friend.FriendId], rank + 1, friendScores[friend.FriendId] > 0 ? GetStarRatingForRank(rank + 1) : 0, false, true);
				}
	...
	private void ShowClockTooltip()
	{
		...
		var seasonTurnAmount = currentPvPSeasonGameData.Balancing.SeasonTurnAmount;
		var localizedText = DIContainerInfrastructure.GetLocaService().Tr("pvp_clock_tt").Replace("{value_1}", seasonTurnAmount.ToString());
