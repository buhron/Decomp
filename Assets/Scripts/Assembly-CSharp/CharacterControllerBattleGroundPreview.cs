...
public class CharacterControllerBattleGroundPreview : CharacterControllerBattleGroundBase
{
	...
	public override IEnumerator DoTurn(int turn)
	{
		...
		foreach (var combatant in m_BattleMgr.Model.m_CombatantsByInitiative)
		{
		...
		if (m_SkillToDo != null)
		{
			...
			case SkillTargetTypes.Support:
				m_Model.AttackTarget = m_BattleMgr.Model.m_CombatantsByInitiative.FirstOrDefault(c => c.CombatantFaction == m_Model.CombatantFaction && c != m_Model);
				break;
			}
		}
		if (m_SkillToDo != null)
	...
	private void OnDestroy()
	{
		...
		foreach (var value in LastingVisualEffects.Values)
		{
			if (value)
			{
		...
		if (m_AssetController && m_SkillToDo != null && m_AssetController.m_BoneAnimation)
		{
			m_AssetController.m_BoneAnimation.UnregisterUserTriggerDelegate(m_SkillToDo.BoneAnimationUserTrigger);
		}
