...
public class AsyncWebRequestMB : MonoBehaviour, IAsyncWebRequest
{
	...
	{
		var www = GetWWW(url, postData, m_headers);
		...
		using (www)
		{
			...
			var timeGoneBy = (DateTime.Now - m_timeoutCounter).TotalSeconds;
			...
			{
				var errorText = !string.IsNullOrEmpty(www.text) ? www.text : www.bytes == null || www.bytes.Length <= 0 ? "(nothing)" : Encoding.UTF8.GetString(www.bytes);
				LogError("Web request error calling " + url + ": " + www.error + ", text: " + errorText + ", postData: " + (postData == null ? "null" : Encoding.UTF8.GetString(postData)) + " --- headers: " + m_headers.Aggregate(string.Empty, (acc, curr) => acc + curr.Key + " -> " + curr.Value.Substring(0, Math.Min(15, curr.Value.Length)) + "...,"));
			}
			else
			{
				var response = !string.IsNullOrEmpty(www.text) ? www.text : www.bytes == null || www.bytes.Length <= 0 ? "(nothing)" : Encoding.UTF8.GetString(www.bytes);
