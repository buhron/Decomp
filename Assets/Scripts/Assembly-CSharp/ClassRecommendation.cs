...
public class ClassRecommendation : MonoBehaviour
{
	...
	public void Init(ClassItemBalancingData classBalancing, BattleHintPopup parentPopup)
	{
		...
		var text = classBalancing.AssetBaseId;
		...
		var gameObject = DIContainerInfrastructure.GetClassAssetProvider().InstantiateObject(text, m_headGearParent, Vector3.zero, Quaternion.identity, false);
		gameObject.transform.localScale = Vector3.one;
		var active = classBalancing.IsPremium && !DIContainerLogic.InventoryService.CheckForItem(DIContainerInfrastructure.GetCurrentPlayer().InventoryGameData, classBalancing.NameId);
