...
public class ABHLocaService
{
	...
	private static extern void _SetSystemLanguageCodeToUserPrefs();
	#endif
	...
	private string GetLanguageCodeWithRegion()
	{
		#if UNITY_ANDROID
		string text = null;
		using (var androidJavaClass = new AndroidJavaClass("java.util.Locale"))
		{
			using (var androidJavaObject = androidJavaClass.CallStatic<AndroidJavaObject>("getDefault", new object[0]))
			{
		...
		return text;
		#elif UNITY_IOS && ENABLE_IOS_NATIVE_CODE
		_SetSystemLanguageCodeToUserPrefs();
		var language = PlayerPrefs.GetString("user_language_code");
		DebugLog.Log(GetType(), "Found system language code " + language);
		return language;
		#endif
		return "English";
	}
	...
	private void ExtractLanguageCodeAndRegion(string languageCodeWithRegion)
	{
		if (!string.IsNullOrEmpty(languageCodeWithRegion))
		{
			...
			var array = languageCodeWithRegion.Split('_');
	...
	public void InitDefaultLoca(MonoBehaviour synchMonoBehaviour = null, string overrideLanguage = null)
	{
		...
		{
			var text = Application.systemLanguage.ToString();
			...
			if (IsLanguageSupported(text))
			{
				if (text == "Chinese")
				{
					...
					var languageCodeWithRegion = GetLanguageCodeWithRegion();
					...
					var languageName = "Chinese (Simplified)";
		...
		var fontInitializer = !ContentLoader.Instance ? null : ContentLoader.Instance.GetComponent<FontInitializer>();
		if (fontInitializer)
		{
	...
	internal string GetEquipmentName(string ident, bool isAncient)
	{
		return isAncient ? locaHelper.Tr(ident + "_name" + "_ancient") : locaHelper.Tr(ident + "_name");
	}
	...
	internal string GetBannerItemName(string ident, bool isAncient)
	{
		return isAncient ? locaHelper.Tr(ident + "_name" + "_ancient") : locaHelper.Tr(ident + "_name");
	}
	...
	{
		return locaHelper.Tr("unlock_mastery_badge");
	}
