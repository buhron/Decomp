...
public class RageMeterController : MonoBehaviour
{
	...
	public void Enter()
	{
		...
		if (m_DraggableTransformMotionTween)
		{
	...
	{
		var j = 0;
		var oldValue = m_battleMgr.Model.m_CurrentRage - value;
		for (var i = 0f; i < value; i += 3f)
		{
	...
	private IEnumerator ChangeValueOverTime(UISprite bar, float maxHealth, float oldValue, float newValue, float time)
	{
		...
		while (timeLeft > 0f)
		{
			...
			var fill = (oldValue + delta * Mathf.Min(1f, 1f - timeLeft / time)) / maxHealth;
			bar.fillAmount = fill;
		}
		m_Changing = false;
	}
	...
	{
		var num = m_battleMgr.Model.m_CurrentRage + value;
	}
	...
	public void m_DragTrigger_onPress(GameObject obj)
	{
		if (!m_battleMgr.m_LockControlHUDs && m_battleMgr.Model.IsRageFull(Faction.Birds) && m_battleMgr.IsRagemeterUsePossible)
		{
			...
			var list = m_battleMgr.Model.m_CombatantsByInitiative.Where(c => c.CombatantFaction == Faction.Birds && !c.IsRageAvailiable).ToList();
	...
	public void OnRelease(Vector3 pos)
	{
		...
		var ray = m_sceneryCamera.ScreenPointToRay(pos);
		DeRegisterEventHandlers();
		RaycastHit hitInfo;
		...
		{
			if (m_DraggableTransform)
			{
				var component = m_DraggedChili.GetComponent<CHMotionTween>();
		...
		var component2 = hitInfo.transform.GetComponent<CharacterControllerBattleGroundBase>();
		if (!component2 || !component2.GetModel().IsParticipating || !m_battleMgr.IsRagemeterUsePossible)
		{
			m_battleMgr.m_DraggedCharacter = null;
			if (m_DraggableTransform)
			{
				var component3 = m_DraggedChili.GetComponent<CHMotionTween>();
	...
	public void OnDrag(Vector3 dragPos)
	{
		...
		var ray = m_sceneryCamera.ScreenPointToRay(dragPos);
		RaycastHit hitInfo;
		if (Physics.Raycast(ray, out hitInfo, 10000f, (1 << LayerMask.NameToLayer("TutorialScenery")) | (1 << LayerMask.NameToLayer("Scenery"))))
		{
			if (m_cachedOverCharacterTransform != hitInfo.transform)
			{
				...
				if (m_cachedOverCharacter && !m_cachedOverCharacter.GetModel().IsRageAvailiable)
				{
