...
public class UIPlayAnimation : MonoBehaviour
{
	...
	public EnableCondition ifDisabledOnPlay = EnableCondition.EnableThenPlay;
	...
	[SerializeField] GameObject eventReceiver;
	...
	public void Play (bool forward, bool onlyIfDifferent)
	{
		if (target)
		{
			...
			// for some reason, someone at ngui thought it would be a good idea to do ALL THIS CRAP instead of just.. playing the animation
			// also doing all that crap just breaks animation events, and they control the click sounds
			...
			if (Time.timeScale == 0f)
			{
				...
				ActiveAnimation.Play(target, clipName, forward ? playDirection : (Direction)(-(int)playDirection), ifDisabledOnPlay, disableWhenFinished);
			}
			else
			{
				target.enabled = true;
