...
public class DailyObjectiveBaseElement : MonoBehaviour
{
	...
	public virtual void Init(PvPObjectivesGameData gameData)
	{
		...
		if (string.IsNullOrEmpty(m_GameData.Requirement2))
		{
			...
			CheckForAtlasSwitch(m_oneIconMainSprite);
		...
		var array = m_GameData.IconSpriteName.Split('&');
		if (array.Length < 2)
		{
		...
		if (m_GameData.RequirementType == ObjectivesRequirement.useClass)
		{
			var replacementName2 = string.Empty;
			if (ClassItemGameData.CheckForReplacement("class_" + m_GameData.Requirement1, out replacementName2))
			{
				m_twoIconMainSpriteA.spriteName = "DailyObjective_" + replacementName2.Replace("Headgear_", string.Empty);
			}
		...
		CheckForAtlasSwitch(m_twoIconMainSpriteA);
		m_twoIconMainSpriteA.MakePixelPerfect();
		CheckForAtlasSwitch(m_twoIconMainSpriteB);
	...
	private void CheckForAtlasSwitch(UISprite sprite)
	{
		var newAtlasSprites = new string[5]
		{
			"DailyObjective_ChallengerSamurai",
			"DailyObjective_ChallengerPrincess",
			"DailyObjective_ChallengerThunderBird",
			"DailyObjective_ChallengerSeaDog",
			"DailyObjective_ChallengerSpy"
		};
		
		var obj = newAtlasSprites.Contains(sprite.spriteName)
			? DIContainerInfrastructure.GetGenericIconAtlasAssetProvider().GetObject("DailyObjectives") as GameObject
			: DIContainerInfrastructure.GetGenericIconAtlasAssetProvider().GetObject("ArenaElements") as GameObject;

		if (obj != null)
			sprite.atlas = obj.GetComponent<UIAtlas>();
	}
}
