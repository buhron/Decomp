...
public class GenericOverlay : MonoBehaviour
{
	...
	{
		var vector = m_InterfaceCamera.ScreenToWorldPoint(orientatedCamera.WorldToScreenPoint(root.position));
	...
	{
		var vector = m_HeaderTextBox.SetTextAndGetBackgroundSize(localizedHeader);
		var vector2 = m_TextBox.SetTextAndGetBackgroundSize(localizedText);
		var size = new Vector3(Mathf.Max(vector.x, vector2.x), vector.y + vector2.y, 0f);
		...
		foreach (var centerSprite in m_CenterSprites)
		{
		...
		var textPosition = m_HeaderTextBox.textPosition;
