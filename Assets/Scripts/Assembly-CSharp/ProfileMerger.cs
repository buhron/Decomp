...
internal class ProfileMerger : IProfileMerger
{
	...
	{
		int luckyCoinCount;
		var flag = TryGetLuckyCoinCount(currentProfile, out luckyCoinCount);
		int luckyCoinCount2;
		var flag2 = TryGetLuckyCoinCount(otherProfile, out luckyCoinCount2);
	...
	private void TransferBirdEquipmentAndBannerItemsFrom(PlayerData from, PlayerData to)
	{
		...
		foreach (var mainHandItem2 in from.Inventory.MainHandItems)
		{
			mainHandItem = mainHandItem2;
			if (mainHandItem.ItemSource == EquipmentSource.Gatcha && !to.Inventory.MainHandItems.Any(toItem => toItem.NameId == mainHandItem.NameId && toItem.Level == mainHandItem.Level))
			{
		...
		foreach (var offHandItem2 in from.Inventory.OffHandItems)
		{
			offHandItem = offHandItem2;
			if (offHandItem.ItemSource == EquipmentSource.Gatcha && !to.Inventory.MainHandItems.Any(toItem => toItem.NameId == offHandItem.NameId && toItem.Level == offHandItem.Level))
			{
		...
		foreach (var bannerItem2 in from.Inventory.BannerItems)
		{
			bannerItem = bannerItem2;
			if (bannerItem.ItemSource == EquipmentSource.Gatcha && !to.Inventory.MainHandItems.Any(toItem => toItem.NameId == bannerItem.NameId && toItem.Level == bannerItem.Level))
			{
	...
	{
		luckyCoinItem = null;
		foreach (var playerStat in profile.Inventory.PlayerStats)
		{
