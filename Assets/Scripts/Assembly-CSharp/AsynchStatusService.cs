...
public class AsynchStatusService : MonoBehaviour
{
	...
	public string ShowInfoAndLootItems(string message, List<IInventoryItemGameData> items, string compareId)
	{
		...
		var newMessage = new DispatchMessage();
		...
		if (m_dispatchMessages.Count(m => m.m_CompareTag == newMessage.m_CompareTag) > 0)
		{
			return null;
		}
		if (m_dispatchMessages.Count > 0)
	...
	public string ShowInfo(string message, string compareId, DispatchMessage.Status status)
	{
		...
		newMessage.m_DispatchMessage = message;
		newMessage.m_CompareTag = compareId;
		if (!string.IsNullOrEmpty(newMessage.m_CompareTag) && m_dispatchMessages.Count(m => m.m_CompareTag == newMessage.m_CompareTag) > 0)
		{
			return null;
		}
		if (m_dispatchMessages.Count > 0)
	...
	{
		var dispatchMessage = new DispatchMessage();
		if (blocking)
		{
	...
	{
		var delta = Time.realtimeSinceStartup - m_lastLoadingDisplay;
		if (delta < 10f)
		{
