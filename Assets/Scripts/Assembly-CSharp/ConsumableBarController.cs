...
public class ConsumableBarController : MonoBehaviour
{
	...
	{
		m_LuckyCoinsController.UpdateAnim();
		StartCoroutine(RefreshBlinds());
	}
	...
	private void OnAdWatchedForConsumableRoll()
	{
		...
		var componentsInChildren = m_Grid.GetComponentsInChildren<ConsumableBattleButtonController>();
		var array = componentsInChildren;
		foreach (var consumableBattleButtonController in array)
		{
	...
	private IEnumerator RefreshBlinds()
	{
		...
		foreach (var consumableRecipe in m_BattleMgr.Model.m_ControllerInventory.CraftingRecipes[InventoryItemType.Consumable].Values)
		{
			var recipe = consumableRecipe.LastOrDefault() as CraftingRecipeGameData;
			...
			if (DIContainerBalancing.Service.TryGetBalancingData<ConsumableItemBalancingData>(recipe.GetResultLoot().Keys.FirstOrDefault(), out cbd4) && !string.IsNullOrEmpty(cbd4.InstantBuyOfferCategoryId) && (!(cbd4.NameId == "potion_xp_01") || DIContainerLogic.InventoryService.GetItemValue(DIContainerInfrastructure.GetCurrentPlayer().InventoryGameData, "xp_multiplier_consumable_01") <= 0))
			{
				...
				var battleButton5 = UnityEngine.Object.Instantiate(m_ConsumableBarButtonPrefab);
				...
				if (DIContainerBalancing.GameConstantsBalancingDataProvider.SponsoredAdPotionType == cbd4.ConsumableStatckingType)
				{
		...
		{
			if (!consumableNames.Exists(n => n == item.BalancingData.NameId) && !string.IsNullOrEmpty(item.BalancingData.InstantBuyOfferCategoryId) && (!(item.BalancingData.NameId == "potion_xp_01") || DIContainerLogic.InventoryService.GetItemValue(DIContainerInfrastructure.GetCurrentPlayer().InventoryGameData, "xp_multiplier_consumable_01") <= 0))
			{
				consumableNames.Add(item.BalancingData.NameId);
				var battleButton4 = UnityEngine.Object.Instantiate(m_ConsumableBarButtonPrefab);
				...
				if (DIContainerBalancing.GameConstantsBalancingDataProvider.SponsoredAdPotionType == item.BalancingData.ConsumableStatckingType)
				{
		...
		if (!consumableNames.Exists(n => n == "potion_xp_01"))
		{
			...
			{
				var battleButton3 = UnityEngine.Object.Instantiate(m_ConsumableBarButtonPrefab);
				...
				if (DIContainerBalancing.GameConstantsBalancingDataProvider.SponsoredAdPotionType == cbd3.ConsumableStatckingType)
				{
		...
		if (!consumableNames.Exists(n => n == "potion_damage_all_01"))
		{
			...
			{
				var battleButton2 = UnityEngine.Object.Instantiate(m_ConsumableBarButtonPrefab);
				...
				if (DIContainerBalancing.GameConstantsBalancingDataProvider.SponsoredAdPotionType == cbd2.ConsumableStatckingType)
				{
		...
		if (!consumableNames.Exists(n => n == "potion_purify_01"))
		{
			...
			{
				var battleButton = UnityEngine.Object.Instantiate(m_ConsumableBarButtonPrefab);
				...
				if (DIContainerBalancing.GameConstantsBalancingDataProvider.SponsoredAdPotionType == cbd.ConsumableStatckingType)
				{
	...
	public IEnumerator LeaveCoroutine()
	{
		...
		foreach (var characterBlocker in m_BattleMgr.m_CharacterInteractionBlockedItems)
		{
