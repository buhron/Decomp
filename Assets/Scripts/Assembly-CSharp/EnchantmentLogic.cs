...
public class EnchantmentLogic
{
	...
	public EnchantingBalancingData GetBalancing(EquipmentGameData equipment, int enchantmentLevel = -1)
	{
		...
		{
			enchantmentLevel = equipment.EnchantmentLevel;
		}
		var levelRange = equipment.Data.Level / 10;
	...
	public EnchantingBalancingData GetBalancing(BannerItemGameData bannerItem, int enchantmentLevel = -1)
	{
		if (enchantmentLevel == -1)
		{
			enchantmentLevel = bannerItem.EnchantmentLevel;
		}
		var levelRange = bannerItem.Data.Level / 10;
		return GetEnchantingBalancing(enchantmentLevel, levelRange);
	}
	...
	{
		var enchList = (from b in DIContainerBalancing.Service.GetBalancingDataList<EnchantingBalancingData>()
	...
	private int CountBalancingEntriesForItem(List<EnchantingBalancingData> enchList, bool isSetItem, int quality)
	{
		if (isSetItem)
		{
			return enchList.Where(b => b.SetAllowed).Count();
		}
		switch (quality)
		{
		case 0:
			return enchList.Where(b => b.Stars0Allowed).Count();
		case 1:
			return enchList.Where(b => b.Stars1Allowed).Count();
		case 2:
			return enchList.Where(b => b.Stars2Allowed).Count();
		case 3:
			return enchList.Where(b => b.Stars3Allowed).Count();
		default:
			return 0;
		}
	}
