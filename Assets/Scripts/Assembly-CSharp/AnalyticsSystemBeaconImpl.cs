...
internal class AnalyticsSystemBeaconImpl : IAnalyticsSystem
{
	...
	public bool StartSession()
	{
		...
		m_analytics = new Analytics(ContentLoader.Instance.m_BeaconConnectionMgr.Identity);
	...
	public bool LogEventWithParameters(string eventName, Dictionary<string, string> parameters, bool isTimed = false)
	{
		...
		if (!parameters.ContainsKey(SessionStartEventSessionIdParameterName))
		{
			parameters.Add(SessionStartEventSessionIdParameterName, GetCurrentSkynestSessionId());
		}
		if (!parameters.ContainsKey(RovioAnalyticsFlagParameterName))
		{
			parameters.Add(RovioAnalyticsFlagParameterName, "true");
		}
		if (!parameters.ContainsKey(CustomerIdParameterName))
		{
			parameters.Add(CustomerIdParameterName, DIContainerConfig.GetSkuName());
		}
