...
public class ConfirmationPopupController : MonoBehaviour
{
	...
	private IEnumerator EnterCoroutine()
	{
		...
		{
			var atlasGob = DIContainerInfrastructure.GetGenericIconAtlasAssetProvider().GetObject(m_atlasName) as GameObject;
			if (atlasGob != null)
			{
		...
		yield return StartCoroutine(CoroutineUtil.WaitForRealSeconds(GetComponent<Animation>()["Popup_Confirmation_Enter"].length));
	...
	private IEnumerator Shutdown(Action actionToInvoke)
	{
		...
		// var timeLeft = GetComponent<Animation>()["Popup_Confirmation_Leave"].length;
		// while (timeLeft > 0f)
		// {
		...
		yield return StartCoroutine(CoroutineUtil.WaitForRealSeconds(GetComponent<Animation>().gameObject.PlayAnimationOrAnimatorState("Popup_Confirmation_Leave")));
