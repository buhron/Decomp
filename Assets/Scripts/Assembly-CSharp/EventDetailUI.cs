...
public class EventDetailUI : MonoBehaviour
{
	...
	[SerializeField]
	private UIInputTrigger m_InvasionEasyBattleButton;

	[SerializeField]
	private UIInputTrigger m_InvasionMediumBattleButton;

	[SerializeField]
	private UIInputTrigger m_InvasionHardBattleButton;
	...
	[SerializeField]
	private UIInputTrigger m_CollectionChestButton;
	...
	[SerializeField]
	public UILabel m_WaitingForServerLabel;
	...
	{
		// if (DIContainerLogic.EventSystemService.IsEventRunning(eventManagerGameData.Balancing) && eventManagerGameData.Data.CurrentState != EventManagerState.Running)
		// {
		// 	eventManagerGameData.Data.CurrentState = EventManagerState.Running;
		// } igtbap fix, we're going for vanilla rn
		...
		{
			var loot = DIContainerLogic.GetLootOperationService().GenerateLootPreview(eventManagerGameData.EventBalancing.EventMiniCampaignItemLootTable, DIContainerInfrastructure.GetCurrentPlayer().Data.Level);
		...
		{
			var loot2 = DIContainerLogic.GetLootOperationService().GenerateLootPreview(eventManagerGameData.EventBalancing.EventGeneratorItemLootTable, DIContainerInfrastructure.GetCurrentPlayer().Data.Level);
			var itemsFromLoot2 = DIContainerLogic.GetLootOperationService().GetItemsFromLoot(loot2);
			foreach (var item2 in itemsFromLoot2)
			{
			...
			var loot3 = DIContainerLogic.GetLootOperationService().GenerateLootPreview(eventManagerGameData.EventBalancing.EventCollectibleGeneratorItemLootTable, DIContainerInfrastructure.GetCurrentPlayer().Data.Level);
			var itemsFromLoot3 = DIContainerLogic.GetLootOperationService().GetItemsFromLoot(loot3);
			foreach (var item3 in itemsFromLoot3)
			{
	...
	private void SetLeaderboardInfo()
	{
		...
		if (m_Model.IsBossEvent)
		{
			if (DIContainerInfrastructure.GetCurrentPlayer().Data.WorldBoss.OwnTeamId == 0)
			{
				...
				this.m_TeamCompetitionPanel.SetActive(false);
				return;
			}
			StartCoroutine(this.SetupBossCooldownOrHealthBar());
			...
			{
				DIContainerLogic.EventSystemService.GetEventBossDefeatLog(this.m_Model, delegate(RESTResultEnum result)
				{
	...
	private void RequestLeaderboard()
	{
		...
		{
			DIContainerLogic.EventSystemService.SubmitEventScore(m_Model, ScoreSourceType.LeaderboardUpdate, (int)m_Model.Data.CurrentScore, OnLeaderboardFound);
		}
		else
		{
	...
	private void SetupPopupDescLabel()
	{
		...
		var collectionGroupBalancingForEvent = DIContainerLogic.EventSystemService.GetCollectionGroupBalancingForEvent(m_Model);
		if (collectionGroupBalancingForEvent != null)
		{
			...
			dictionary.Add("{value_1}", collectionGroupBalancingForEvent.ComponentRequirements.Where(r => r.RequirementType == RequirementType.HaveEventScore).FirstOrDefault().Value.ToString());
			var loot = !DIContainerInfrastructure.EventSystemStateManager.UseCollectionFallbackReward() ? collectionGroupBalancingForEvent.Reward : collectionGroupBalancingForEvent.FallbackReward;
			var loot2 = DIContainerLogic.GetLootOperationService().GenerateLoot(loot, 1);
			var itemsFromLoot = DIContainerLogic.GetLootOperationService().GetItemsFromLoot(loot2);
	...
	{
		var targetTime = DIContainerInfrastructure.GetCurrentPlayer().Data.BossStartTime + (uint)m_Model.CurrentEventBoss.BalancingData.TimeToReactivate;
	...
	private void SetupBossCompetitionSlider()
	{
		...
		var list = new List<Leaderboard.Score>(m_sortedScores);
		...
		foreach (var item in list)
		{
			...
			DebugLog.Log(GetType(), "[EPIC SERVER] ------------ score for OWN team (team1score)= " + team1score);
			DebugLog.Log(GetType(), "[EPIC SERVER] ------------ score for OPPONENT team (team2score)= " + team2score);
		}
		var team1IsWinning = true;
		m_ownTeam = null;
		m_enemyTeam = null;
		if (currentPlayer.Data.WorldBoss.OwnTeamId == 1)
		{
		...
		else
		{
			...
			var tempTeam1Score = team1score;
			team1score = team2score;
			team2score = tempTeam1Score;
		}
		team1score = Math.Max(team1score, 0);
		team2score = Math.Max(team2score, 0);
		team1IsWinning = team1score >= team2score;
		m_Team1LeadingSliderObject.SetActive(team1IsWinning);
		m_Team2LeadingSliderObject.SetActive(!team1IsWinning);
		m_Team1Points.text = team1score.ToString();
		m_Team2Points.text = team2score.ToString();
		...
		if (team1IsWinning)
		{
			...
			{
				m_Team1LeadingSliderObject.GetComponent<UISlider>().sliderValue = 0.5f * ((float)team2score / (float)team1score);
			}
		}
		else
		{
			m_Team2LeadingSliderObject.GetComponent<UISlider>().sliderValue = 0.5f * ((float)team1score / (float)team2score);
		}
		var teamColor = m_ownTeam.TeamColor;
		var teamColor2 = m_enemyTeam.TeamColor;
		...
		var list2 = Enumerable.ToList(m_TeamCompetitionPanel.GetComponentsInChildren<Transform>(true));
	...
	private void SetupBossReward()
	{
		...
		{
			DebugLog.Error(base.GetType(), "SetupBossReward: No model found. Cannot proceed.");
			return;
		}
		var collectionBalancing = this.m_Model.GetCollectionBalancing();
		...
		{
			var value = collectionBalancing.ComponentRequirements.Where(r => r.RequirementType == RequirementType.HaveEventScore).FirstOrDefault<Requirement>().Value;
			this.m_BossRewardCollectionFiller.fillAmount = Mathf.Min(1f, num / value);
			this.m_BossRewardCollectionLabel.text = Math.Min(num, value) + "/" + value;
			if (this.m_BossRewardSlot != null && DIContainerLogic.EventSystemService.GetCurrentCollectionRewardStatus() >= EventCampaignRewardStatus.chest_claimed)
			{
				var confirmedChestLootId = this.m_Model.Data.ConfirmedChestLootId;
				var inventoryItemGameData = DIContainerLogic.InventoryService.GenerateNewInventoryItemGameData(1, 1, confirmedChestLootId, 1);
				this.m_BossEliteChestButton.gameObject.SetActive(false);
				this.m_BossRewardSlot.gameObject.SetActive(true);
				this.m_BossRewardSlot.SetModel(inventoryItemGameData, null);
			}
			if (num >= value)
				this.m_BossRewardCollectionAnimator.Play("Material_Idle_Active");
			else
				this.m_BossRewardCollectionAnimator.Play("Material_Idle_Inactive");
		}
		else
		{
			this.m_BossRewardCollectionFiller.fillAmount = 0f;
			this.m_BossRewardCollectionLabel.text = num.ToString();
		}
	}
	...
	private void SetupLeaderBoardPreview()
	{
		...
		var flag = false;
		for (var i = 0; i < m_sortedScores.Count; i++)
		{
			if (m_BossPreviewLeaderboardGrid.transform.childCount > 3 && flag)
			{
				break;
			}
			var score = m_sortedScores[i];
			flag = true;
			var opponentInfoElement = UnityEngine.Object.Instantiate(m_LeaderBoardBlindPrefab);
	...
	private void SpawnBossPrefab()
	{
		...
		var characterControllerWorldMap = UnityEngine.Object.Instantiate(m_CharacterControllerPrefab);
	...
	private IEnumerator CountDownTimer()
	{
		...
		var targetTime = DIContainerLogic.EventSystemService.GetEventEndTime(m_Model.Balancing);
	...
	private void RegisterEventHandler()
	{
		...
		foreach (var item in m_RankingRay)
		{
		...
		if (m_CollectionChestButton)
		{
			m_CollectionChestButton.Clicked += OpenChestContentInfo;
		}
		...
		if (m_InvasionEasyBattleButton != null)
		{
			m_InvasionEasyBattleButton.Clicked += StartInvasionBattleEasy;
		}
		if (m_InvasionMediumBattleButton != null)
		{
			m_InvasionMediumBattleButton.Clicked += StartInvasionBattleMedium;
		}
		if (m_InvasionHardBattleButton != null)
		{
			m_InvasionHardBattleButton.Clicked += StartInvasionBattleHard;
		}
		
	}

	private void StartInvasionBattle(int difficulty)
	{
		if (m_Model.IsBossEvent || m_Model.IsCampaignEvent)	
			return;

		if (m_PossibleEventItems == null || m_PossibleEventItems.Count < difficulty + 1)
		{
			DebugLog.Warn(GetType(), "StartInvasionBattle: No EventItem found! Aborting.");
			return;
		}
		
		var validInvasionPlacements = DIContainerBalancing.EventBalancingService
			.GetBalancingDataList<EventPlacementBalancingData>()
			.Where(placement =>
				placement.NameId.Contains("eventlocation") &&
				DIContainerLogic.RequirementService.CheckGenericRequirements(
					DIContainerInfrastructure.GetCurrentPlayer(),
					placement.SpawnAbleRequirements))
			.ToArray();

		if (validInvasionPlacements.Length <= 0)
		{
			DebugLog.Error(GetType(), "StartInvasionBattle: No Event Placement found!");
			return;
		}
		
		var worldMapStateMgr = DIContainerInfrastructure.LocationStateMgr as WorldMapStateMgr;
		if (worldMapStateMgr != null)
			worldMapStateMgr.ShowBattlePreperationScreenForEvent(m_PossibleEventItems[difficulty], validInvasionPlacements[Random.Range(0, validInvasionPlacements.Length)], true);

		StartCoroutine(LeaveCoroutine(false));
	}

	private void StartInvasionBattleEasy()
	{
		StartInvasionBattle(0);
	}

	private void StartInvasionBattleMedium()
	{
		StartInvasionBattle(1);
	}

	private void StartInvasionBattleHard()
	{
		StartInvasionBattle(2);
	}
	...
	private void StartBossBattle()
	{
		...
		var list = DIContainerInfrastructure.GetCurrentPlayer().InventoryGameData.Items[InventoryItemType.EventBossItem];
		...
		var searchString = string.Concat("bosslocation_", Random.Range(1, 7).ToString("00"), "_", eventItemGameData.Name.Replace("_boss", string.Empty));
		DIContainerBalancing.EventBalancingService.TryGetBalancingData<EventPlacementBalancingData>(searchString, out balancing);
	...
	private void SetupCollectionDetails()
	{
		...
		m_PigObjectivesRoot.SetActive(false);
		var gameObject = DIContainerInfrastructure.EventSystemStateManager.InstantiateEventObject("Entrance", m_CampaignEntranceRoot);
		...
		for (var i = 0; i < m_Model.CurrentMiniCampaign.CollectionGroupBalancing.ComponentRequirements.Count; i++)
		{
			var collectionItemSlot = m_CollectionResources[i];
			var nameId = m_Model.CurrentMiniCampaign.CollectionGroupBalancing.ComponentRequirements[i].NameId;
			var value = m_Model.CurrentMiniCampaign.CollectionGroupBalancing.ComponentRequirements[i].Value;
		...
		m_CollectionRewardLabel.text = DIContainerInfrastructure.GetLocaService().Tr(chestBalancing.LocaId + "_desc");
		if (m_CollectionReward != null)
		{
			if (DIContainerLogic.EventSystemService.GetCurrentCollectionRewardStatus() < EventCampaignRewardStatus.chest_claimed)
				return;
			var rewardItem = DIContainerLogic.InventoryService.GenerateNewInventoryItemGameData(1, 1, m_Model.Data.ConfirmedChestLootId, 1);
			m_CollectionChestButton.gameObject.SetActive(false);
			m_ChestButtonSprite.transform.parent.gameObject.SetActive(false);
			m_CollectionReward.gameObject.SetActive(true);
			m_CollectionReward.SetModel(rewardItem);
		}
	}
	...
	private void SetupInvasionDetails()
	{
		...
		var num = m_CharacterPanel.clipRange.z;
		var num2 = 1;
		foreach (var possibleEventItem in m_PossibleEventItems)
		{
			if (possibleEventItem.BalancingData.ItemType == InventoryItemType.EventBattleItem)
			{
				var eventObjectivesSlot = UnityEngine.Object.Instantiate(m_ObjectivesSlotPrefab);
			...
			else
			{
				...
				{
					var assetBaseId = possibleEventItem.BalancingData.AssetBaseId;
					var gameObject = DIContainerInfrastructure.PropLiteAssetProvider().InstantiateObject(assetBaseId, !m_AdditionRoot ? base.transform : m_AdditionRoot, Vector3.zero, Quaternion.identity);
					if (gameObject)
					{
	...
	private void SetupRankingRay()
	{
		...
		var dictionary = EvaluateOpponentScoreOffsets();
		var rankedPlayers = m_Model.GetRankedPlayers(false);
		for (var i = 0; i < rankedPlayers.Count && i < m_RankingRay.Count; i++)
		{
	...
	{
		var dictionary = new Dictionary<int, OpponentInfoAlignment>();
	...
	{
		if (DIContainerLogic.InventoryService.GetItemValue(DIContainerInfrastructure.GetCurrentPlayer().InventoryGameData, "event_energy") >= DIContainerBalancing.GameConstantsBalancingDataProvider.EventItemMaxCap)
		{
		...
		var dateTime = DIContainerLogic.GetTimingService().GetDateTimeFromTimestamp(DIContainerInfrastructure.GetCurrentPlayer().Data.LastEnergyAddTime).AddSeconds(DIContainerBalancing.GameConstantsBalancingDataProvider.EnergyRefreshTimeInSeconds);
		DateTime trustedTime;
		if (DIContainerLogic.GetTimingService().TryGetTrustedTime(out trustedTime))
		{
			var span = dateTime - trustedTime;
			var localizedText = DIContainerInfrastructure.GetLocaService().Tr("eventdetail_next_energy").Replace("{value_1}", DIContainerInfrastructure.GetFormatProvider().GetDurationFormatStandard(span));
