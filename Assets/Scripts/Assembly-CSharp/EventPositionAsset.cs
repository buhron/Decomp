...
public class EventPositionAsset : MonoBehaviour
{
	...
	public void SetItem(EventItemGameData eventItem, bool instant = false)
	{
		...
		var assetBaseId = m_placedItem.BalancingData.AssetBaseId;
		if (assetBaseId.StartsWith("pig_"))
		{
			var characterControllerWorldMap = Object.Instantiate(m_CampCharacterControllerPrefab);
			var balancingData = DIContainerBalancing.Service.GetBalancingData<PigBalancingData>(assetBaseId);
		...
		foreach (var item in m_ObjectsToEnable)
		{
	...
	private IEnumerator SpawnAddition()
	{
		...
		foreach (var o in m_ObjectsToEnable)
		{
			o.SetActive(true);
		}
		if (!m_instant)
