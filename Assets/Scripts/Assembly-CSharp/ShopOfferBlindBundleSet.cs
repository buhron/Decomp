...
public class ShopOfferBlindBundleSet : ShopOfferBlindBundleBase
{
	...
	public override void SetModel(BasicShopOfferBalancingData model, ShopWindowStateMgr stateMgr)
	{
		...
		var equipment = m_Items.FirstOrDefault(i => i.ItemBalancing.ItemType == InventoryItemType.MainHandEquipment) as EquipmentGameData;
		var equipment2 = m_Items.FirstOrDefault(i => i.ItemBalancing.ItemType == InventoryItemType.OffHandEquipment) as EquipmentGameData;
	...
	private void HandleSetItemInfo(BundleSaleSetInfo setItemInfo, EquipmentGameData equipment)
	{
		...
		var skillBattleDataBase = equipment.SetItemSkill.GenerateSkillBattleData();
