...
public class PopupRankUpStateMgr : MonoBehaviour
{
	...
	public WaitTimeOrAbort ShowRankUpPopup(Dictionary<string, int> classesWithOldLevels)
	{
		...
		var springPanel = GetComponentInChildren<SpringPanel>();
		if (springPanel != null)
		{
			Destroy(springPanel);
		}
		foreach (var key in classesWithOldLevels.Keys)
		{
			...
			{
				var rankUpClassBlind = Object.Instantiate(m_RankUpClassBlindPrefab, Vector3.zero, Quaternion.identity) as RankUpClassBlind;
