...
public class CampStateMgr : BaseCampStateMgr
{
	...
	[SerializeField]
	private CampProp m_SkinOverviewButton;
	...
	private void Awake()
	{
		...
		if (m_FriendInfo != null && ClientInfo.IsFriend)
		{
			if (ClientInfo.InspectedFriend.PublicPlayerData.Inventory != null)
			{
				...
				var itemValue = DIContainerLogic.InventoryService.GetItemValue(inventory, "star_collection");
				if (itemValue > 0)
				{
	...
	private IEnumerator Start()
	{
		...
		if (m_FriendInfo && ClientInfo.IsFriend)
		{
		...
		if (m_Birds.Count > 0)
		{
			...
			for (var i = 0; i < m_Birds.Count; i++)
			{
				yield return new WaitForEndOfFrame();
				var bird = m_Birds[i];
				var characterController = Object.Instantiate(m_CharacterCampPrefab);
		...
		var hotLinkName = DIContainerInfrastructure.GetCoreStateMgr().m_HotLinkItemName;
		if (!string.IsNullOrEmpty(hotLinkName))
		{
			...
			case "story_goldenpig":
				...
				break;
			default:
				if (hotLinkName.StartsWith("bird_manager"))
				{
					var birdId = hotLinkName.Split(':')[1];
					GoToBirdManager(birdId);
					hotLinkName = null;
				}
				else if (hotLinkName.StartsWith("Starcollection"))
				{
	...
	{
		var source = DIContainerBalancing.Service.GetBalancingDataList<HotspotBalancingData>().ToList();
		var dungeonHotSpots = source.Where(h => h.NameId.Contains("_dungeon") && !(h is ChronicleCaveHotspotBalancingData) && h.EnterRequirements != null && h.EnterRequirements.FirstOrDefault().RequirementType == RequirementType.IsSpecificWeekday).ToList();
		var list = new List<DungeonInfoPopup.DungeonInfo>();
		dungeonHotSpots = OrderHotspotsByWeekday(dungeonHotSpots);
		var flag = false;
		for (var i = 0; i < dungeonHotSpots.Count; i++)
		{
			var hotspotBalancingData = dungeonHotSpots[i];
			var item = default(DungeonInfoPopup.DungeonInfo);
			item.LocalizedName = DIContainerInfrastructure.GetLocaService().GetZoneName(hotspotBalancingData.ZoneLocaIdent);
			var balancingData = DIContainerBalancing.Service.GetBalancingData<BattleBalancingData>(hotspotBalancingData.BattleId.LastOrDefault());
			var balancingData2 = DIContainerBalancing.Service.GetBalancingData<BattleBalancingData>(hotspotBalancingData.BattleId.LastOrDefault() + "_hard");
			var levelRange = DIContainerInfrastructure.GetCurrentPlayer().GetLevelRange();
			var nameId = balancingData.LootTableWheel.FirstOrDefault().Key.Replace("{levelrange}", levelRange.ToString("00"));
			var nameId2 = balancingData2.LootTableWheel.FirstOrDefault().Key.Replace("{levelrange}", levelRange.ToString("00"));
			var balancingData3 = DIContainerBalancing.Service.GetBalancingData<LootTableBalancingData>(nameId);
			var balancingData4 = DIContainerBalancing.Service.GetBalancingData<LootTableBalancingData>(nameId2);
			...
			{
				var num2 = DIContainerLogic.GetBonusEventService.m_CurrentValidBalancing.BonusFactor / 100f;
			...
			var flag2 = DIContainerLogic.WorldMapService.IsHotspotEnterable(DIContainerInfrastructure.GetCurrentPlayer(), value, out firstFailedReq);
			var isFree = true;
	...
	{
		var array = new HotspotBalancingData[7];
		for (var i = 0; i < DungeonHotSpots.Count; i++)
		{
			var hotspotBalancingData = DungeonHotSpots[i];
	...
	public void RefreshLeveledCauldron()
	{
		...
		for (var i = 0; i < m_CauldronCampLeveled.Count; i++)
		{
	...
	private void OpenDungeonInfo(BasicItemGameData whyThisParamIDontEvent = null)
	{
		...
		{
			DIContainerInfrastructure.GetCoreStateMgr().m_DungeonsLockedPopup.Show();
		}
	...
	private void OpenSkinInfo(BasicItemGameData whyThisParamIDontEvent = null)
	{
		ShowSkinUI();
	}
	...
	private void ShowSkinUI()
	{
		if (m_SkinUI == null)
		{
			DIContainerInfrastructure.GetCoreStateMgr().SceneLoadingMgr.AddUILevel("Window_ClassUpgradeCollection", OnSkinOverviewLoaded);
		}
		else
		{
			m_SkinUI.Show();
	...
	public void OnSkinOverviewLoaded()
	{
		m_SkinUI = FindObjectOfType<SkinOverview>();
		m_SkinUI.Show();
	}
	...
	private void RefreshLeveledForge()
	{
		...
		for (var i = 0; i < m_ForgeCampLeveled.Count; i++)
		{
			if (i + 1 == m_ForgeLevel)
			{
	...
	public override void RegisterEventHandler()
	{
		...
		if (m_DungeonInfoButton)
		{
			m_DungeonInfoButton.OnPropClicked += OpenDungeonInfo;
		}
		if (m_SkinOverviewButton)
		{
			m_SkinOverviewButton.OnPropClicked += OpenSkinInfo;
		}
		foreach (var item in m_CauldronCampLeveled)
		{
			item.OnPropClicked += CauldronCampOnPropClicked;
		}
		foreach (var item2 in m_ForgeCampLeveled)
		{
	...
	public void GoToBirdManager(string birdNameId)
	{
		...
		var birdGameData = m_Birds.Where(b => b.BalancingData.NameId == birdNameId).FirstOrDefault();
	...
	{
		RefreshLeveledCauldron();
		var campProp = m_CauldronCampLeveled.FirstOrDefault();
		if (campProp != null)
		{
			campProp.Awake();
		}
