...
public class CharacterControllerBattleGround : CharacterControllerBattleGroundBase
{
	...
	private void OnDestroy()
	{
		...
		{
			var gameObject = LastingVisualEffects.Values.ElementAt(i);
			if (gameObject)
			{
		...
		if (m_AssetController && DIContainerInfrastructure.GetCharacterAssetProvider(false))
		{
	...
	public override void RecreateAssetController()
	{
		...
		if (m_Model != null && m_AssetController == null)
		{
			...
			var bossAssetController = m_AssetController as BossAssetController;
			if (bossAssetController != null)
			{
				bossAssetController.SetController(this);
			}
			var tentacleAssetController = m_AssetController as TentacleAssetController;
	...
	public override void SpawnHealthBar()
	{
		...
		if (!m_HealthBar)
		{
			...
			if (GetModel() is BossCombatant || m_Model.CombatantNameId.Contains("hogrider"))
			{
	...
	private IEnumerator SpawnLevelUpEffect()
	{
		...
		if (DIContainerLogic.GetVisualEffectsBalancing().TryGetVisualEffectSetting("LevelUp", out setting))
		{
			...
			var effect = DIContainerInfrastructure.GetBattleEffectAssetProvider().InstantiateObject(setting.VisualEffects[0].PrefabName, m_AssetController.BodyRoot, m_AssetController.BodyRoot.position, Quaternion.identity);
	...
	public override IEnumerator DoTurn(int turn)
	{
		...
		CheckForTauntTarget();
		if (m_Model is BirdCombatant && m_BattleMgr.AutoBattle && m_BattleMgr.Model.IsRageFull(Faction.Birds) && !m_BattleMgr.Model.m_CombatantsPerFaction[Faction.Birds].Any(b => b is BirdCombatant && (b as BirdCombatant).UseRage) && m_BattleMgr is BattleMgr)
		{
		...
		{
			if (m_BattleMgr.Model.m_CombatantsPerFaction[Faction.Pigs].Any(p => p.CurrentHealth != 0f))
			{
		...
		var chargeEffects = m_Model.CurrrentEffects.Values.Where(e => e.m_Effects.Any(ef => ef.EffectTrigger == EffectTriggerType.OnCharge)).ToList();
		if (m_Model is PigCombatant && chargeEffects.Count > 0)
		{
			PlayAffectedAnimation();
			for (var j = chargeEffects.Count - 1; j >= 0; j--)
			{
				if ((m_Model as PigCombatant).ChargeDone)
				{
					var effectGameData = chargeEffects[j];
		...
		{
			PlayAffectedAnimation();
			for (var i = chargeEffects.Count - 1; i >= 0; i--)
			{
				if ((m_Model as BossCombatant).ChargeDone)
				{
					var effectGameData2 = chargeEffects[i];
	...
	private IEnumerator DoTurnAfterInput()
	{
		...
		if (m_SkillToDo != null)
		{
			...
			{
				var aiTurn = DIContainerLogic.GetBattleService().m_PvpIntelligence.GetCommand(GetModel());
			...
			var lightningBird = GetModel().CombatantClass != null && GetModel().CombatantClass.BalancingData.NameId.Contains("lightningbird");
			var skulker = GetModel().CombatantClass != null && GetModel().CombatantClass.BalancingData.NameId.Contains("skulker");
			...
			{
				var attack = m_Model.GetSkill(0) as AttackSkillTemplate;
			...
			{
				foreach (var enemy in m_BattleMgr.Model.m_CombatantsByInitiative.Where(c => c.CombatantFaction != m_Model.CombatantFaction))
				{
					DIContainerLogic.GetBattleService().ApplyEffectsOnTriggerType(1f, EffectTriggerType.AfterEnemyRageUsed, enemy, m_Model);
				}
				foreach (var ally in m_BattleMgr.Model.m_CombatantsByInitiative.Where(c => c.CombatantFaction == m_Model.CombatantFaction))
				{
	...
	{
		var attackTarget = m_Model.AttackTarget;
		yield return new WaitForSeconds(0.5f);
		var skill = m_Model.GetSkills().FirstOrDefault(s => s.Model.Balancing.TargetType == SkillTargetTypes.Attack);
		var copyOffset = Mathf.Sign(m_Model.CombatantView.transform.position.x) * 100f;
		...
		m_Model.Defeated += skillTarget_Defeated;
		var cachedModifier = m_Model.DamageModifier;
		m_Model.DamageModifier = m_BattleMgr.m_IllusionistDamageFactor / 100f;
		var originalAsset = m_Model.CombatantView.m_AssetController;
		...
		var motion = m_BattleMgr.m_IllusionistCopy.gameObject.AddComponent<CHMotionTween>();
	...
	private IEnumerator MirrorAttackMelee()
	{
		...
		var targetOffset = new Vector3((0f - (m_BattleMgr.m_IllusionistCopy.OffsetFromEnemyX + attackTarget.CombatantView.m_AssetController.OffsetFromEnemyX)) * m_BattleMgr.m_IllusionistCopy.transform.localScale.x, 0f, -15f);
		...
		var duration = DIContainerLogic.GetPacingBalancing().TimeFromFocusPosToAttackPosInSec;
		motion.m_DurationInSeconds = duration;
		motion.Play();
		var cachedModifier = m_Model.DamageModifier;
		m_Model.DamageModifier = m_BattleMgr.m_IllusionistDamageFactor / 100f;
		yield return m_Model.CombatantView.StartCoroutine(skill.DoAction(m_BattleMgr.Model, m_Model, attackTarget, false, true));
	...
	{
		if (m_BattleMgr.m_IllusionistCopy && m_BattleMgr.m_CurrentOriginal != null)
		{
	...
	{
		if (GetModel().ActedThisTurn && m_CurrentControlHUD && m_CurrentControlHUD.gameObject.activeInHierarchy)
		{
			ActivateControlHUD(false);
		}
	}
	...
	private void HandleDrag()
	{
		...
		{
			if (m_CurrentGlow && m_CurrentGlow.gameObject.activeInHierarchy)
			{
		...
		{
			for (var i = 0; i < m_BattleMgr.Model.m_CombatantsByInitiative.Count; i++)
			{
				var combatant = m_BattleMgr.Model.m_CombatantsByInitiative[i];
		...
		for (var j = 0; j < m_BattleMgr.Model.m_CombatantsByInitiative.Count; j++)
		{
			var combatant2 = m_BattleMgr.Model.m_CombatantsByInitiative[j];
			var skill = combatant2.CombatantFaction != m_Model.CombatantFaction ? m_Model.GetSkills()[0] : m_Model.GetSkills()[1];
	...
	public void SetDragHUD(Vector3 currentScreenPos)
	{
		...
		var ray = m_sceneryCamera.ScreenPointToRay(currentScreenPos);
		RaycastHit hitInfo;
		if (Physics.Raycast(ray, out hitInfo, 10000f, (1 << LayerMask.NameToLayer("TutorialScenery")) | (1 << LayerMask.NameToLayer("Scenery"))))
		{
			if (m_cachedOverCharacterTransform != hitInfo.transform)
			{
				...
				if (cachedOverCharacter != m_cachedOverCharacter)
				{
					...
					{
						var skill = m_cachedOverCharacter.GetModel().CombatantFaction != m_Model.CombatantFaction ? m_Model.GetSkills()[0] : m_Model.GetSkills()[1];
			...
			if (m_cachedOverCharacter && !(m_cachedOverCharacter == this) && (m_tauntedTarget == null || m_cachedOverCharacter.GetModel() == m_tauntedTarget || m_tauntedTarget.CombatantFaction != m_cachedOverCharacter.GetModel().CombatantFaction))
			{
				var num = 1f;
	...
	{
		for (var i = 0; i < m_BattleMgr.m_CharacterInteractionBlockedItems.Count; i++)
		{
	...
	public void HandleRelease(Vector2 releasePos)
	{
		...
		var ray = m_sceneryCamera.ScreenPointToRay(releasePos);
		RaycastHit hitInfo;
		...
		if (component != null)
		{
			var skill = component.GetModel().CombatantFaction != m_Model.CombatantFaction ? m_Model.GetSkills()[0] : m_Model.GetSkills()[1];
	...
	{
		if (m_CachedTransform && Math.Abs(m_CachedZ - m_CachedTransform.position.y) > 1f && m_HealthBar)
		{
