...
public class CHMotionFadeAndTween : MonoBehaviour
{
	...
	private void Update()
	{
		...
		var num = (Time.time - m_StartTimestamp) / m_KeyDuration;
		if (num >= 1f)
		{
			if (m_IsLooping)
			{
		...
		{
			a = Vector3.zero;
			foreach (var item in m_AnimationCurvesPerAxis)
			{
				switch (item.Axis)
				{
				case Axis.X:
		...
		if (m_FadeTimeInSeconds > 0f)
		{
			var alphaOfAllChilds = 1f;
			var num2 = Time.time - m_StartTimestamp;
			if (num2 <= m_FadeTimeInSeconds)
			{
				alphaOfAllChilds = num2 / m_FadeTimeInSeconds;
			}
			var num3 = m_KeyDuration - num2;
