...
public class AdServiceBeaconImpl : IAdService
{
	...
	{
		AddPlacement(AdPlacementLookup.EVENT_ENERGY);
		AddPlacement(AdPlacementLookup.EVENT_CAMPAIGN);
	}
	...
	private bool HandleActionInvoked(string placement, string action)
	{
		...
		if (text.Equals("OpenShop"))
		{
			...
			{
				var battleUIStateMgr = UnityEngine.Object.FindObjectOfType<BattleUIStateMgr>();
	...
	public bool ShowAd(string placementId)
	{
		...
		{
			var showInterstitialRequirements = DIContainerConfig.GetClientConfig().ShowInterstitialRequirements;
			...
			var flag = m_adService.Show(placementId);
			if (flag)
			{
		...
		{
			var flag2 = m_adService.Show(placementId);
			if (flag2)
			{
				DIContainerInfrastructure.GetCurrentPlayer().Data.LastAdShownTime = DIContainerLogic.GetDeviceTimingService().GetCurrentTimestamp();
			}
		...
		var flag3 = m_adService.Show(placementId);
	...
	private bool IsUserInAdCooldown()
	{
		...
		var timeSpan = trustedTime - DIContainerLogic.GetTimingService().GetDateTimeFromTimestamp(DIContainerInfrastructure.GetCurrentPlayer().Data.LastAdShownTime);
	...
	{
		var adCooldownBalancing = DIContainerBalancing.GameConstantsBalancingDataProvider.AdCooldownBalancing;
		var dictionary = new Dictionary<int, int>();
		for (var i = 0; i < adCooldownBalancing.Count; i++)
		{
			if ((i & 1) != 0)
			{
				dictionary.Add((int)adCooldownBalancing[i - 1], (int)adCooldownBalancing[i]);
			}
		}
		var timeSinceCreation = trustedTime - DIContainerLogic.GetTimingService().GetDateTimeFromTimestamp(DIContainerInfrastructure.GetCurrentPlayer().Data.CreationDate);
		...
		var source = dictionary.Keys.Where(key => key <= timeSinceCreation.TotalDays);
		var num = source.Max();
		DebugLog.Log(GetType(), string.Concat(" Checking cooldown: timeSinceCreation: ", timeSinceCreation, ", user is past x days: ", num, ", using cooldown: ", dictionary[num]));
	...
	{
		return DIContainerLogic.InventoryService.GetItemValue(DIContainerInfrastructure.GetCurrentPlayer().InventoryGameData, "unlock_sponsored_rewards") > 0 && GetState(placementId) == 3;
	}

	public bool IsNewsFeedShowPossible(string placementId)
	{
		var state = m_adService.GetState(placementId);
