...
public class GameProgressPopup : MonoBehaviour
{
	...
	private IEnumerator SwitchTab(GameProgressTab newTab)
	{
		if (!m_AreaSelectionAnimation || !m_ContentBoardAnimation || !m_FooterAnimation)
		{
			...
			var gameProgressPopup = this;
			IEnumerator routine;
			if (newTab == GameProgressTab.Progress)
			{
				var enumerator = SetupProgress();
				routine = enumerator;
			}
			else
			{
	...
	private IEnumerator SetupProgress()
	{
		...
		m_ProgressParentFooter.SetActive(true);
		m_StarCollectionFooter.SetActive(false);
		foreach (var leveledProp in m_LeveledCampProps)
		{
			if (leveledProp && leveledProp.GetModel() != null)
			{
				leveledProp.SetCounter(leveledProp.GetModel().Data.Level);
			}
		}
		foreach (var stackingCampProp in m_StackingCampProps)
		{
			if (stackingCampProp && stackingCampProp.GetModel() != null)
			{
		...
		while (m_CampProps.Count(p => !p.m_IsInitialized) > 0)
		{
	...
	private IEnumerator CreateStarCollection()
	{
		...
		var currentStars = DIContainerLogic.InventoryService.GetItemValue(m_inventory, "star_collection");
		var maxStars = 0;
		...
		foreach (var offer in DIContainerLogic.GetShopService().GetShopOffers(player, "shop_star_rewards"))
		{
			...
			foreach (var req in offerBalancing.BuyRequirements)
			{
			...
			var rewards = DIContainerLogic.GetShopService().GetShopOfferContent(player, offerBalancing, DIContainerLogic.GetSalesManagerService().GetOfferSaleDetails(offer.NameId));
			foreach (var tempReward in rewards)
			{
			...
			var OfferListElement = Object.Instantiate(m_StarRewardPrefab);
			OfferListElement.name = neededStars + "_ListElement_BattleStarsReward";
			var cost = OfferListElement.GetComponentInChildren<ResourceCostBlind>();
			if (cost != null)
			{
			...
			var lootDisplay = OfferListElement.GetComponent<LootDisplayContoller>();
			...
			if ((float)currentStars >= neededStars)
			{
				...
				var popupname = offerBalancing.NameId.Replace("offer_star_reward", "star_popup");
				var isNew = false;
