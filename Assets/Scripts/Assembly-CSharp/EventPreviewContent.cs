...
public class EventPreviewContent : MonoBehaviour
{
	...
	[SerializeField]
	private UILabel m_chestDescLabel;

	public void SetModel(EventManagerGameData eventManager, bool isStarting)
	{
		...
		if (m_chestDescLabel)
			m_chestDescLabel.text = DIContainerInfrastructure.GetLocaService().Tr("teaser_chest_" + eventManager.EventBalancing.LocaBaseId);

		if (m_headerLabel)
		{
			m_headerLabel.text = isStarting 
				? DIContainerInfrastructure.GetLocaService().Tr("event_subheader_started") 
				: DIContainerInfrastructure.GetLocaService().Tr("eventpreview_subheader");
		}
		
		m_CharacterSlots = m_CharacterRoot.GetComponentsInChildren<EventPreviewCharacterSlot>(true).ToList();
		foreach (var characterSlot in m_CharacterSlots)
		{
			var model = UnityEngine.Object.Instantiate(m_CharacterControllerPrefab);
	...
	private void SetEventRewardOption()
	{
		...
		string lootTableId;
		var chestReward = DIContainerLogic.EventSystemService
			.GetAvailableChestReward(DIContainerInfrastructure.GetCurrentPlayer(), out lootTableId)
			.FirstOrDefault();
		var rewardItemType = chestReward.ItemBalancing.ItemType;
		
		m_ClassRewardRoot.SetActive(rewardItemType == InventoryItemType.Class);
		m_SkinRewardRoot.SetActive(rewardItemType != InventoryItemType.Class);
	}
	...
	private IEnumerator CountDownTimer()
	{
		...
		var targetTime = DIContainerLogic.EventSystemService.GetTeasingEndTime(m_model.Balancing);
