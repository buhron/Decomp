...
public abstract class CharacterControllerBattleGroundBase : MonoBehaviour
{
	...
	protected bool m_RetargetDeadCounterModel;
	...
	public void CheckForTauntTarget()
	{
		...
		for (var i = 0; i < m_Model.CurrrentEffects.Count; i++)
		{
			var battleEffectGameData = m_Model.CurrrentEffects.Values.ElementAt(i);
	...
	public void ReSizeCollider()
	{
		...
		if (m_BoxCollider == null && GetComponent<Collider>())
		{
		...
		var num2 = Mathf.Lerp(0.85f, 1.33f, t);
	...
	protected virtual void OnHealthChanged(float oldHealth, float newHealth)
	{
		...
		{
			var scrollingCombatText = InstantiateScrollingCombatText(m_ScrollingCombatTextPrefab);
		...
		{
			var scrollingCombatText2 = InstantiateScrollingCombatText(m_ScrollingCombatTextPrefab);
		...
		{
			var scrollingCombatText3 = InstantiateScrollingCombatText(m_ScrollingCombatTextPrefab);
	...
	{
		return Instantiate(prefab, m_CachedTransform.position + Vector3.Scale(m_AssetController.CombatTextPosition, new Vector3(m_AssetController.transform.localScale.x, m_AssetController.transform.localScale.y, 1f * prefab.transform.localScale.z)), Quaternion.identity);
	}
	...
	{
		var scrollingCombatText = InstantiateScrollingCombatText(m_ScrollingCombatTextWithLabelPrefab);
		var gameObject = UnityEngine.Object.Instantiate(m_CriticalAdditionalEffect);
	...
	private void TentacleBossReaction()
	{
		...
		{
			var componentInChildren = m_BattleMgr.GetComponentInChildren<BossAssetController>();
	...
	{
		var transform = !m_AssetController.OffHandBone ? m_AssetController.BodyCenter : m_AssetController.OffHandBone;
	...
	private void m_Model_PerkUsed(PerkType type, ICombatant target)
	{
		...
		{
			var perkVisualEffectTarget = GetPerkVisualEffectTarget(type, m_Model, target);
			for (var i = 0; i < setting.VisualEffects.Count; i++)
			{
				var effect = setting.VisualEffects[i];
	...
	private void CheckKillAchievement()
	{
		...
		if (!achievementTracking.DefeatedClasses.Contains(GetModel().CombatantClass.BalancingData.NameId))
		{
		...
		for (var i = 0; i < list.Count; i++)
		{
			...
			if (!string.IsNullOrEmpty(classItemBalancingData.RestrictedBirdId) && !classItemBalancingData.Inactive)
			{
				num++;
			}
		}
		...
		{
			var achievementIdForStoryItemIfExists = DIContainerInfrastructure.GetAchievementService().GetAchievementIdForStoryItemIfExists("defeatAllClasses");
	...
	{
		var source = m_BattleMgr.Model.m_CombatantsByInitiative.Where(c => c.CombatantFaction != m_Model.CombatantFaction && c.AttackTarget == m_Model && c.IsCharging);
		for (var i = 0; i < source.Count(); i++)
		{
			var combatant = source.ElementAt(i);
			var model = GetModel();
	...
	{
		var source = m_BattleMgr.Model.m_CombatantsByInitiative.Where(c => c.CombatantFaction != m_Model.CombatantFaction);
		for (var i = 0; i < source.Count(); i++)
		{
			var combatant = source.ElementAt(i);
			var characterControllerBattleGround = combatant.CombatantView as CharacterControllerBattleGround;
			if (characterControllerBattleGround && characterControllerBattleGround.m_tauntedTarget == m_Model)
			{
	...
	public void ActivateRageSkill()
	{
		...
		{
			m_Model.AttackTarget = m_BattleMgr.Model.m_CombatantsByInitiative.FirstOrDefault(c => c.CombatantFaction == DIContainerLogic.GetBattleService().GetOppositeFaction(m_Model.CombatantFaction));
		}
		else
		{
			m_Model.AttackTarget = m_BattleMgr.Model.m_CombatantsByInitiative.FirstOrDefault(c => c.CombatantFaction == m_Model.CombatantFaction);
		}
	...
	public IEnumerator DoConsumableSkill(ConsumableItemGameData consumable)
	{
		...
		var dictionary = potionsUsed = m_BattleMgr.Model.m_PotionsUsed;
		string key;
		var key2 = key = consumable.Name;
		var num = potionsUsed[key];
		dictionary[key2] = num + 1;
		var values = new Dictionary<string, string> { { "ConsumableName", consumable.Name } };
		ABHAnalyticsHelper.AddPlayerStatusToTracking(values);
		DIContainerInfrastructure.GetAnalyticsSystem(true).LogEventWithParameters(ABHAnalyticsEvents.ConsumableUsed, values);
		...
		if (consumable.ConsumableSkill.Balancing.TargetType == SkillTargetTypes.Attack)
		{
			...
			yield return StartCoroutine(consumable.CombatSkill.DoAction(m_BattleMgr.Model, m_Model, m_BattleMgr.Model.m_CombatantsByInitiative.FirstOrDefault(c => c.CombatantFaction != m_Model.CombatantFaction)));
		}
		else
		{
	...
	public float PlayGoToBasePositionFromStartPosition()
	{
		...
		var num = DIContainerLogic.GetPacingBalancing().BaseBattleEnterDelay;
	...
	private IEnumerator PlaySpawnWithDelay()
	{
		...
		m_Model.CombatantView.m_AssetController.PlayAnimationsQueued(new List<string>{ "Spawn", "Idle" });
		m_hidden = false;
		...
		if (GetComponentInChildren<CharacterHealthBar>())
		{
	...
	public float PlayGoToStartPositionFromBasePosition(bool destroy)
	{
		...
		var baseBattleEnterDelay = DIContainerLogic.GetPacingBalancing().BaseBattleEnterDelay;
		if (m_TweenMovement)
		{
	...
	{
		if (m_TweenMovement == null || m_BattleMgr == null || m_AssetController == null || m_Model == null ||
		    m_Model.IsBanner ||
		    (!m_AssetController.HasNoBoneAnimation && m_AssetController.m_BoneAnimation != null && m_AssetController.m_BoneAnimation.IsPlaying("Defeated")))
		{
			return 0f;
		}

		var isBoss = false;
		if (m_Model is BossCombatant && m_AssetController as BossAssetController != null)
		{
			if ((m_AssetController as BossAssetController).m_Stationary)
				return 0f;
		...
		{
			SetupTweenMovementPositions(base.transform, Vector3.zero, isBoss ? transform.parent : m_BattleMgr.m_PigCenterPosition, CachedUnfocusedPos);
		}
		
		m_TweenMovement.m_DurationInSeconds = DIContainerLogic.GetPacingBalancing().TimeFromFocusPosToBasePosInSec;
		m_TweenMovement.Play();
		PlayMoveAnim();
	...
	public void BeSheltered()
	{
		if (!m_Model.IsBanner)
		{
			...
			var timeToJumpInFrontOfAlly = DIContainerLogic.GetPacingBalancing().TimeToJumpInFrontOfAlly;
	...
	public float Shelter(ICombatant victim)
	{
		...
		if (victim.IsBanner)
		{
			var endOffset = Vector3.zero;
			var num = m_Model.CombatantFaction != 0 ? -1f : 1f;
	...
	public float PlayGoToFocusPosition()
	{
		...
		var num = Vector3.Magnitude(base.transform.position - (m_BattleMgr.m_BirdCenterPosition.position + m_CachedFocusPos));
		if (num < 1f)
		{
			return 0f;
		}
		...
		else if (m_Model is BossCombatant && !(m_AssetController as BossAssetController).m_Stationary)
		{
			var assetController = (BossAssetController)m_AssetController;
			SetupTweenMovementPositions(null, Vector3.zero, null, assetController.m_FocusOffset);
			assetController.PlayMoveAnim();
		}
		else
		{
			SetupTweenMovementPositions(null, Vector3.zero, m_BattleMgr.m_PigFocusPosition, new Vector3(0f, 0f, -3.5f));
			PlayMoveAnim();
		}
		var timeFromBasePosToFocusPosInSec = DIContainerLogic.GetPacingBalancing().TimeFromBasePosToFocusPosInSec;
	...
	public float PlayGoToAttackPosition(bool centerPosition = false, bool focusPosition = false)
	{
		...
		{
			var endOffset = new Vector3((0f - (m_AssetController.OffsetFromEnemyX + m_Model.AttackTarget.CombatantView.m_AssetController.OffsetFromEnemyX)) * m_AssetController.transform.localScale.x, 0f, OffsetFromEnemyZ);
		...
		{
			var endOffset2 = new Vector3((m_AssetController.OffsetFromEnemyX + m_Model.AttackTarget.CombatantView.m_AssetController.OffsetFromEnemyX) * m_AssetController.transform.localScale.x, 0f, OffsetFromEnemyZ);
			if (m_Model.AttackTarget.IsBanner)
			{
		...
		var timeFromFocusPosToAttackPosInSec = DIContainerLogic.GetPacingBalancing().TimeFromFocusPosToAttackPosInSec;
	...
	protected void RegisterForInput()
	{
		...
		for (var num = m_BattleMgr.Model.m_CombatantsByInitiative.Count - 1; num >= 0; num--)
		{
			var combatant = m_BattleMgr.Model.m_CombatantsByInitiative[num];
			if (combatant != null && combatant.CombatantView)
			{
				combatant.CombatantView.ReSizeCollider();
			}
		}
		if (m_BattleMgr.Model.m_CombatantsByInitiative.Any(c => c.CanUseConsumable))
		{
	...
	public void ReviveCombatant(ICombatant combatant, float healthPercentage)
	{
		...
		DIContainerLogic.GetBattleService().HealCurrentTurn(combatant, m_BattleMgr.Model, true, true, false, true);
	...
	protected void DeregisterFromInput()
	{
		...
		{
			var combatant = m_BattleMgr.Model.m_CombatantsByInitiative[num];
			var combatantView = combatant.CombatantView;
	...
	{
		if (m_AssetController && (m_AssetController.HasNoBoneAnimation || !m_AssetController.GetComponent<Animation>().IsPlaying("Defeated")))
		{
	...
	{
		if (m_AssetController && (!m_BattleMgr.Model.IsPvP || m_Model.IsBanner))
		{
			m_AssetController.PlayDefeatAnim();
			return m_AssetController.GetDefeatAnimationLength();
		}
	...
	{
		var pigDefeatedLoot = m_Model.CharacterModel.DefeatLoot;
		var useBonus = false;
		...
		{
			useBonus = true;
			foreach (var lootData in pigDefeatedLoot.Values)
			{
		...
		{
			var extraPercentage = GetXpDifferenceFactor();
			pigDefeatedLoot["experience"].Value = (int)((float)pigDefeatedLoot["experience"].Value * extraPercentage);
		}
		if (m_BattleMgr.Model.Balancing.BonusLoot != null && m_BattleMgr.Model.CurrentWaveIndex == m_BattleMgr.Model.Balancing.BattleParticipantsIds.Count - 1 && m_BattleMgr.Model.m_CombatantsPerFaction[Faction.Pigs].Where(p => p.IsAlive).Count() == 0 && m_BattleMgr.Model.Balancing.BonusLoot.ContainsKey("experience") && !m_BattleMgr.m_bonusXpGained)
		{
			var bonusXp = m_BattleMgr.Model.Balancing.BonusLoot.Where(p => p.Key.Equals("experience")).FirstOrDefault().Value;
		...
		var loot = new List<IInventoryItemGameData>();
		if (spawnLoot && m_Model.CombatantFaction == Faction.Pigs && pigDefeatedLoot != null)
		{
			...
			var classItemList = (from b in m_BattleMgr.Model.m_CombatantsPerFaction[Faction.Birds]
		...
		if (spawnLoot && m_Model.CombatantFaction == Faction.Pigs)
		{
			...
			case CharacterSizeType.Medium:
			{
				var effectName = "PigDefeated_Medium";
				if (DIContainerLogic.GetVisualEffectsBalancing().TryGetVisualEffectSetting(effectName, out setting))
				{
					yield return StartCoroutine(m_BattleMgr.InstantiateEffects(m_Model, setting.VisualEffects[0], setting, new List<ICombatant> { m_Model }, false));
				}
			...
			case CharacterSizeType.Large:
			{
				var effectName = "PigDefeated_Large";
				if (DIContainerLogic.GetVisualEffectsBalancing().TryGetVisualEffectSetting(effectName, out setting))
				{
					yield return StartCoroutine(m_BattleMgr.InstantiateEffects(m_Model, setting.VisualEffects[0], setting, new List<ICombatant> { m_Model }, false));
				}
			...
			case CharacterSizeType.Boss:
			{
				var effectName = "PigDefeated_VeryLarge";
				if (DIContainerLogic.GetVisualEffectsBalancing().TryGetVisualEffectSetting(effectName, out setting))
				{
					yield return StartCoroutine(m_BattleMgr.InstantiateEffects(m_Model, setting.VisualEffects[0], setting, new List<ICombatant> { m_Model }, false));
				}
		...
		if (!(m_Model is BossCombatant) || m_Model.CombatantNameId.Contains("hogrider"))
		{
			DestroyCharacter();
		}
	}
	...
	private float GetXpDifferenceFactor()
	{
		...
		foreach (var item in from b in DIContainerBalancing.Service.GetBalancingDataList<ExperienceScalingBalancingData>()
	...
	public IEnumerator KnockOutBird()
	{
		...
		for (var i = 0; i < LastingVisualEffects.Count; i++)
		{
			var effect = LastingVisualEffects.Values.ElementAt(i);
			if (effect)
			{
	...
	{
		var vector = Vector3.Scale(m_AssetController.HealthBarPosition, m_AssetController.transform.localScale);
	...
	public IEnumerator KnockOutCharacter(int score)
	{
		...
		{
			m_Model.KnockOutOnDefeat = true;
			if (m_Model.CombatantView.m_AssetController)
			{
	...
	public void RemoveAllAppliedEffects()
	{
		...
		{
			var list = m_Model.CurrrentEffects.Values.ToList();
			for (var num = list.Count - 1; num >= 0; num--)
			{
				list[num].RemoveEffect();
			}
		}
	}
	...
	private IEnumerator InstantiateBubbleCoroutine(BubbleSetting setting, List<ICombatant> m_Targets, ICombatant m_Source, int duration)
	{
		...
		var source = m_Source.CombatantView;
		if (m_Targets != null)
		{
	...
	public void ResetBubbleTargetIcon()
	{
		...
		{
			var characterSpeechBubble = m_SpeechBubbles.Values.FirstOrDefault(b => b.m_IsTargetedBubble);
		}
	}
	...
	private IEnumerator SpawnBubblesOnTargets(BubbleSetting setting, List<CharacterControllerBattleGroundBase> targets, CharacterControllerBattleGroundBase source, int duration)
	{
		for (var i = 0; i < targets.Count; i++)
		{
			...
			{
				var targetingBubble = character.m_SpeechBubbles.Values.FirstOrDefault();
			...
			var speechBubble = GenerateBubble(setting);
	...
	public void PositionBubble(CharacterControllerBattleGroundBase character, GameObject speechBubble)
	{
		...
		var vector = Vector3.Scale(character.m_AssetController.BubblePosition, m_AssetController.transform.localScale);
	...
	{
		var characterSpeechBubble = UnityEngine.Object.Instantiate(DIContainerLogic.GetVisualEffectsBalancing().LargeBubble);
		SetLayerRecusively(characterSpeechBubble.gameObject, base.gameObject.layer);
		return characterSpeechBubble;
	}
	...
	public void InitCounterAttack(ICombatant attacker, ICombatant target, ICombatant victim, float damageModifier, bool retargetVictim)
	{
		if (attacker == null || !attacker.IsAlive || (!attacker.CombatantView.m_CounterAttack && !attacker.CombatantView.m_AssetController.m_IsIllusion))
		{
			...
			m_RetargetDeadCounterModel = retargetVictim;
			if (m_CounterModel == null || m_CounterModel.IsStunned || !m_CounterModel.IsParticipating)
			{
				...
				m_RetargetDeadCounterModel = false;
			}
			else
			{
	...
	{
		var cachedTarget = m_CounterModel.AttackTarget;
		if (m_RetargetDeadCounterModel && !m_CounterModelTarget.IsParticipating)
		{
			var counterTargetEnemies = m_BattleMgr.Model.m_CombatantsByInitiative
				.Where(c => c.CombatantFaction != m_CounterModel.CombatantFaction)
				.ToList();
			m_CounterModelTarget = counterTargetEnemies[UnityEngine.Random.Range(0, counterTargetEnemies.Count)];
		}
	...
	public void ShowScore(int score)
	{
		if (score != 0)
		{
			...
			var scorePopup = UnityEngine.Object.Instantiate(m_ScorePopup);
	...
	public void PlayHideAnimation()
	{
		if (m_AssetController)
		{
			m_AssetController.PlayHideAnimation();
			m_hidden = true;
		}
	}
}
