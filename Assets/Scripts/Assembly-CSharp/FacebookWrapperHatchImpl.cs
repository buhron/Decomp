...
public class FacebookWrapperHatchImpl : IFacebookWrapper
{
	...
	private void OnGetFriendsSuccess(List<User> users)
	{
		...
		{
			var socialNetworkProfile = user.GetSocialNetworkProfiles().FirstOrDefault(profile => profile.SocialNetwork == User.SocialNetwork.SocialNetworkFacebook);
		...
		var list = users.Select(u => u.GetAccountId()).ToList();
