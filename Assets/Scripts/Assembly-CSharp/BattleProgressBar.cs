...
public class BattleProgressBar : MonoBehaviour
{
	...
	{
		var count = m_BattleMgr.Model.Balancing.BattleParticipantsIds.Count;
		var num = 0;
		for (var i = 0; i < count; i++)
		{
			var gameObject = (GameObject)Object.Instantiate(m_CenterTrackPrefab, Vector3.zero, Quaternion.identity);
			...
			var localPosition = m_StartTrack.transform.localPosition;
			localPosition.x -= 32f;
			m_StartTrack.transform.localPosition = localPosition;
			var localPosition2 = m_EndTrack.transform.localPosition;
			...
			{
				var gameObject2 = (GameObject)Object.Instantiate(GetWheelPrefab(num), Vector3.zero, Quaternion.identity);
		...
		var localPosition3 = m_CenterTracks.gameObject.transform.localPosition;
		...
		var nameId = m_BattleMgr.Model.Balancing.LootTableWheel.FirstOrDefault().Key.Replace("{levelrange}", DIContainerInfrastructure.GetCurrentPlayer().GetLevelRange().ToString("00"));
