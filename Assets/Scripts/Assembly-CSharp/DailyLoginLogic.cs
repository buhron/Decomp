...
public class DailyLoginLogic
{
	...
	{
		var dailyLoginGiftsList = DIContainerBalancing.Service.GetBalancingDataList<DailyLoginGiftsBalancingData>();
		var lastMonth = 12;
		for (var i = dailyLoginGiftsList.Count - 1; i >= dailyLoginGiftsList.Count - 12; i--)
		{
			dailyLoginGiftsList[i].NameId = trustedTime.Year + lastMonth.ToString("00");
			lastMonth--;
		}
		var currentPlayer = DIContainerInfrastructure.GetCurrentPlayer();
		var lastClaimed = DIContainerLogic.GetTimingService().GetDateTimeFromTimestamp(currentPlayer.Data.LastDailyGiftClaimedTime);
		DebugLog.Log(GetType(), "OnGetServerTime: lastClaimed = " + lastClaimed);
		m_currentDayOfTheMonth = trustedTime.Day;
		var flag = trustedTime.Month > lastClaimed.Month || trustedTime.Year > lastClaimed.Year || currentPlayer.Data.GiftsClaimedThisMonth > trustedTime.Day;
		var nextMidnight = new DateTime(lastClaimed.Year, lastClaimed.Month, lastClaimed.Day, 0, 0, 0).AddDays(1.0);
		var flag2 = nextMidnight > trustedTime;
		DebugLog.Log(GetType(), "OnGetServerTime: nextMidnight = " + nextMidnight.ToString() + " trustedTime = " + trustedTime.ToString());
		var nameId = trustedTime.Year + trustedTime.Month.ToString("00");
		var balancingData = DIContainerBalancing.Service.GetBalancingData<DailyLoginGiftsBalancingData>(nameId);
	...
	private void CreateItemArrayFromLoginBalancing(DailyLoginGiftsBalancingData login)
	{
		...
		var lootOperationService = DIContainerLogic.GetLootOperationService();
	...
	public List<IInventoryItemGameData> ClaimGift(bool isForced = false)
	{
		...
		var dictionary = m_RewardItems[player.Data.GiftsClaimedThisMonth - 1].m_Loot;
		if (dictionary.FirstOrDefault().Key.Contains("mastery"))
		{
			var balancingData = DIContainerBalancing.Service.GetBalancingData<MasteryItemBalancingData>(dictionary.FirstOrDefault().Key);
		...
		var list = DIContainerLogic.GetLootOperationService().RewardLoot(player.InventoryGameData, 4, dictionary, "daily_gift", EquipmentSource.Gatcha);
