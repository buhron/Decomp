...
public class PopupMissingResourcesStateMgr : MonoBehaviour
{
	...
	public WaitTimeOrAbort ShowMissingResourcesPopup(List<IInventoryItemGameData> items)
	{
		...
		for (var i = num; i < m_LootDisplays.Count; i++)
		{
		...
		m_Grid.Reposition();
		var shopOffers = DIContainerLogic.GetShopService().GetShopOffers(DIContainerInfrastructure.GetCurrentPlayer(), "shop_global_instant_buy_items");
		foreach (var item in m_Items)
		{
			foreach (var item2 in shopOffers)
			{
				if (item2.OfferContents.ContainsKey(item.ItemBalancing.NameId))
				{
					for (var j = 0; j < item.ItemValue; j++)
					{
		...
		var summedBuyRequirements = DIContainerLogic.GetShopService().GetSummedBuyRequirements(DIContainerInfrastructure.GetCurrentPlayer(), m_SummedInstantOffers);
		var requirement = summedBuyRequirements.FirstOrDefault();
		if (requirement != null)
		{
