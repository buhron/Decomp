...
public class CoinBarController : MonoBehaviour
{
	...
	private void SetCoinLabelText()
	{
		...
		if (hasMaximum && m_ItemName == "event_energy")
		{
			var itemValue = DIContainerLogic.InventoryService.GetItemValue(m_Inventory, m_ItemName + "_cap_extension");
			text = "/" + DIContainerInfrastructure.GetFormatProvider().GetResourceAmountFormat(DIContainerBalancing.GameConstantsBalancingDataProvider.EventItemMaxCap + itemValue);
		}
