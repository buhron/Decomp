...
public class AutoScalingTextBox : MonoBehaviour
{
	...
	private struct NineSliceSprite
	{
		...
		public void ApplyNewSize(Rect newRect)
		{
			...
			var num = newRect.height - m_topHeight - m_bottomHeight;
			var num2 = newRect.width - m_leftWidth - m_rightWidth;
			var yMax = newRect.yMax;
			var y = newRect.yMin + num / 2f + m_bottomHeight;
			var yMin = newRect.yMin;
			var xMin = newRect.xMin;
			var x = newRect.xMin + num2 / 2f + m_leftWidth;
			var xMax = newRect.xMax;
	...
	private void InitializeAnchoredObjects()
	{
		...
		vector.x *= num;
		vector.y *= num2;
		for (var i = 0; i < m_anchoredObjects.Length; i++)
		{
			var anchoredObject = m_anchoredObjects[i];
			if (anchoredObject.m_Transform == null)
			{
				anchoredObject.m_Offset = default(Vector2);
				continue;
			}
			var relativePivot = GetRelativePivot(anchoredObject.m_XAlignment, anchoredObject.m_YAlignment);
			relativePivot.x *= num;
	...
	{
		var num = m_maxTotalWidth - 2f * m_verticalBorders;
		var num2 = 0f;
		...
		var printedSize = m_label.printedSize;
		printedSize = new Vector2(Mathf.Max(printedSize.x, m_minTotalWidth - Mathf.Abs(m_horizontalOffset)), printedSize.y);
		var vector = new Vector2(printedSize.x + 2f * m_verticalBorders + Mathf.Abs(m_horizontalOffset), printedSize.y + 2f * m_horizontalBorders + Mathf.Abs(m_verticalOffset));
	...
	private void SetBackgroundPosAndSize(Vector2 size)
	{
		if (m_backgroundSprite)
		{
			...
			var vector = m_backgroundSprite.pivotOffset - new Vector2(0.5f, 0.5f) - GetRelativePivot(m_horizontalAlignment, m_verticalAlignment);
			vector.x *= size.x;
			vector.y *= size.y;
		...
		else
		{
			...
			var vector2 = new Vector2(-0.5f, -0.5f) - GetRelativePivot(m_horizontalAlignment, m_verticalAlignment);
			vector2.x *= size.x;
			vector2.y *= size.y;
			newRect.x = vector2.x;
			newRect.y = vector2.y;
	...
	private void SetTextPos(Vector2 labelSize, Vector2 totalSize)
	{
		if (m_label)
		{
			var vector = m_label.pivotOffset - new Vector2(0.5f, 0.5f);
			vector.x *= labelSize.x;
			vector.y *= labelSize.y;
			var relativePivot = GetRelativePivot(m_horizontalAlignment, m_verticalAlignment);
	...
	private void SetAnchoredObjects(Vector2 targetSize)
	{
		...
		{
			var anchoredObject = m_anchoredObjects[i];
			var vector = GetRelativePivot(m_horizontalAlignment, m_verticalAlignment) * -1f + GetRelativePivot(anchoredObject.m_XAlignment, anchoredObject.m_YAlignment);
