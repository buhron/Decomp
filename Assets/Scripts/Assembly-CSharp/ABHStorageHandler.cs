...
public class ABHStorageHandler
{
	public void OnGotPublicProfiles(string key, Dictionary<string, string> accountToValueDictionary, Action<Dictionary<string, PublicPlayerData>> callback)
	{
		...
		var dictionary = new Dictionary<string, PublicPlayerData>();
		foreach (var key2 in accountToValueDictionary.Keys)
		{
			var text = accountToValueDictionary[key2];
			...
			try
			{
				var publicPlayerData = DIContainerInfrastructure.GetStringSerializer().Deserialize<PublicPlayerData>(text);
