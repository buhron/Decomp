...
public class UiSortBehaviour : MonoBehaviour
{
	...
	private void OnDisable()
	{
		if (m_orderWidget)
		{
			 m_orderWidget.onRender -= AdjustRenderQueueValues;
		}
	...
	private void LateUpdate()
	{
		...
		if (m_autoSearchRenderers)
		{
			for (var num = m_renderers.Count - 1; num >= 0; num--)
			{
	...
	private void FindParticleSystems()
	{
		...
		foreach (var particleSystem in m_particleSystems)
		{
			...
			var componentsInChildren = particleSystem.GetComponentsInChildren<Renderer>();
	...
	private void AdjustRenderQueueValues()
	{
		if (m_orderWidget != null)
		{
			...
			var renderQueue = m_lastRenderQueue = (int)(m_orderWidget.drawCall.renderQueue + m_orderPosition);
	...
	private void AdjustRenderQueueValues(Material mat)
	{
		...
		var num = (int)(mat.renderQueue + m_orderPosition);
		if (num == m_lastRenderQueue)
		{
			return;
		}
		m_lastRenderQueue = num;
		for (var num2 = m_renderers.Count - 1; num2 >= 0; num2--)
		{
			...
			{
				var materials = m_renderers[num2].materials;
				foreach (var material in materials)
				{
	...
	private IEnumerator ReorderAfterFrame()
	{
		...
		if (m_autoSearchRenderers)
		{
			for (var i = m_renderers.Count - 1; i >= 0; i--)
			{
