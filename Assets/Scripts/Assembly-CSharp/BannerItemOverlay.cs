...
public class BannerItemOverlay : MonoBehaviour
{
	...
	private void FillEquipmentContent(BannerItemGameData equip, BannerGameData currentBanner)
	{
		...
		if (m_Stats)
		{
			...
			var bannerItemGameData = currentBanner.BannerCenter;
		...
		if (equip != null && equip.IsSetItem)
		{
			...
			if (m_EliteEmblemInfo)
			{
				var skill = equip.SetItemSkill.GenerateSkillBattleData();
		...
		if (m_LootDisplays.Count > 0)
		{
			...
			var component = m_LootDisplays[0].transform.parent.GetComponent<UIGrid>();
			if (component != null)
			{
				component.Reposition();
			}
		}
		var primarySkill = equip.PrimarySkill;
		if (primarySkill != null)
		{
			var skill2 = primarySkill.GenerateSkillBattleData();
