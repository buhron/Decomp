...
public class PvPAIService
{
	...
	public List<BirdCommand> CalculateTurn(BattleGameData data = null)
	{
		...
		if ((float)Random.Range(0, 100) >= 100f - PvPAIConstants.ChanceToUseRandomTarget)
		{
			if (m_showLog)
			{
	...
	private void ResetValues()
	{
		...
		m_Targeting = new PvPAITargeting(this, m_battleData, m_cachedCombatants);
		m_DamageCalculation = new PvPAIDamageCalculation(this, m_battleData);
		m_RageCalculation = new PvPAIRageCalculation(this, m_battleData);
	}

	public ICombatant GetNextCombatant()
	{
		foreach (var item in m_currentTurn)
		{
	...
	{
		var num = 0;
		foreach (var birdTurn in birdTurns)
		{
		...
		foreach (var birdTurn2 in birdTurns)
		{
		...
		foreach (var birdTurn3 in birdTurns)
		{
			if (num == 3)
			{
				break;
	...
	{
		var num = m_battleData.m_CombatantsByInitiative.Where(c => c.CombatantFaction == Faction.Birds && c.IsAlive && !c.IsBanner).Count();
		if (m_showLog)
		{
			DebugLog.Log("[PvPAIService] Check if we try to focus on a specific bird. Birds alive: " + num);
		}
		float num2 = Random.Range(0, 100);
		if (num == 3 && num2 < PvPAIConstants.ChanceToFocusBirdWith3)
		{
			return true;
		}
		if (num == 2 && num2 < PvPAIConstants.ChanceToFocusBirdWith2)
		{
			return true;
		}
		if (num == 1 && num2 < PvPAIConstants.ChanceToFocusBirdWith1)
		{
			return true;
		}
		if (m_showLog)
