...
public class CharacterHealthBar : MonoBehaviour
{
	...
	public virtual void SetModel(ICombatant model)
	{
		...
		RefreshInitative();
		if (model is BossCombatant && m_BossLevel || model.CombatantNameId.Contains("hogrider"))
		{
	...
	private IEnumerator SetHealthBarSpacer()
	{
		...
		yield return new WaitForEndOfFrame();
		var amountOfSpacers = GetSpacerAmount();
		if (amountOfSpacers != 0)
		{
			for (var i = 1; i <= amountOfSpacers; i++)
			{
				var spacerBar = Object.Instantiate(m_SpacerPrefab);
				spacerBar.transform.parent = m_SpacerParent.transform;
				spacerBar.transform.localScale = Vector3.one;
				var unevenXpos = m_HealthBarsize / ((float)amountOfSpacers + 1f) * (float)i;
				var roundedXpos = (int)(unevenXpos / 2f) * 2;
	...
	{
		var modifiedHealth = m_Model.ModifiedHealth;
		var balancingData = DIContainerBalancing.GameConstantsBalancingDataProvider;
		var maxHPChunks = balancingData.MaxHPChunks;
		var hPChunkSteps = balancingData.HPChunkSteps;
		var hPChunksLowest = balancingData.HPChunksLowest;
		var hPChunksHighest = balancingData.HPChunksHighest;
		if (hPChunkSteps <= 0f)
		{
			return 0;
		}
