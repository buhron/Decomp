...
public class PopupSpecialOfferStateMgr : MonoBehaviour
{
	...
	{
		var saleItemDetails = sale.SaleDetails.FirstOrDefault();
		...
		var requirement = DIContainerLogic.GetShopService().GetBuyResourcesRequirements(1, balancingData, false).FirstOrDefault(r => r.RequirementType == RequirementType.PayItem);
		...
		{
			var value = balancingData.OfferContents.FirstOrDefault().Value;
	...
	public WaitTimeOrAbort ShowSpecialOfferPopup(BasicItemGameData basicItem)
	{
		...
		foreach (var shopOffer in DIContainerLogic.GetShopService().GetAllShopOffers())
		{
			if (shopOffer.DiscountRequirements == null || !shopOffer.DiscountRequirements.Any(r => r.RequirementType == RequirementType.HaveItem && r.NameId == basicItem.BalancingData.NameId))
			{
				continue;
			}
			specialOffer = shopOffer;
			break;
		}
		...
		var shopBalancingData = DIContainerBalancing.Service.GetBalancingDataList<ShopBalancingData>().FirstOrDefault(shop => shop.Categories.Any(c => c == specialOffer.Category));
	...
	private List<GameObject> InstantiateFeatureObject(BasicItemGameData basicItemGameData, Transform centerTransform, Transform rooTransform)
	{
		...
		if (m_IsStarCollection)
		{
			...
			var itemValue = DIContainerLogic.InventoryService.GetItemValue(currentPlayer.InventoryGameData, "star_collection");
			var list2 = new List<float>();
			var allShopOffersInCategory = DIContainerLogic.GetShopService().GetAllShopOffersInCategory("star_rewards");
			BasicShopOfferBalancingData basicShopOfferBalancingData = null;
			foreach (var item in allShopOffersInCategory)
			{
				if (item == null)
				{
					continue;
				}
				foreach (var buyRequirement in item.BuyRequirements)
				{
			...
			list2.Sort();
			var num4 = 0f;
			foreach (var item2 in basicShopOfferBalancingData.BuyRequirements.Where(b => b.RequirementType == RequirementType.HaveItem && b.NameId == "star_collection"))
			{
				num4 = item2.Value;
			}
			for (var i = 0; i < list2.Count; i++)
			{
				if ((float)itemValue >= list2[i] && list2[i] >= num4)
				{
					...
					if (num2 > m_lastcollectedPopup && num2 != num)
					{
			...
			if (num != num2 && num != 0f)
			{
				var text = DIContainerInfrastructure.GetLocaService().Tr("popup_starcollection_subheader");
			...
			var text2 = DIContainerInfrastructure.GetLocaService().Tr("popup_starcollection_desc");
			text2 = text2.Replace("{value_1}", num2.ToString());
			text2 = text2.Replace("{value_2}", m_BasicItemGameData.ItemLocalizedName);
			var value = basicShopOfferBalancingData.OfferContents.FirstOrDefault().Value;
		...
		if (m_IsRainbowRiot)
		{
			...
			m_OfferDescLabel.text = m_OfferDescLabel.text.Replace("{value_1}", multiplier);
		}
	...
	private GameObject CreateFeatureObjectAsset(string assetName, BasicItemGameData basicItemGameData, Transform centerTransform, Transform rooTransform)
	{
		...
		{
			m_FeatureObjectNameId.Add(assetName);
			var gameObject = DIContainerInfrastructure.GetClassAssetProvider().InstantiateObject(assetName, centerTransform, Vector3.zero, Quaternion.identity, false);
		...
		if (assetName.StartsWith("ShopOffer_"))
		{
			...
			{
				var gameObject2 = DIContainerInfrastructure.GetGenericIconAtlasAssetProvider().GetObject("ShopAndSocialElements") as GameObject;
		...
		{
			m_FeatureObjectNameId.Add(assetName);
			var gameObject3 = DIContainerInfrastructure.PropLiteAssetProvider().InstantiateObject(assetName, centerTransform, Vector3.zero, Quaternion.identity);
		...
		{
			m_FeatureObjectNameId.Add(assetName);
			var gameObject4 = DIContainerInfrastructure.PropLiteAssetProvider().InstantiateObject(assetName, centerTransform, Vector3.zero, Quaternion.identity);
			gameObject4.SetActive(true);
			var component = gameObject4.GetComponent<VectorContainer>();
			if (component)
			{
	...
	private IEnumerator LeaveCoroutine()
	{
		...
		if (m_ShopOffer)
		{
			for (var index = 0; index < m_FeatureObject.Count; index++)
			{
				UnityEngine.Object.Destroy(m_FeatureObject[index]);
			}
		}
		...
		{
			for (var index2 = 0; index2 < m_FeatureObjectNameId.Count; index2++)
			{
				var feature = m_FeatureObjectNameId[index2];
				DIContainerInfrastructure.PropLiteAssetProvider().DestroyObject(feature, m_FeatureObject[index2]);
			}
		}
		else if (m_ObjectType == FeatureObjectType.ClassUnlock)
		{
			for (var index3 = 0; index3 < m_FeatureObjectNameId.Count; index3++)
			{
				var feature2 = m_FeatureObjectNameId[index3];
				DIContainerInfrastructure.GetClassAssetProvider().DestroyObject(feature2, m_FeatureObject[index3]);
			}
		}
		else if (m_ObjectType == FeatureObjectType.ResourceUnlock)
		{
			for (var index4 = 0; index4 < m_FeatureObjectNameId.Count; index4++)
			{
				var feature3 = m_FeatureObjectNameId[index4];
				DIContainerInfrastructure.PropLiteAssetProvider().DestroyObject(feature3, m_FeatureObject[index4]);
			}
		}
		if (m_FeatureObject != null)
		{
	...
	private void GetItButtonClicked()
	{
		...
		else if (m_ShopOffer)
		{
			...
			else
			{
				...
				DIContainerInfrastructure.GetCoreStateMgr().ShowShop(m_shopCategory, delegate { DIContainerInfrastructure.LocationStateMgr.WorldMenuUI.Enter(); }, 0, false, "Popup");
				return;
			}
		}
	...
	private void HintCheckoutClickedHandler()
	{
		...
		case "samurai_shop":
			if (worldMapStateMgr)
			{
				...
				var worldMapView2 = value2.WorldMapView;
		...
		case "lightningbird_shop":
			if (worldMapStateMgr)
			{
				...
				var worldMapView = value.WorldMapView;
	...
	public WaitTimeOrAbort ShowSpecialOfferPopup(SalesManagerBalancingData sale)
	{
		...
		m_StandardLabelRoot.SetActive(true);
		var flag = sale.SaleType == SaleAvailabilityType.Conditional || sale.SaleType == SaleAvailabilityType.ConditionalCooldown;
		...
		var dummyText = DIContainerInfrastructure.GetLocaService().Tr("shop_specialofferlabel");
		...
		if (!string.IsNullOrEmpty(sale.PopupIconId))
		{
			...
			{
				var gameObject = DIContainerInfrastructure.GetGenericIconAtlasAssetProvider().GetObject(sale.PopupAtlasId) as GameObject;
			...
			{
				var gameObject2 = DIContainerInfrastructure.GetShopIconAtlasAssetProvider().GetObject(sale.PopupAtlasId) as GameObject;
				m_ResourcesOfferSprite.atlas = gameObject2.GetComponent<UIAtlas>();
			}
