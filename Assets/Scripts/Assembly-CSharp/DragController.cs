...
public class DragController : MonoBehaviour
{
	...
	[SerializeField]
	private float m_SpeedFactorDrag = 1000f;

	[SerializeField]
	private float m_SpeedFactorMomentum = 1f;
	...
	private void Awake()
	{
		for (var i = 0; i < m_DragContainers.Count; i++)
		{
			m_DragContainers[i].gameObject.SetActive(false);
		}
		HotspotGameData value = null;
		for (var num = m_DragContainers.Count - 1; num >= 0; num--)
		{
		...
		m_InterfaceCamera = Camera.allCameras.FirstOrDefault(c => c.CompareTag("UICamera"));
	...
	public void SetActiveDepth(bool active, int depth)
	{
		...
		for (var i = 0; i < m_ActiveArray.Length; i++)
		{
			if (!m_ActiveArray[i])
			{
				base.enabled = false;
	...
	{
		var outOfBounds = GetOutOfBoundsValue(base.transform.position);
		...
		do
		{
			...
			{
				var degree = 0f - SinusoidalEaseInOut(passedTime, 0f, 1f);
				var delta = outOfBounds * degree;
				base.transform.Translate(delta - lastDelta);
				lastDelta = delta;
			}
			var offset = DampVec2(ref velocity, 9f, Time.deltaTime);
			var momentumBounds = GetOutOfBoundsValue((Vector2)base.transform.position + offset);
			...
			{
				base.transform.Translate(new Vector3(offset.x, offset.y, 0f) * m_SpeedFactorMomentum);
			}
			else if (outOfBounds.x == 0f)
			{
				base.transform.Translate(new Vector3(offset.x, 0f, 0f) * m_SpeedFactorMomentum);
			}
			else if (outOfBounds.y == 0f)
			{
				base.transform.Translate(new Vector3(0f, offset.y, 0f) * m_SpeedFactorMomentum);
			}
		}
	...
	private void Drag()
	{
		...
		var a = Vector2.Scale(vector - m_lastPosition, m_scale);
		var num = m_camera.orthographicSize / m_BaseOrthoSize;
	...
	private Vector2 GetBoundsFactor()
	{
		...
		var vector = new Vector2(m_camera.orthographicSize * ((float)Screen.width / (float)Screen.height), m_camera.orthographicSize);
