...
public class CharacterSpeechBubble : MonoBehaviour
{
	...
	{
		if (m_BubbleIcon && !string.IsNullOrEmpty(iconAssetId))
		{
	...
	{
		if (m_TimeAnimation != null && m_TimeAnimation["Bubble_TimerWarning"])
		{
	...
	public float UseSkill()
	{
		...
		var num = base.gameObject.PlayAnimationOrAnimatorState("Bubble_UseSkill");
		if (num == 0f)
		{
	...
	{
		var currLength = base.gameObject.GetAnimationOrAnimatorStateLength("Bubble_UseSkill");
