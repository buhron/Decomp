...
public class EventDelegate
{
	...
	public override bool Equals(object obj)
	{
		...
		if (obj is Callback)
		{
			...
			var monoBehaviour = callback.Target as MonoBehaviour;
			return mTarget == monoBehaviour && string.Equals(mMethodName, GetMethodName(callback));
		}
		if (obj is EventDelegate)
		{
			var eventDelegate = obj as EventDelegate;
	...
	private void Cache()
	{
		...
		if (mParameters == null || mParameters.Length != mParameterInfos.Length)
		{
			...
			var i = 0;
			for (var num = mParameters.Length; i < num; i++)
			{
				mParameters[i] = new Parameter();
			}
		}
		var j = 0;
		for (var num2 = mParameters.Length; j < num2; j++)
		{
			...
			else
			{
				...
				catch (ArgumentException ex)
				{
					...
					else
					{
						...
						for (var j = 1; j < mParameterInfos.Length; j++)
						{
					...
					else
					{
						...
						for (var k = 1; k < mParameters.Length; k++)
						{
				...
				var l = 0;
				for (var num2 = mArgs.Length; l < num2; l++)
				{
	...
	public override string ToString()
	{
		if (mTarget != null)
		{
			var text = mTarget.GetType().ToString();
	...
	public static EventDelegate Set(List<EventDelegate> list, Callback callback)
	{
		if (list != null)
		{
			var eventDelegate = new EventDelegate(callback);
			list.Clear();
	...
	public static EventDelegate Add(List<EventDelegate> list, Callback callback, bool oneShot)
	{
		if (list != null)
		{
			...
			var eventDelegate2 = new EventDelegate(callback);
	...
	public static void Add(List<EventDelegate> list, EventDelegate ev, bool oneShot)
	{
		...
		else if (list != null)
		{
			...
			var eventDelegate2 = new EventDelegate(ev.target, ev.methodName);
			eventDelegate2.oneShot = oneShot;
			if (ev.mParameters != null && ev.mParameters.Length > 0)
			{
				eventDelegate2.mParameters = new Parameter[ev.mParameters.Length];
				for (var j = 0; j < ev.mParameters.Length; j++)
				{
