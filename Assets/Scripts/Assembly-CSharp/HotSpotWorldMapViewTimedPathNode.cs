...
public class HotSpotWorldMapViewTimedPathNode : HotSpotWorldMapViewPathNode
{
	...
	private IEnumerator UnlockCoroutine()
	{
		...
		var nextGate = GetNextHotspotWhere(spot => spot.m_MiniCampaignHotspot && (spot is HotSpotWorldMapViewTimedPathNode || spot is HotSpotWorldMapViewCampaignStarPathNode));
		if (nextGate == null)
		{
	...
	public override IEnumerator ActivateFollowUpStagesAsync(HotSpotWorldMapViewBase parentHotSpot, HotSpotWorldMapViewBase activateTo, bool instant = false)
	{
		...
		if (m_MiniCampaignHotspot)
		{
			...
			var nextGate = GetNextHotspotWhere(spot => spot.m_MiniCampaignHotspot && spot is HotSpotWorldMapViewTimedPathNode) as HotSpotWorldMapViewTimedPathNode;
			if (nextGate != null)
			{
	...
	protected IEnumerator TimeLeftCountdown()
	{
		...
		{
			var req = base.Model.BalancingData.EnterRequirements.Where(r => r.RequirementType == RequirementType.HaveItem && r.NameId == "event_gate_unlock").FirstOrDefault();
			var requiredUnlockItems = (int)req.Value;
			...
			var hoursInt = m_latestUpdatedTime / 3600;
			var hours = hoursInt.ToString("D2");
			var minutesInt = m_latestUpdatedTime % 3600 / 60;
			var minutes = minutesInt.ToString("D2");
			var seconds = (m_latestUpdatedTime % 60).ToString("D2");
			...
			foreach (var field2 in m_eventGateTimerFields)
			{
		...
		{
			foreach (var field in m_eventGateTimerFields)
			{
				field.spriteName = "0";
				field.MarkAsChanged();
			}
	...
	private void CountdownFinishedHandler()
	{
		...
		var hotSpotWorldMapViewTimedPathNode = GetNextHotspotWhere(spot => spot.m_MiniCampaignHotspot && spot is HotSpotWorldMapViewTimedPathNode) as HotSpotWorldMapViewTimedPathNode;
	...
	{
		var num = 0;
		var animationState = base.Model.Data.AnimationState;
	...
	{
		var numberOfCampaignGateUnlocks = DIContainerInfrastructure.EventSystemStateManager.GetNumberOfCampaignGateUnlocks();
		var requirement = base.Model.BalancingData.EnterRequirements.Where(r => r.RequirementType == RequirementType.HaveItem && r.NameId == "event_gate_unlock").FirstOrDefault();
		var num = (int)requirement.Value;
		var hotspotAnimationState = HotspotAnimationState.None;
