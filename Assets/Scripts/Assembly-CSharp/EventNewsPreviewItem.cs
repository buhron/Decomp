...
public class EventNewsPreviewItem : MonoBehaviour
{
	...
	private IEnumerator HandleIconAsset()
	{
		...
		if (m_IconPrefabContainer.transform.childCount == 0)
		{
			...
			var chestReward = DIContainerLogic.EventSystemService
				...
				.FirstOrDefault();
			var iconName = chestReward.ItemBalancing.ItemType == InventoryItemType.Class && m_EventModel.IsCampaignEvent ? "Icon_Large_ClassChest" : "Icon_Large";
			
			var eventIcon = DIContainerInfrastructure.EventSystemStateManager.InstantiateEventObject(iconName, m_IconPrefabContainer.transform, m_EventModel);
			if (eventIcon)
	...
	private void CheckOut()
	{
		...
		{
			var category = m_salesModel.Category;
			if (category == "shop_global_premium" || category == "shop_global_premium_soft")
			{
				DIContainerInfrastructure.GetCoreStateMgr().m_GenericUI.GetLuckyCoinController().SwitchToShop();
				m_newsParent.HideOnly();
			}
			else
			{
		...
		var flag = DIContainerInfrastructure.LocationStateMgr is WorldMapStateMgr;
		...
		case BonusEventType.DungeonBonus:
		{
			var dayOfWeek = (int)DIContainerLogic.GetTimingService().GetPresentTime().DayOfWeek;
	...
	{
		var trustedTime = DIContainerLogic.GetTimingService().GetDateTimeFromTimestamp(DIContainerLogic.GetTimingService().GetCurrentTimestamp());
		while (m_targetTime > trustedTime)
		{
			if (DIContainerLogic.GetTimingService().TryGetTrustedTime(out trustedTime))
			{
				var timeLeft = DIContainerLogic.GetTimingService().TimeLeftUntil(m_targetTime);
