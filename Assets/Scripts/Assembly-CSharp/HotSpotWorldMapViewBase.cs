...
public class HotSpotWorldMapViewBase : MonoBehaviour
{
	...
	public virtual void Complete(HotSpotState state, bool startUp)
	{
		...
		var component = GetComponent<ExecuteActionTree>();
		if (component != null)
		{
	...
	public virtual IEnumerator ActivateFollowUpStagesAsync(HotSpotWorldMapViewBase parentHotSpot, HotSpotWorldMapViewBase activateTo, bool instant = false)
	{
		...
		for (var i = m_outgoingHotSpots.Count - 1; i >= 0; i--)
		{
			var hotspot = m_outgoingHotSpots[i];
			if (hotspot == parentHotSpot || (activateTo != null && this == activateTo)) 
				continue;
	...
	public bool CalculatePath(HotSpotWorldMapViewBase parent, HotSpotWorldMapViewBase target, ref List<HotSpotWorldMapViewBase> list)
	{
		...
		for (var i = 0; i < m_outgoingHotSpots.Count; i++)
		{
			var hotSpotWorldMapViewBase = m_outgoingHotSpots[i];
	...
	public virtual void HandleMouseButtonUp(bool directMove = false)
	{
		...
		if (m_active && !DIContainerInfrastructure.GetCoreStateMgr().SceneLoadingMgr.IsLoading())
		{
	...
	public void PlayActiveAnimation()
	{
		...
		if (GetComponent<Animation>()[AnimationPrefix + "SetActive"])
		{
		...
		if (GetComponent<Animation>()[AnimationPrefix + "Idle"])
		{
	...
	{
		if (GetComponent<Animation>()[AnimationPrefix + "Idle_Active"] && !GetComponent<Animation>().IsPlaying("Idle_Active"))
		{
	...
	{
		if (GetComponent<Animation>()[AnimationPrefix + "Used"] && !GetComponent<Animation>().IsPlaying(AnimationPrefix + "Used"))
		{
	...
	protected HotSpotWorldMapViewBase GetNextHotspotWhere(Func<HotSpotWorldMapViewBase, bool> condition)
	{
		...
		for (var j = 0; j < m_outgoingHotSpots.Count; j++)
		{
			var nextHotspotWhere = m_outgoingHotSpots[j].GetNextHotspotWhere(condition);
