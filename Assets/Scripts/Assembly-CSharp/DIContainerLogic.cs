...
public class DIContainerLogic
{
	...
	private static SetFusionLogic m_fusionLogic;
	...
	public static CraftingService CraftingService
	{
		get
		{
			...
			m_craftingService.SetLootService(GetLootOperationService()).SetDebugLog(DebugLog.Log).SetErrorLog(DebugLog.Error);
			return m_craftingService;
		}
		set
		{
	...
	public static SetFusionLogic FusionLogic
	{
		get
		{
			if (m_fusionLogic == null)
			{
				m_fusionLogic = new SetFusionLogic();
			}
			return m_fusionLogic;
		}
	}
	...
	public static IProfileMerger ProfileMerger
	{
		get
		{
			...
			{
				var enableProfileMerging = DIContainerBalancing.Service.GetBalancingDataList<ClientConfigBalancingData>().FirstOrDefault().EnableProfileMerging;
	...
	internal static ITimingService GetTimingService()
	{
		...
		var useSkynestTimingService = false;
	...
	{
		return m_serverOnlyTimingService ?? (m_serverOnlyTimingService = new TimingService());
	}
	...
	internal static BattleService GetBattleService()
	{
		if (m_battleService == null)
		{
			...
			m_battleService.SetRequirementService(RequirementService);
