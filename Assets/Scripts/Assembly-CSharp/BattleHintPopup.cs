...
public class BattleHintPopup : MonoBehaviour
{
	...
	private void SetupClassGrid()
	{
		...
		var classRecommendationHelpers = GetClassRecommendationHelpers();
		foreach (var item in classRecommendationHelpers)
		{
			var balancing = item.Value.m_Balancing;
			var classRecommendation = Object.Instantiate(m_recommendClassPrefab);
	...
	{
		var dictionary = new Dictionary<string, ClassRecommendationHelper>();
		foreach (var recommendedClass in m_balancing.RecommendedClasses)
		{
			var balancingData = DIContainerBalancing.Service.GetBalancingData<ClassItemBalancingData>(recommendedClass.Key);
			var currentPlayer = DIContainerInfrastructure.GetCurrentPlayer();
			if (currentPlayer.GetBird(balancingData.RestrictedBirdId) == null)
			{
				continue;
			}
			var value = default(ClassRecommendationHelper);
			...
			var classRecommendationHelper = dictionary[balancingData.RestrictedBirdId];
	...
	private void SetupIcons(UISprite sprite, string atlasId, string spriteName)
	{
		...
		{
			var gameObject = DIContainerInfrastructure.GetGenericIconAtlasAssetProvider().GetObject(atlasId) as GameObject;
		...
		{
			var gameObject2 = DIContainerInfrastructure.GetShopIconAtlasAssetProvider().GetObject(atlasId) as GameObject;
