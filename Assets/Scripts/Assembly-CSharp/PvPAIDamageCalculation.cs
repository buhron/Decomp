...
public class PvPAIDamageCalculation
{
	...
	public bool IsKillPossible(ICombatant target, out List<BirdCommand> turnsToKill)
	{
		...
		var birdOrderForMaxDamage = GetBirdOrderForMaxDamage();
		if (m_showLog)
		{
			for (var i = 0; i < birdOrderForMaxDamage.Count; i++)
			{
		...
		for (var j = 0; j < birdOrderForMaxDamage.Count; j++)
		{
			var combatant2 = birdOrderForMaxDamage[j];
			if (combatant2 == null)
			{
				continue;
			}
			var nameId = combatant2.CombatantClass.BalancingData.NameId;
			if (combatant2.GetTauntTarget() != null && combatant2.GetTauntTarget() != target && nameId != "class_mage" && nameId != "class_rainbird" && nameId != "class_lightningbird")
			{
			...
			num += num2;
			var birdCommand = new BirdCommand();
			birdCommand.m_Source = combatant2;
			birdCommand.m_Sortorder = j;
			birdCommand.m_Target = target;
			var flag2 = false;
			foreach (var value3 in combatant2.CurrrentEffects.Values)
			{
				foreach (var effect in value3.m_Effects)
				{
			...
			else
			{
				...
				if (nameId == "class_lightningbird" && m_battleData.m_CombatantsPerFaction[Faction.Birds].Count <= 1)
				{
	...
	private void GetMostDamageFromLightningBird(ICombatant currentBird, BirdCommand attack)
	{
		...
		foreach (var item in m_battleData.m_CombatantsByInitiative)
		{
			...
			{
				var num2 = item.ModifiedAttack * (value / 100f) * m_DamageValues.m_DamageIncrease;
				if (num2 > num)
				{
					num = num2;
	...
	private float CheckForGangUp(ICombatant source)
	{
		...
		var num = 0f;
		foreach (var item in m_battleData.m_CombatantsByInitiative.Where(c => !c.IsBanner && c.CombatantFaction == source.CombatantFaction))
		{
			if (item.CombatantNameId.Contains("bird_black"))
			{
				num += m_DamageValues.m_BonusDamagePerAttack;
				var value = 0f;
				var balancing = item.GetSkill(0).Model.Balancing;
				...
				{
					var num2 = item.CurrentHealth / item.ModifiedHealth * 100f;
	...
	{
		foreach (var value in target.CurrrentEffects.Values)
		{
	...
	{
		var sortedDictionary = new SortedDictionary<int, ICombatant>();
		foreach (var item in m_battleData.m_CombatantsByInitiative)
		{
			if (item.CombatantFaction == Faction.Pigs && !item.IsBanner && !item.ActedThisTurn)
			{
				...
				if (nameId == "class_skulkers")
				{
					sortedDictionary.Add(1, item);
				}
				else if (nameId == "class_thunderbird")
				{
					sortedDictionary.Add(2, item);
				}
				else if (nameId == "class_marksmen")
				{
					sortedDictionary.Add(3, item);
				}
				else if (nameId == "class_tricksters")
				{
					sortedDictionary.Add(4, item);
				}
				else if (nameId == "class_captn")
				{
					sortedDictionary.Add(5, item);
				}
				else if (nameId == "class_berserk")
				{
	...
	{
		var nameId = currentBird.CombatantClass.BalancingData.NameId;
		if (nameId == "class_berserk")
		{
			var value = 0f;
			...
			{
				var factionRage = m_battleData.GetFactionRage(currentBird.CombatantFaction);
				birdDamage *= (100f - factionRage) * value / 100f / 100f;
			}
		}
		else if (nameId == "class_avenger")
		{
			var value2 = 0f;
			...
			{
				var num = currentBird.CurrentHealth / currentBird.ModifiedHealth * 100f;
				birdDamage *= 1f + (100f - num) * value2 / 100f / 100f;
			}
		}
		else if (nameId == "class_seadog")
		{
			var value3 = 0f;
			...
			{
				var num2 = currentBird.CurrentHealth / currentBird.ModifiedHealth * 100f;
	...
	private float CheckForDispel(ICombatant currentBird, float birdDamage, ICombatant target)
	{
		...
		{
			foreach (var value2 in target.CurrrentEffects.Values)
			{
				if (value2.m_EffectType != SkillEffectTypes.Blessing)
				{
					continue;
				}
				foreach (var effect in value2.m_Effects)
				{
	...
	private void CalculateDamageEffects(ICombatant currentBird, ICombatant target)
	{
		...
		else if (nameId == "class_druid" && !target.CurrrentEffects.ContainsKey("ThornyVine"))
		{
			var value4 = 0f;
		...
		else if (nameId == "class_rogues" && !target.CurrrentEffects.ContainsKey("StickyGoo"))
		{
			var value5 = 0f;
		...
		else if (nameId == "class_rainbird" && !target.CurrrentEffects.ContainsKey("AcidRain"))
		{
			var value6 = 0f;
