...
public class SocialWindowUI : MonoBehaviour
{
	...
	[SerializeField]
	private UIInputTrigger m_ClaimAllButton;
	...
	[SerializeField] 
	private UILabel m_rovioIdLabel;
	...
	[SerializeField]
	private GameObject m_starCollectionUpdateMarker;
	...
	[SerializeField]
	private GameObject m_StarCollectionFooterRoot;
	...
	private Dictionary<SocialWindowCategory, GameObject> m_RootDictionary = new Dictionary<SocialWindowCategory, GameObject>(4);

	private Dictionary<SocialWindowCategory, GameObject> m_FooterDictionary = new Dictionary<SocialWindowCategory, GameObject>(4);

	private Dictionary<SocialWindowCategory, Vector3> m_ArrowPostions = new Dictionary<SocialWindowCategory, Vector3>(4);
	...
	[SerializeField]
	private GameObject m_ClaimAllButtonObject;
	...
	[SerializeField]
	private UIInputTrigger m_StarCollectionButton;
	...
	private IEnumerator InitializeFriends()
	{
		...
		{
			DebugLog.Log("Is on last page! FriendCount: " + DIContainerLogic.SocialService.GetFriendCount(DIContainerInfrastructure.GetCurrentPlayer(), true, true, false));
			blindCount = Mathf.Min(DIContainerLogic.SocialService.GetFriendCount(DIContainerInfrastructure.GetCurrentPlayer(), true, true, false) - DIContainerLogic.SocialService.GetFriendsPerPage() * m_CurrentPage[SocialWindowCategory.Friends], DIContainerLogic.SocialService.GetFriendsPerPage());
		...
		foreach (var npc in m_LowLevelNpcs)
		{
			...
			npcBlind2 = UnityEngine.Object.Instantiate(m_FriendVisitingBlind);
			...
			npcBlind2.Initialize(this, negativeIndex, false);
			npcBlind2.transform.parent = getGrid().transform;
			npcBlind2.transform.localPosition = Vector3.zero;
		...
		{
			var addFriendBlind = UnityEngine.Object.Instantiate(m_AddFriendBlind);
	...
	{
		var friendVisitingBlind = UnityEngine.Object.Instantiate(m_FriendVisitingBlind);
		friendVisitingBlind.transform.parent = getGrid().transform;
		friendVisitingBlind.transform.localPosition = Vector3.zero;
		friendVisitingBlind.Initialize(this, i + m_CurrentPage[SocialWindowCategory.Friends] * DIContainerLogic.SocialService.GetFriendsPerPage(), false);
		m_VisitingBlinds.Insert(i, friendVisitingBlind);
	}

	public void FriendList_ChangePage(int change)
	{
		var num = m_CurrentPage[SocialWindowCategory.Friends] + change;
		if (num < 0)
		{
			num = m_MaxPage + num;
		}
	...
	private void DeRegisterFriendListEventHandlers()
	{
		...
		foreach (var lowLevelNpcBlind in m_LowLevelNpcBlinds)
		{
			lowLevelNpcBlind.DeRegisterEventHandler();
		}
		foreach (var visitingBlind in m_VisitingBlinds)
		{
	...
	private void FriendList_FriendsRefreshed(List<FriendGameData> friends)
	{
		...
		m_FriendsList = friends.Where(f => !f.isNpcFriend).OrderByDescending(f => f.FriendLevel).ToList();
		for (var i = 0; i < m_VisitingBlinds.Count; i++)
		{
			if (i + m_CurrentPage[SocialWindowCategory.Friends] * DIContainerLogic.SocialService.GetFriendsPerPage() <= DIContainerLogic.SocialService.GetFriendCount(DIContainerInfrastructure.GetCurrentPlayer(), false, true, false))
			{
				m_VisitingBlinds[i].SetModel(m_FriendsList[i + m_CurrentPage[SocialWindowCategory.Friends] * DIContainerLogic.SocialService.GetFriendsPerPage()]);
			}
		}
	}
	...
	{
		var viewableMessagesCount = GetViewableMessagesCount();
	...
	private IEnumerator InitializeMailboxMessages()
	{
		...
		m_ShowAbleMessages = DIContainerInfrastructure.GetCurrentPlayer().SocialEnvironmentGameData.MailboxMessages.Values.Where(m => !m.IsViewed).ToList();
		CombinePvpChallengeMessages(m_ShowAbleMessages);
		m_ShowAbleMessages = m_ShowAbleMessages.OrderByDescending(m => m.SendTime).ToList();
		m_MailboxMessages.Clear();
		SetMailboxHeader();
		yield return new WaitForEndOfFrame();
		for (var i = m_MessagesPerPage * m_CurrentPage[SocialWindowCategory.Mailbox]; i < Mathf.Min(m_ShowAbleMessages.Count, m_MessagesPerPage * (m_CurrentPage[SocialWindowCategory.Mailbox] + 1)); i++)
		{
	...
	{
		foreach (var message in m_MailboxMessages)
		{
			message.ConfirmButtonTriggerClicked();
		}
	}
	...
	private void CombinePvpChallengeMessages(List<IMailboxMessageGameData> Messages)
	{
		...
		var list = new List<IMailboxMessageGameData>();
	...
	{
		var mailboxMessageBlind = UnityEngine.Object.Instantiate(m_MailboxMessagePrefab);
		mailboxMessageBlind.transform.parent = m_MailboxGrid.transform;
		mailboxMessageBlind.transform.localPosition = Vector3.zero;
		var flag = message is WonInPvpChallengeMessage;
	...
	public void SoftMailboxRefresh(MailboxMessageBlind mailToRemove)
	{
		if (!mailToRemove)
		{
			for (var i = 0; i < m_ShowAbleMessages.Count; i++)
			{
				var model = m_ShowAbleMessages[i];
				var child = m_MailboxGrid.transform.GetChild(i);
				if (child)
				{
					var component = child.GetComponent<MailboxMessageBlind>();
	...
	{
		m_IsRefreshing = true;
		var nextMessages = m_MailboxMessages;
		...
		yield return new WaitForEndOfFrame();
		for (var i = 0; i < m_MailboxMessages.Count; i++)
		{
	...
	private IEnumerator RemoveMailAndRefresh(MailboxMessageBlind mailToRemove)
	{
		...
		var nextMessages = new List<MailboxMessageBlind>();
		var takeNext = false;
		...
		if (destroyed)
		{
			...
			{
				var item = UnityEngine.Object.Instantiate(m_LootForExplosionPrefab, destroyed.m_ConfirmButtonTrigger.transform.position + new Vector3(0f, 0f, -3f), Quaternion.identity) as LootDisplayContoller;
				...
				var explodedItems = item.Explode(true, false, 0f, true, 0f, 0f);
	...
	{
		var num = m_CurrentPage[SocialWindowCategory.Mailbox] + change;
		if (num < 0)
		{
			num = m_PageCount + num;
		}
	...
	private void DeRegisterMailboxEventHandlers()
	{
		...
		if (m_ClaimAllButton)
		{
			m_ClaimAllButton.Clicked -= ClaimAll;
		}
	}
	...
	private void RovioIdAwake()
	{
		...
		var rovioId = DIContainerInfrastructure.GetCurrentPlayer().Data.RovioId;
		if (string.IsNullOrEmpty(rovioId))
		{
			rovioId = DIContainerLogic.BackendService.RovioId;
			DIContainerInfrastructure.GetCurrentPlayer().Data.RovioId = rovioId;
		}
		m_rovioIdLabel.text = "Rovio Id: " + rovioId;
	}
	...
	private void CheckAndRewardRovioIdLogin()
	{
		if (!DIContainerInfrastructure.IdentityService.IsGuest() && DIContainerInfrastructure.GetCurrentPlayer() != null && !DIContainerInfrastructure.GetCurrentPlayer().Data.RovioIdRegisterOnce && DIContainerInfrastructure.GetRovioAccSyncState().IsAccountActionPossible())
		{
			...
			var items = DIContainerLogic.GetLootOperationService().RewardLootGetInputCopy(DIContainerInfrastructure.GetCurrentPlayer().InventoryGameData, 1, DIContainerLogic.GetLootOperationService().GenerateLoot(new Dictionary<string, int> { { "loot_rovioaccount_01", 1 } }, 1), "registered_for_rovio_id");
	...
	public IEnumerator SetLoginStates()
	{
		...
		if (m_AchievementsIcon)
		{
			...
			#elif UNITY_IOS
			m_AchievementsIcon.spriteName = "Gamecenter";
			m_AchievementsButtonText.text = DIContainerInfrastructure.GetLocaService().Tr("social_gamecenter_button");
			#endif
		}
	...
	private void HandleRovioIdLoggedInState()
	{
		...
		m_RovioIdSignInButton.gameObject.SetActive(true);
		m_RovioIdRegisterText.gameObject.SetActive(false);
		var email = DIContainerInfrastructure.IdentityService.UserCredentials.Email;
	...
	private IEnumerator LoadAvatarTexture(string avatarAsset)
	{
		...
		{
			var pictureDownload = new WWW(avatarAsset);
	...
	private void StarCollectionAwake()
	{
		foreach (var offer in DIContainerLogic.GetShopService().GetShopOffers(DIContainerInfrastructure.GetCurrentPlayer(),"shop_star_rewards"))
		{
			var nameId = offer.NameId.Replace("offer_star_reward", "star_popup");
			...
			if (DIContainerLogic.InventoryService.TryGetItemGameData(
				    ...
				    DIContainerBalancing.Service.GetBalancingData<BasicItemBalancingData>(nameId),
				    out data))
			{
				if (data.ItemData.IsNew)
				{
					m_starCollectionUpdateMarker.SetActive(true);
					return;
				}
			}
		}
		m_starCollectionUpdateMarker.SetActive(false);
	}

	private IEnumerator InitializeStarCollection()
	{
		m_LeftButton.gameObject.SetActive(false);
		m_RightButton.gameObject.SetActive(false);
		m_HeaderText.text = DIContainerInfrastructure.GetLocaService().Tr("social_starcollection_header", "Star Collection");
		...
		var currentStars = DIContainerLogic.InventoryService.GetItemValue(DIContainerInfrastructure.GetCurrentPlayer().InventoryGameData, "star_collection");
		var caveStars = DIContainerBalancing.Service.GetBalancingDataList<ChronicleCaveHotspotBalancingData>().Count(c => c.Type == HotspotType.Battle) * 3;
		var worldMapStars = DIContainerBalancing.Service.GetBalancingDataList<HotspotBalancingData>().Count(h => h.CountForStars) * 3;
		var maxStars = caveStars + worldMapStars;
		m_StarCollectionCounter.text = currentStars + " / " + maxStars;

		var player = DIContainerInfrastructure.GetCurrentPlayer();

		foreach (var offer in DIContainerLogic.GetShopService().GetShopOffers(player, "shop_star_rewards"))
		{
			var shopOffer = offer;
			if (shopOffer.NameId == "offer_star_reward_04_snoutling")
			{
				if (player.Data.WonAvengerByStars)
				{
					shopOffer = DIContainerLogic.GetShopService().GetShopOffer("offer_star_reward_04");
				}
			}
			var neededStars = 0f;
			foreach (var requirement in shopOffer.BuyRequirements)
			{
				if (requirement.NameId == "star_collection")
				{
					neededStars = requirement.Value;
				}
			}

			var content = DIContainerLogic.GetShopService().GetShopOfferContent(player, shopOffer, DIContainerLogic.GetSalesManagerService().GetOfferSaleDetails(offer.NameId));
			IInventoryItemGameData reward = null;
			foreach (var offerReward in content)
			{
				if (!offerReward.ItemBalancing.NameId.StartsWith("star_aquiredreward"))
				{
					reward = offerReward;
					break;
				}
			...
			var starReward = Instantiate(m_StarRewardPrefab);
			starReward.name = neededStars + "_ListElement_BattleStarsReward";
			var costBlind = starReward.GetComponentInChildren<ResourceCostBlind>();

			if (costBlind != null)
			{
				costBlind.SetModel(string.Empty, null, DIContainerInfrastructure.GetFormatProvider().GetResourceAmountFormat((int)neededStars), string.Empty, false);
			}
			
			starReward.transform.parent = m_StarCollectionGrid.transform;
			starReward.transform.localPosition = Vector3.zero;
			starReward.transform.localScale = Vector3.one;
			
			if (reward is ConsumableItemGameData)
			{
				starReward.GetComponent<GenericOverlayInvoker>().m_LocaIdent = reward.ItemBalancing.LocaBaseId + "_desc";
			}
			else
			{
				starReward.GetComponent<GenericOverlayInvoker>().m_LocaIdent = reward.ItemBalancing.LocaBaseId + "_tt";
			}
			
			starReward.GetComponent<LootDisplayContoller>().SetModel(reward, new List<IInventoryItemGameData>(), LootDisplayType.None, "_Large", false, false, true);
			if (currentStars >= neededStars)
			{
				var nameId = shopOffer.NameId.Replace("offer_star_reward", "star_popup");
				IInventoryItemGameData data;
				if (DIContainerLogic.InventoryService.TryGetItemGameData(player.InventoryGameData, DIContainerBalancing.Service.GetBalancingData<BasicItemBalancingData>(nameId), out data) && data.ItemData.IsNew)
				{
					StartCoroutine(PlayDelayedAnimation(starReward.GetComponent<Animator>(), "BattleStarsReward_Achieved", 1.5f));
					data.ItemData.IsNew = false;
				}
				else
				{
					starReward.GetComponent<Animator>().Play("BattleStarsReward_SetAchieved");
				}
			}
		}
		m_StarCollectionGrid.GetComponent<UIGrid>().Reposition();
	...
	private void Awake()
	{
		...
		m_RootDictionary.Add(SocialWindowCategory.StarCollection, m_StarCollectionRoot);
		...
		m_FooterDictionary.Add(SocialWindowCategory.StarCollection, m_StarCollectionFooterRoot);
		...
		m_ArrowTransform.position = new Vector3(m_ArrowTransform.position.x, m_StarCollectionButton.transform.position.y + m_arrowStartPositionCompensation, m_ArrowTransform.position.z);
		m_ArrowPostions.Add(SocialWindowCategory.StarCollection, m_ArrowTransform.localPosition);
	...
	private IEnumerator EnterCoroutine()
	{
		...
		if (m_AreaSelectionAnimation && m_AreaSelectionAnimation.Play("AreaSelection_Enter"))
		{
			enterTime = Mathf.Max(enterTime, m_AreaSelectionAnimation["AreaSelection_Enter"].length);
		}
		if (m_HeaderAnimation && m_HeaderAnimation.Play("Header_Enter"))
		{
			enterTime = Mathf.Max(enterTime, m_HeaderAnimation["Header_Enter"].length);
		}
		if (m_FooterAnimation && m_FooterAnimation.Play("Footer_Enter"))
		{
		...
		if (m_ContentAnimation && m_ContentAnimation.Play("List_Enter"))
		{
	...
	private IEnumerator LeaveCoroutine(bool withRoot = true)
	{
		...
		var leaveTime = 0f;
		if (m_AreaSelectionAnimation && m_AreaSelectionAnimation.Play("AreaSelection_Leave"))
		{
			leaveTime = Mathf.Max(leaveTime, m_AreaSelectionAnimation["AreaSelection_Leave"].length);
		}
		if (m_HeaderAnimation && m_HeaderAnimation.Play("Header_Leave"))
		{
			leaveTime = Mathf.Max(leaveTime, m_HeaderAnimation["Header_Leave"].length);
		}
		if (m_FooterAnimation && m_FooterAnimation.Play("Footer_Leave"))
		{
			leaveTime = Mathf.Max(leaveTime, m_FooterAnimation["Footer_Leave"].length);
		}
		if (m_ContentAnimation && m_ContentAnimation.Play("List_Leave"))
		{
	...
	private IEnumerator SwitchCategory(SocialWindowCategory category, bool animated)
	{
		...
		if (animated)
		{
			var changeOutTime = 0f;
			if (m_ContentAnimation && m_ContentAnimation.Play("List_Change_Out"))
			{
		...
		foreach (var key in m_RootDictionary.Keys)
		{
			var value = m_RootDictionary[key];
			value.SetActive(key == category);
		}
		switch (category)
		{
		case SocialWindowCategory.RovioId:
			if (m_ClaimAllButtonObject)
				m_ClaimAllButtonObject.SetActive(false);
			m_FacebookFooterRoot.SetActive(false);
			m_StarCollectionFooterRoot.SetActive(false);
		...
		case SocialWindowCategory.Friends:
			if (m_ClaimAllButtonObject)
				m_ClaimAllButtonObject.SetActive(false);
			m_ClaimAllButtonObject.transform.parent.GetComponent<UIGrid>().Reposition();
			if (m_StarCollectionFooterRoot)
				m_StarCollectionFooterRoot.SetActive(false);
			if (!DIContainerInfrastructure.GetFacebookWrapper().IsUserAuthenticated())
			{
		...
		case SocialWindowCategory.Mailbox:
			if (m_ClaimAllButtonObject)
				m_ClaimAllButtonObject.SetActive(true);
			m_ClaimAllButtonObject.transform.parent.GetComponent<UIGrid>().Reposition();
			if (m_StarCollectionFooterRoot)
				m_StarCollectionFooterRoot.SetActive(false);
			if (!DIContainerInfrastructure.GetFacebookWrapper().IsUserAuthenticated())
			...
			break;
		case SocialWindowCategory.StarCollection:
			m_FriendshipEssenceFooterRoot.SetActive(false);
			m_ChallengeFooterRoot.SetActive(false);
			m_RovioLoginFooterRoot.SetActive(false);
			m_FacebookFooterRoot.SetActive(false);
			m_StarCollectionFooterRoot.SetActive(true);
			if (m_ClaimAllButtonObject)
				m_ClaimAllButtonObject.SetActive(false);
			break;
		}
		...
		case SocialWindowCategory.Friends:
			...
			break;
		case SocialWindowCategory.StarCollection:
			yield return StartCoroutine(InitializeStarCollection());
			break;
		}
		...
		{
			var changeInTime = 0f;
			if (m_ContentAnimation && m_ContentAnimation.Play("List_Change_In"))
			{
	...
	private void RegisterCategoryButtons()
	{
		...
		if (m_StarCollectionButton)
		{
			m_StarCollectionButton.Clicked += m_StarCollectionButton_Clicked;
		}
		if (m_MailboxButton)
		{
	...
	private void m_StarCollectionButton_Clicked()
	{
		if (m_CurrentCategory != SocialWindowCategory.StarCollection)
		{
			StartCoroutine(SwitchCategory(SocialWindowCategory.StarCollection, true));
		}
	}	
	...
	private void m_FriendshipEssenceButton_Clicked()
	{
		...
		foreach (var value in DIContainerInfrastructure.GetCurrentPlayer().SocialEnvironmentGameData.Friends.Values)
		{
