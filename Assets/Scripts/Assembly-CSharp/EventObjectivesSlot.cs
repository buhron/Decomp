...
public class EventObjectivesSlot : MonoBehaviour
{
	...
	public float SetModelAndGetExtent(string assetName, int difficulty, float energyCost)
	{
		...
		{
			var character = Object.Instantiate(m_CharacterControllerPrefab);
			character.SetModel(assetName, false);
			character.transform.parent = m_CharacterControllerContainer ?? base.transform;
			var sizeTypeToSpacerScale = m_SizeTypeToSpacerScaleList.FirstOrDefault(s => s.CharacterSize == character.GetModel().CharacterSize);
		...
		{
			var gameObject = DIContainerInfrastructure.PropLiteAssetProvider().InstantiateObject(assetName, base.transform, Vector3.zero, Quaternion.identity);
			if (gameObject != null)
			{
				UnityHelper.SetLayerRecusively(gameObject.gameObject, base.gameObject.layer);
			}
			var sizeTypeToSpacerScale2 = m_SizeTypeToSpacerScaleList.FirstOrDefault(s => s.CharacterSize == CharacterSizeType.Medium);
