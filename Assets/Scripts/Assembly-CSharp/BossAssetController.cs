...
public class BossAssetController : CharacterAssetController
{
	...
	{
		var userTriggerEvent = new UserTriggerEvent();
	...
	private IEnumerator HandleReviveTimer()
	{
		...
		var targetTime = player.Data.BossStartTime + (uint)player.CurrentEventManagerGameData.CurrentEventBoss.BalancingData.TimeToReactivate;
		var timeRemaining2 = (int)(targetTime - DIContainerLogic.GetTimingService().GetCurrentTimestamp());
		while (DIContainerLogic.EventSystemService.IsBossOnCooldown() && base.gameObject.activeSelf)
		{
			...
			var minutes = timeRemaining2 % 3600 / 60;
			var seconds = timeRemaining2 % 60;
			var locaIdent = player.CurrentEventManagerGameData.CurrentEventBoss.BalancingData.DefeatedLabelLocaId;
