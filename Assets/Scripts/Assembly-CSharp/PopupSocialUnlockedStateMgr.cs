...
public class PopupSocialUnlockedStateMgr : MonoBehaviour
{
	...
	private GameObject InstantiateFeatureObject(BasicItemGameData basicItemGameData)
	{
		...
		if (basicItemGameData.ItemAssetName.StartsWith("bird_"))
		{
			...
			var characterControllerCamp = Object.Instantiate(m_CampCharacterControllerPrefab);
		...
		{
			m_FeatureObjectNameId = basicItemGameData.ItemAssetName;
			var characterControllerCamp2 = Object.Instantiate(m_CampCharacterControllerPrefab);
			m_ObjectType = FeatureObjectType.BirdUnlock;
			...
			var balancingData = DIContainerBalancing.Service.GetBalancingData<PigBalancingData>(m_FeatureObjectNameId);
		...
		{
			m_FeatureObjectNameId = basicItemGameData.ItemAssetName;
			var gameObject = DIContainerInfrastructure.PropLiteAssetProvider().InstantiateObject(m_FeatureObjectNameId, m_CenteredItemRoot, Vector3.zero, Quaternion.identity);
