...
public class AndroidBackButtonMgr : MonoBehaviour
{
	...
	{
		var flag = false;
		for (var i = 0; i < m_currentBackButtonActions.Count; i++)
		{
		...
		m_currentBackButtonActions = m_currentBackButtonActions.OrderBy(a => a.Depth).ToList();
	}

	public void DeRegisterAction(int depth)
	{
		for (var num = m_currentBackButtonActions.Count - 1; num >= 0; num--)
		{
	...
	public void DeRegisterBlockReason(string reason)
	{
		...
		foreach (var blockReason in m_BlockReasons)
		{
