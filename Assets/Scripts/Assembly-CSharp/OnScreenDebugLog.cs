...
public class OnScreenDebugLog : MonoBehaviour
{
	...
	private void Update()
	{
		...
		{
			var logEntry = m_logQueue.Dequeue();
	...
	{
		var path = Application.persistentDataPath + "\\InGameDebugLog_" + DateTime.Now.Date.ToString("dd.MM.yyyy") + "_" + DateTime.Now.ToString("hhmmss") + ".txt";
	...
	private void DebugLogWindow(int windowID)
	{
		...
		var i = 0;
		for (var count = LogEntries.Count; i < count; i++)
		{
	...
	{
		var point = GUIUtility.ScreenToGUIPoint(new Vector2(Input.mousePosition.x, (float)screenHeight - Input.mousePosition.y));
		var rect = GUILayoutUtility.GetRect(m_gcDrag, m_styleWindowResize);
