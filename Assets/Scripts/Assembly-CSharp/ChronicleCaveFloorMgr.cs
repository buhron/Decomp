...
public class ChronicleCaveFloorMgr : ChronicleCaveFloorSlot
{
	...
	private void Awake()
	{
		...
		var hotspotGameData = m_Model.HotspotGameDatas.Values.FirstOrDefault(h => h.BalancingData.Type == HotspotType.Battle);
		if (hotspotGameData != null)
		{
			...
			{
				{ "{value_1}", m_StateMgr.m_Floors.Count.ToString("0") },
				{ "{value_2}", DIContainerInfrastructure.GetLocaService().GetZoneName(hotspotGameData.BalancingData.ZoneLocaIdent) }
			});
		}
		...
		foreach (var hotSpotWorldMapViewBase in array)
		{
	...
	public override void InstantiateRedKeyBubble()
	{
		...
		var firstPossibleBattle = DIContainerLogic.GetBattleService().GetFirstPossibleBattle(m_LeavingHotSpot.Model.BalancingData.BattleId, DIContainerInfrastructure.GetCurrentPlayer(), true);
		...
		if (!string.IsNullOrEmpty(firstPossibleBattle) && DIContainerBalancing.Service.TryGetBalancingData<ChronicleCaveBattleBalancingData>(firstPossibleBattle, out balancing) && balancing.LootTableAdditional != null && balancing.LootTableAdditional.ContainsKey("key_red"))
		{
			...
			var gameObject = UnityEngine.Object.Instantiate(m_StateMgr.m_RedKeyBubble);
			if (gameObject != null)
			{
	...
	{
		var hotspotWorldMapView = m_EnteringHotSpot.GetHotspotWorldMapView(hotspot.NameId);
	...
	{
		yield return StartCoroutine(m_EnteringHotSpot.ActivateFollowUpStagesAsync(m_EnteringHotSpot.GetPreviousHotspot(), m_LeavingHotSpot));
	}
