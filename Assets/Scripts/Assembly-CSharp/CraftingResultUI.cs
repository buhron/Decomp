...
public class CraftingResultUI : MonoBehaviour
{
	...
	private void SetupRerollBlind()
	{
		if (m_amount == 1)
		{
			var rerollRequirement = DIContainerLogic.CraftingService.GetRerollRequirement(m_timesRerolled);
		...
		{
			var multiRerollRequirement = DIContainerLogic.CraftingService.GetMultiRerollRequirement(m_timesRerolled);
	...
	private IEnumerator LeaveCoroutine(bool disable, bool leaveBackground = true)
	{
		...
		{
			foreach (var item in m_ExplodedLoot)
			{
		...
		var starAnimations = m_StarAnimations;
		foreach (var sanim in starAnimations)
		{
	...
	public void OnSkipClicked()
	{
		...
		for (var i = 0; i < m_DiceRoots.Count; i++)
		{
		...
		var num2 = 0;
		for (var k = 0; k < m_starList.Count; k++)
		{
			if (!m_starList[k])
			{
				continue;
	...
	public void OnRepeatClicked()
	{
		...
		{
			var list = DIContainerLogic.CraftingService.CraftItem(m_recipe, DIContainerInfrastructure.GetCurrentPlayer().InventoryGameData, m_IsAlchemy, m_amount);
	...
	public void OnRerollButtonClicked()
	{
		...
		if (DIContainerLogic.CraftingService.IsRerollPossible(DIContainerInfrastructure.GetCurrentPlayer().InventoryGameData) && DIContainerLogic.CraftingService.ExecuteRerollCost(m_timesRerolled, DIContainerInfrastructure.GetCurrentPlayer().InventoryGameData, m_amount))
		{
			DeregisterEventHandler();
	...
	private IEnumerator EnterCoroutine()
	{
		...
		if (m_equipableItem)
		{
			var newStat = m_item.ItemMainStat;
			var oldStat = m_item.ItemBalancing.ItemType != InventoryItemType.MainHandEquipment ? m_PossibleBird.OffHandItem.ItemMainStat : m_PossibleBird.MainHandItem.ItemMainStat;
		...
		var destroyFX = UnityEngine.Object.Instantiate(m_DiceDestroyFX, m_FXRoot.transform.position, Quaternion.identity) as GameObject;
		...
		for (var k = 0; k < m_DiceRoots.Count; k++)
		{
		...
		m_ItemStatsRoot.SetActive(m_equipableItem);
		m_CraftedItemRoot.SetActive(true);
		var scrapItems = new List<IInventoryItemGameData>();
		m_ItemDisplay.gameObject.SetActive(true);
		...
		for (var i = 0; i < m_starList.Count; i++)
		{
			if (!m_starList[i])
			{
				continue;
	...
	private void SpawnScrappingBubble()
	{
		...
		var num = m_item.ItemMainStat - m_PossibleBird.MainHandItem.ItemMainStat;
	...
	private IEnumerator AnimateBird()
	{
		...
		var minWaitTime = m_birdAnimation["Cheer"].length + m_birdAnimation["Idle"].length;
		while (true)
		{
			if (m_betterItem)
			{
	...
	public void SetItem(IInventoryItemGameData newItem, CraftingRecipeGameData recipe)
	{
		...
		for (var j = 0; j < m_item.ItemData.Quality; j++)
		{
			m_starList[j] = true;
		}
		if (m_equipableItem)
