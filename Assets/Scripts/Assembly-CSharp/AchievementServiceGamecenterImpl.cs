...
public class AchievementServiceGamecenterImpl : AchievementServiceBase, IAchievementService
{
	...
	private void processAchievements(IAchievement[] achievementList)
	{
		...
		var obj = (float)achievementList.Length / (float)DIContainerBalancing.Service.GetBalancingDataList<ThirdPartyIdBalancingData>().Count(a => !string.IsNullOrEmpty(a.GamecenterAchievementId));
