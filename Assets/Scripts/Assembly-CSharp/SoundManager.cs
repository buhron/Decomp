...
public class SoundManager : MonoBehaviour, ISoundManager
{
	...
	public void SetVolume(float volume, int channelId)
	{
		...
		foreach (var item in m_ChannelLookup[channelId])
		{
			if (item.Source != null)
			{
	...
	public float GetVolume(int channelId)
	{
		...
		using (var enumerator = m_ChannelLookup[channelId].GetEnumerator())
		{
	...
	public IEnumerator FadeOut(float time)
	{
		...
		while (DIContainerInfrastructure.AudioManager.GetVolume(0) > 0f && DIContainerInfrastructure.AudioManager.GetVolume(1) > 0f)
		{
			...
			var cVol0 = Mathf.Lerp(GetVolume(0), 0f, cTime / time);
			var cVol1 = Mathf.Lerp(GetVolume(1), 0f, cTime / time);
			SetVolume(cVol0, 0);
			SetVolume(cVol1, 1);
	...
	public IEnumerator FadeIn(float time)
	{
		...
		{
			cTime += Time.deltaTime;
			var cVol0 = Mathf.Lerp(DIContainerInfrastructure.AudioManager.GetVolume(0), 1f, cTime / time);
			var cVol1 = Mathf.Lerp(DIContainerInfrastructure.AudioManager.GetVolume(1), 1f, cTime / time);
	...
	{
		var muteReasonsFor = GetMuteReasonsFor(channelId);
