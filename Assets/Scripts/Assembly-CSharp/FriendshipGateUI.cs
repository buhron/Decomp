...
public class FriendshipGateUI : MonoBehaviour
{
	...
	private void OnBuyFriendsButtonClicked()
	{
		if (DIContainerLogic.RequirementService.ExecuteRequirements(DIContainerInfrastructure.GetCurrentPlayer().InventoryGameData, new List<Requirement> { GetNeededFriendsCost() }, "buy_friends"))
		{
			...
			var neededFriends = GetNeededFriends(m_hotspotBalancing.EnterRequirements, DIContainerInfrastructure.GetCurrentPlayer());
			for (var i = 0; i < neededFriends; i++)
			{
				AddNPCFriendUnlock();
			}
			FillInfos();
		...
		var neededFriendsCost = GetNeededFriendsCost();
	...
	{
		var lowNPCFriend = DIContainerLogic.SocialService.GetLowNPCFriend(DIContainerInfrastructure.GetCurrentPlayer().Data.Level);
	...
	private Requirement GetNeededFriendsCost()
	{
		...
		var basicShopOfferBalancingData = DIContainerLogic.GetShopService().GetShopOffers(DIContainerInfrastructure.GetCurrentPlayer(), "friend_gate").FirstOrDefault();
		if (basicShopOfferBalancingData != null)
		{
			var requirement2 = basicShopOfferBalancingData.BuyRequirements.FirstOrDefault();
	...
	{
		var requirement = reqs.FirstOrDefault(r => r.RequirementType == RequirementType.UsedFriends);
		var value = new List<string>();
		player.SocialEnvironmentGameData.Data.FriendShipGateUnlocks.TryGetValue(m_hotspotBalancing.NameId, out value);
		var num = value != null ? value.Count : 0;
	...
	private void SetFacebookText()
	{
		...
		{
			var gameObject = DIContainerInfrastructure.GetGenericIconAtlasAssetProvider().GetObject("ShopAndSocialElements") as GameObject;
			if (gameObject != null)
			{
	...
	private void FillInfos()
	{
		...
		for (num = 0; num < Mathf.Min(value.Count, m_FriendInfos.Count); num++)
		{
			...
			if (DIContainerInfrastructure.GetCurrentPlayer().SocialEnvironmentGameData.Friends.TryGetValue(text, out value3))
			{
				...
				if (!value3.isNpcFriend)
				{
					...
					var componentsInChildren = m_FriendInfos[num].GetComponentsInChildren<UITexture>(true);
					foreach (var uITexture in componentsInChildren)
					{
		...
		for (var j = num; j < m_FriendInfos.Count; j++)
		{
			m_FriendInfos[j].SetNPCIcon(false);
			var componentsInChildren2 = m_FriendInfos[j].GetComponentsInChildren<UITexture>(true);
			foreach (var uITexture2 in componentsInChildren2)
			{
	...
	{
		var list = player.SocialEnvironmentGameData.MailboxMessages.Values.Where(m => m is ResponseFriendshipGateMessage).ToList();
	...
	private void OnSendHelpFriendsButtonClicked()
	{
		...
		foreach (var value2 in DIContainerInfrastructure.GetCurrentPlayer().SocialEnvironmentGameData.Friends.Values)
		{
	...
	private void SendFriendshipGateMessage(List<string> friendIds)
	{
		...
		{
			// yes this is just here like this... they forgot to remove it
			var message = new MessageDataIncoming
			{
