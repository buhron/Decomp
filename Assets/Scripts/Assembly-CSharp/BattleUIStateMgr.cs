...
public class BattleUIStateMgr : MonoBehaviour
{
	...
	public SkillGameData m_EnvironmentalEffect;
	...
	public void SetBattleMgr(BattleMgrBase battleMgr)
	{
		...
		var flag = m_BattleMgr.Model.m_ChronicleCaveBattle || m_BattleMgr.Model.Balancing.NameId.StartsWith("battle_event");
	...
	private void m_ReviveBirdsButton_Clicked()
	{
		...
		var requirement = m_BattleMgr.Model.IsPvP 
			? DIContainerBalancing.GameConstantsBalancingDataProvider.ReviveBirdsPvpRequirement 
			: DIContainerBalancing.GameConstantsBalancingDataProvider.ReviveBirdsRequirement;
	...
	public float EnterConsumableButton()
	{
		...
		if (m_BattleMgr.Model.m_CombatantsByInitiative.Any(c => c.CanUseConsumable && c.CurrentHealth / c.ModifiedHealth <= m_WarningHealthPercent))
		{
	...
	{
		if (m_PauseButtonRoot && m_PauseButtonEntered && !m_BattleMgr.m_BattleMainLoopDone)
		{
			m_PauseButtonRoot.GetComponent<Animation>().Play("PauseButton_Leave");
			yield return new WaitForSeconds(m_PauseButtonRoot.GetComponent<Animation>()["PauseButton_Leave"].length);
			m_PauseButtonEntered = false;
	...
	{
		if (m_AutoBattleButtonRoot && m_AutoBattleButtonEntered && !m_BattleMgr.m_BattleMainLoopDone)
		{
			m_AutoBattleButtonRoot.GetComponent<Animation>().Play("AutoBattle_Leave");
			yield return new WaitForSeconds(m_AutoBattleButtonRoot.GetComponent<Animation>()["AutoBattle_Leave"].length);
			m_AutoBattleButtonEntered = false;
