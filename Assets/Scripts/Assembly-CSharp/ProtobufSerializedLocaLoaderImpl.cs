...
public class ProtobufSerializedLocaLoaderImpl : ILocaLoader
{
	...
	public ProtobufSerializedLocaLoaderImpl Init(ISerializer binarySerializer)
	{
	...
	public Dictionary<string, string> Load(string language)
	{
		...
		{
			var textAsset = Resources.Load(EditorLocaResourcesPath + GetLocaFilenameFromLanguage(language), typeof(TextAsset)) as TextAsset;
			if (textAsset == null)
			{
		...
		foreach (var injectedText in m_InjectedTexts)
		{
		...
		var bytes = DIContainerInfrastructure.GetCompressionService().DecompressIfNecessary(textAsset2.bytes);
		var serializedLocalizedTexts = m_binarySerializer.Deserialize<SerializedLocalizedTexts>(bytes);
		foreach (var injectedText2 in m_InjectedTexts)
		{
		...
		return serializedLocalizedTexts.Texts;
	}
