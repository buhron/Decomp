...
public class MailboxMessageBlind : MonoBehaviour
{
	...
	public void SetModel(IMailboxMessageGameData model, BaseCampStateMgr stateMgr, int index, bool PictureAlreadySet = false)
	{
		...
		if (m_highlight != null)
		{
			m_highlight.SetActive((index & 1) == 1); // number is odd ( aka (index % 2) == 1 )
		}
		if (m_Model.IsNotSimpleCustomMessage)
		{
			...
			{
				var gameObject = DIContainerInfrastructure.GetGenericIconAtlasAssetProvider().GetObject(m_Model.IconAtlasName) as GameObject;
				if (gameObject != null)
				{
	...
	public void ConfirmButtonTriggerClicked()
	{
		if (m_SoundTriggers)
	...
	{
		if (m_ConfirmButtonTrigger)
		{
			m_ConfirmButtonTrigger.Clicked -= ConfirmButtonTriggerClicked;
		}
		if (m_TextButtonTrigger)
		{
			m_TextButtonTrigger.Clicked -= TextButtonClicked;
		}
	}
