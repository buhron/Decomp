...
public class EventSystemWorldMapStateMgr : MonoBehaviour
{
	...
	public bool m_BossDefeatAniShown;

	private bool m_BossHpUpdateRunning;

	public int m_MaxAttacksOnFirstSpawn = 5;
	...
	private IEnumerator Start()
	{
		...
		{
			if (allHotspotPositionNodes[i].SynchBalancing(bossSuffix) && DIContainerLogic.RequirementService.CheckGenericRequirements(player, allHotspotPositionNodes[i].GetModel().SpawnAbleRequirements))
			{
	...
	private void OnApplicationPause(bool pauseStatus)
	{
		...
		else if (DIContainerInfrastructure.GetCurrentPlayer().CurrentEventManagerGameData != null && DIContainerInfrastructure.GetCurrentPlayer().CurrentEventManagerGameData.IsBossEvent)
		{
			...
			{
				OnGetBossDefeatLogCallback(result, false);
			});
		}
	}
	...
	private IEnumerator SpawnMiniCampaignPortal()
	{
		...
		var eventCampaignItems = DIContainerInfrastructure.GetCurrentPlayer().InventoryGameData.Items[InventoryItemType.EventCampaignItem];
		if (eventCampaignItems.Count > 0)
		{
			for (var i = 0; i < eventCampaignItems.Count; i++)
			{
				var item = eventCampaignItems[i];
		...
		var miniCampName = mcItem.BalancingData.EventParameters[0];
		var cMiniMapData = DIContainerInfrastructure.GetCurrentPlayer().CurrentEventManagerGameData.Data.EventCampaignData;
		var cMiniMap = cMiniMapData == null ? new EventCampaignGameData(miniCampName) : new EventCampaignGameData(cMiniMapData);
		DIContainerInfrastructure.GetCurrentPlayer().CurrentEventManagerGameData.CurrentMiniCampaign = cMiniMap;
		foreach (var node in m_EntranceNodes.Values)
		{
	...
	private IEnumerator SpawnEncounters()
	{
		...
		if (DIContainerInfrastructure.EventSystemStateManager.lastRemovedEventItem != null)
		{
			...
			if (m_PossiblePositionNodes.TryGetValue(DIContainerInfrastructure.EventSystemStateManager.lastRemovedEventItem.Data.PositionId, out possiblePositionNode))
			{
				possiblePositionNode.SetItem(DIContainerInfrastructure.EventSystemStateManager.lastRemovedEventItem, true);
				yield return new WaitForSeconds(DIContainerInfrastructure.LocationStateMgr.TweenCameraToTransform(possiblePositionNode.transform));
				while (!possiblePositionNode.IsNodeSpawned)
				{
					yield return new WaitForEndOfFrame();
				}
				yield return new WaitForSeconds(possiblePositionNode.AccomplishItem());
			}
		...
		var battleEventItemGameDatas = DIContainerInfrastructure.GetCurrentPlayer().InventoryGameData.Items[InventoryItemType.EventBattleItem];
		var orderedByDistance2 = m_PossiblePositionNodes.Values.ToList();
		for (var j = 0; j < battleEventItemGameDatas.Count; j++)
		{
			var inventoryItemGameData = battleEventItemGameDatas[j];
			var battleEventItemGameData = (EventItemGameData)inventoryItemGameData;
			if (!string.IsNullOrEmpty(battleEventItemGameData.Data.PositionId))
			{
				EventPositionNode possiblePositionNode2 = null;
				if (m_PossiblePositionNodes.TryGetValue(battleEventItemGameData.Data.PositionId, out possiblePositionNode2))
				{
					possiblePositionNode2.SetItem(battleEventItemGameData);
		...
		if (DIContainerInfrastructure.GetCurrentPlayer().CurrentEventManagerGameData.EventBalancing.MaxAmountOfEventItems > battleEventItemGameDatas.Count)
		{
			...
			var takeNearest = false;
			if (DIContainerInfrastructure.LocationStateMgr.SceneryCamera && battleEventItemGameDatas.Count == 0)
			{
				orderedByDistance2 = orderedByDistance2.OrderBy(node => Vector3.Distance(node.transform.position, DIContainerInfrastructure.LocationStateMgr.SceneryCamera.transform.position)).ToList();
				nearestEventPositionNode = orderedByDistance2.FirstOrDefault();
				if (nearestEventPositionNode && !nearestEventPositionNode.HasItem())
				{
					takeNearest = true;
				}
			}
			var eventItemsCountOld = battleEventItemGameDatas.Count;
			for (var i = 0; i < DIContainerInfrastructure.GetCurrentPlayer().CurrentEventManagerGameData.EventBalancing.MaxAmountOfEventItems - eventItemsCountOld; i++)
			{
				if (takeNearest)
				{
				...
				var randomNodes = m_PossiblePositionNodes.Values.ToList();
				if (randomNodes.Count <= 0)
				{
					DebugLog.Error("[EventSystemWorldMapStateMgr] No further spawn locations available!");
					break;
				}
				var randomEventPositionNode = randomNodes[Random.Range(0, randomNodes.Count)];
	...
	private IEnumerator SpawnCollectibles()
	{
		...
		{
			EventPositionNode possiblePositionNode = null;
			if (m_PossibleCollectibleNodes.TryGetValue(DIContainerInfrastructure.EventSystemStateManager.lastRemovedEventItem.Data.PositionId, out possiblePositionNode))
			{
				possiblePositionNode.SetItem(DIContainerInfrastructure.EventSystemStateManager.lastRemovedEventItem, true);
				yield return new WaitForSeconds(DIContainerInfrastructure.LocationStateMgr.TweenCameraToTransform(possiblePositionNode.transform));
				while (!possiblePositionNode.IsNodeSpawned)
		...
		var collectibleEventItemGameDatas = DIContainerInfrastructure.GetCurrentPlayer().InventoryGameData.Items[InventoryItemType.EventCollectible];
		for (var j = 0; j < collectibleEventItemGameDatas.Count; j++)
		{
			var inventoryItemGameData = collectibleEventItemGameDatas[j];
			var collectibleEventItemGameData = (EventItemGameData)inventoryItemGameData;
			...
			EventPositionNode possiblePositionNode2 = null;
			if (m_PossibleCollectibleNodes.TryGetValue(collectibleEventItemGameData.Data.PositionId, out possiblePositionNode2))
			{
				possiblePositionNode2.SetItem(collectibleEventItemGameData);
				m_PossibleCollectibleNodes.Remove(collectibleEventItemGameData.Data.PositionId);
			}
		}
		var lastSpawn = DIContainerInfrastructure.GetCurrentPlayer().CurrentEventManagerGameData.Data.LastCollectibleSpawnTime;
		var respawnTime = DIContainerInfrastructure.GetCurrentPlayer().CurrentEventManagerGameData.EventBalancing.TimeForCollectibleRespawnInSec;
		var numberOfCollectiblesRemaining = DIContainerInfrastructure.GetCurrentPlayer().CurrentEventManagerGameData.EventBalancing.MaxNumberOfCollectibles - DIContainerInfrastructure.GetCurrentPlayer().InventoryGameData.Items[InventoryItemType.EventCollectible].Count;
		var numberOfItemsToSpawn = lastSpawn == 0 ? DIContainerInfrastructure.GetCurrentPlayer().CurrentEventManagerGameData.EventBalancing.MaxNumberOfCollectibles : 0;
		...
		{
			var k = (int)((float)(DIContainerLogic.GetTimingService().GetCurrentTimestamp() - lastSpawn) / respawnTime);
		...
		if (numberOfItemsToSpawn > 0)
		{
			...
			{
				var eventItem = DIContainerLogic.EventSystemService.GenerateEventCollectible(DIContainerInfrastructure.GetCurrentPlayer().CurrentEventManagerGameData.EventBalancing, DIContainerInfrastructure.GetCurrentPlayer());
				var randomNodes = (from node in m_PossibleCollectibleNodes.Values.ToList()
	...
	private IEnumerator SpawnBoss()
	{
		...
		var player = DIContainerInfrastructure.GetCurrentPlayer();
		var emgr = player.CurrentEventManagerGameData;
		if (emgr == null || !emgr.IsBossEvent)
		{
			yield break;
		}
		EventItemGameData eItem = null;
		var eventBossItems = player.InventoryGameData.Items[InventoryItemType.EventBossItem];
		if (eventBossItems.Count > 0)
		{
			eItem = eventBossItems[0] as EventItemGameData;
		}
		else
		{
			eItem = DIContainerLogic.EventSystemService.GenerateEventBoss(emgr.EventBalancing, player);
		}
		var currentBossNodeId = emgr.CurrentEventBoss.EventNodeId;
		if (currentBossNodeId == null)
		{
			var bossNodeId = GetBossNodeId(Random.Range(0, this.m_BossNodes.Count - 1), 0, 10);
		...
		if (m_BossNodes.TryGetValue(currentBossNodeId, out this.m_currentBossNode))
		{
			m_currentBossNode.SetItem(eItem, false);
			if (string.IsNullOrEmpty(player.m_LastBattle) || player.m_LastBattle.Contains("boss"))
			{
				DIContainerInfrastructure.LocationStateMgr.TweenCameraToTransform(this.m_currentBossNode.transform);
			}
		}
		if (string.IsNullOrEmpty(emgr.Data.LeaderboardId))
		{
			DebugLog.Log(base.GetType(), "SpawnBoss: No leaderboard yet, therefore not initializing boss combatant!!");
			yield break;
		}
	...
	public EventPositionNode GetBossNode()
	{
		...
		{
			var eventNodeId = DIContainerInfrastructure.GetCurrentPlayer().CurrentEventManagerGameData.CurrentEventBoss.EventNodeId;
			m_BossNodes.TryGetValue(eventNodeId + "_" + GetBossSuffix(), out m_currentBossNode);
		}
		return m_currentBossNode;
	}
	
	private string GetBossSuffix()
	{
		var player = DIContainerInfrastructure.GetCurrentPlayer();
		if (!DIContainerLogic.EventSystemService.IsCurrentEventAvailable(player) || !player.CurrentEventManagerGameData.IsBossEvent)
		{
			return string.Empty;
		}
		var bossName = player.CurrentEventManagerGameData.EventBalancing.NameId.Replace("event_boss_", string.Empty);
		
		if (bossName.Contains("_"))
			return bossName.Substring(0, Math.Max(bossName.IndexOf("_"), 0));
	...
	public IEnumerator PlayBossDefeatAnimation()
	{
		...
		var bossAssetController = m_currentBossNode.GetComponentInChildren<BossAssetController>();
		var timeout = 0f;
	...
	private IEnumerator WorldBossWasAttackedVisuals(PublicPlayerData attackingPlayer)
	{
		...
		var balancing = DIContainerInfrastructure.GetCurrentPlayer().CurrentEventManagerGameData.CurrentEventBoss.BalancingData;
	...
	{
		foreach (var value in m_BossNodes.Values)
		{
			if (value.m_balancingName.Contains(index.ToString()))
			{
				return value.m_balancingName + "_" + GetBossSuffix();
			}
		}
	...
	private IEnumerator UpdateWorldBossHPCoroutine()
	{
		...
		var player = DIContainerInfrastructure.GetCurrentPlayer();
		var bossData = player.Data.WorldBoss;
		...
		var ownTeam = bossData.OwnTeamId != 1 ? bossData.Team2 : bossData.Team1;
		var currentEventBoss = player.CurrentEventManagerGameData.CurrentEventBoss;
		...
		var healthBar = m_currentBossNode.GetComponentInChildren<CharacterChunkHealthBar>();
		...
		for (var i = 0; i < bossData.DefeatsToProcess.Count; i++)
		{
			if (!m_BossHpUpdateRunning)
			{
			...
			var teamMateId = bossData.DefeatsToProcess[i].Key;
	...
	{
		if (m_currentBossNode && !DIContainerLogic.EventSystemService.IsBossOnCooldown())
		{
			var componentInChildren = m_currentBossNode.GetComponentInChildren<BossAssetController>();
	...
	{
		var index = Random.Range(0, m_BossNodes.Count - 1);
		for (var i = 0; i < 20; i++)
		{
