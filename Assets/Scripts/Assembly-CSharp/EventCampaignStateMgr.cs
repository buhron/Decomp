...
public class EventCampaignStateMgr : BaseLocationStateManager
{
	...
	public bool SetupHotspotBattle(PlayerGameData playerGameData, HotspotGameData hotspot, List<BirdGameData> battleBirdList, BattleParticipantTableBalancingData addition, bool hardmode = false)
	{
		...
		var possibleFollowUpBattles = new List<string>();
	...
	private void Awake()
	{
		...
		if (DIContainerInfrastructure.GetCurrentPlayer().CurrentEventManagerGameData != null)
		{
			...
			{
				var gameObject = DIContainerInfrastructure.EventSystemStateManager.InstantiateEventObject("Map", m_MiniMapRoot);
			...
			var componentsInChildren = m_MiniMapRoot.GetComponentsInChildren<HotSpotWorldMapViewBase>(true);
		...
		var outOfBoundsValue = m_DragController.GetOutOfBoundsValue(m_CamRoot.transform.position);
		...
		for (var j = 0; j < Camera.allCameras.Length; j++)
		{
	...
	private IEnumerator TryPlayWorldMapMusic()
	{
		...
		var musicTitle = m_Model.BalancingData.MusicTitle;
		if (musicTitle != null)
		{
	...
	private void OnMenuLoaded()
	{
		...
		var @object = UnityEngine.Object.FindObjectOfType(typeof(EventCampaignMenuUI));
	...
	private IEnumerator UpdateAndStartGateTimers(float delay, float extraDelay = 0.3f)
	{
		...
		var unlocked = DIContainerInfrastructure.EventSystemStateManager.GetNumberOfCampaignGateUnlocks();
		var nextGateInLineFound = false;
		for (var index = 0; index < m_gates.Length; index++)
		{
			var anyGate = m_gates[index];
			yield return new WaitForSeconds(extraDelay);
			if (anyGate is HotSpotWorldMapViewTimedPathNode)
			{
				var gate = anyGate as HotSpotWorldMapViewTimedPathNode;
				var req = gate.Model.BalancingData.EnterRequirements.Where(r => r.RequirementType == RequirementType.HaveItem && r.NameId == "event_gate_unlock").FirstOrDefault();
				var requiredUnlockItems = (int)req.Value;
				var timerOver = requiredUnlockItems <= unlocked;
			...
			{
				var stargate = anyGate as HotSpotWorldMapViewCampaignStarPathNode;
