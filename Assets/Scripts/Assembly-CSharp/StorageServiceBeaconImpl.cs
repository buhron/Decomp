...
public class StorageServiceBeaconImpl : IRemoteStorageService
{
	private class RetrieveFriendsState
	{
		...
		{
			m_storage = new Storage(ContentLoader.Instance.m_BeaconConnectionMgr.Identity);
		...
		private void OnSuccess(string key, Dictionary<string, string> accountToValueDictionary)
		{
			...
			foreach (var key2 in accountToValueDictionary.Keys)
			{
		...
		{
			var list = m_requests.Dequeue();
			if (list != null)
			{
				var array = list.ToArray();
				DebugLog.Log(GetType(), "Requesting " + array.Length + " friends: " + array.Aggregate(string.Empty, (acc, curr) => acc + curr + ","));
		...
		if (SyncEnabled)
		{
			...
			}, delegate(string key, string localValue, string remoteValue)
			{
				try
				{
					...
					var result = m_stringSerializer.Serialize(resolver(playerData, m_stringSerializer.Deserialize<ABH.Shared.Models.PlayerData>(remoteValue)));
