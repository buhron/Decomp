	...
	public class OpenChestTutorialStep : BaseTutorialStep
	{
		...
		protected override void StartStep(string trigger, List<string> parameters)
		{
			...
			if (DIContainerInfrastructure.GetCurrentPlayer().WorldGameData.HotspotGameDatas.TryGetValue(list[0], out value) && value.Data.UnlockState >= HotspotUnlockState.ResolvedNew)
			{
				...
				var worldMapStateMgr = Object.FindObjectOfType(typeof(WorldMapStateMgr)) as WorldMapStateMgr;
				var hotspotWorldMapView = worldMapStateMgr.m_startingHotSpot.GetHotspotWorldMapView(value.BalancingData.NameId);
				var componentInChildren = hotspotWorldMapView.GetComponentInChildren<ChestPositioning>();
