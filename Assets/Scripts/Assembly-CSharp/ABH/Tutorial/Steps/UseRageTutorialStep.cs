	...
	public class UseRageTutorialStep : BaseTutorialStep
	{
		...
		protected override void StartStep(string trigger, List<string> parameters)
		{
			...
			for (var i = 0; i < array.Length; i++)
			{
				...
				if (characterControllerBattleGround && characterControllerBattleGround.GetModel().CombatantFaction == Faction.Birds && characterControllerBattleGround.GetModel().IsRageAvailiable && (string.IsNullOrEmpty(text) || text == characterControllerBattleGround.GetModel().CombatantNameId))
				{
