...
namespace ABH.Tutorial.Steps
{
	public class TapAndHoldTutorialStep : BaseTutorialStep
	{
		...
		protected override void StartStep(string trigger, List<string> parameters)
		{
			...
			var battleMgr = Object.FindObjectOfType(typeof(BattleMgr)) as BattleMgr;
			m_character = null;
			...
			for (var i = 0; i < array.Length; i++)
			{
				...
				if (characterControllerBattleGround && characterControllerBattleGround.GetModel().CombatantFaction == faction && (string.IsNullOrEmpty(text) || text == characterControllerBattleGround.GetModel().CombatantNameId))
				{
