...
namespace ABH.Tutorial.Steps
{
	public class BattleRuleTutorialStep : BaseTutorialStep
	{
		...
		protected override void StartStep(string trigger, List<string> parameters)
		{
			...
			var componentsInChildren = m_battleUI.m_RulesGrid.GetComponentsInChildren<RulesOverlayInvoker>(true);
			m_targetedRule = null;
		...
		private void AddHelpersAndBlockers()
		{
			if (m_battleUI)
			{
				...
				var componentsInChildren = m_battleUI.GetComponentsInChildren<BattlePrepCharacterButton>();
				for (var i = 0; i < componentsInChildren.Count(); i++)
				{
					var battlePrepCharacterButton = componentsInChildren[i];
