...
namespace ABH.Tutorial.Steps
{
	public class ClickCampPropTutorialStep : BaseTutorialStep
	{
		...
		protected override void StartStep(string trigger, List<string> parameters)
		{
			...
			var componentsInChildren = m_campStateMgr.GetComponentsInChildren<CampProp>();
			var array = componentsInChildren;
			foreach (var campProp in array)
			{
		...
		private void OnPropClicked(BasicItemGameData propUnlockItem)
		{
			if (m_campProp)
			{
				if (propUnlockItem.BalancingData.NameId != m_campProp.GetModel().BalancingData.NameId)
					return;
