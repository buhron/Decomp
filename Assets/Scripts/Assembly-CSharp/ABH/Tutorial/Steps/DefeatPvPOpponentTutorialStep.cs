...
namespace ABH.Tutorial.Steps
{
	public class DefeatPvPOpponentTutorialStep : BaseTutorialStep
	{
		...
		private void OnActionTaken(ICombatant source, ICombatant target)
		{
			...
			{
				var characterControllerBattleGround = m_characters[i] as CharacterControllerBattleGround;
				if (characterControllerBattleGround)
				{
		...
		protected override void StartStep(string trigger, List<string> parameters)
		{
			...
			m_target = null;
			m_source = null;
			var indexOfAllyToAttack = string.Empty;
			var targetName = string.Empty;
			if (m_possibleParams.Count > 0)
			{
				indexOfAllyToAttack = m_possibleParams[0];
			}
			if (m_possibleParams.Count > 1)
			{
				targetName = m_possibleParams[1];
			}
			var list = new List<CharacterControllerBattleGround>();
			for (var i = 0; i < m_characters.Length; i++)
			{
				var characterControllerBattleGround = m_characters[i] as CharacterControllerBattleGround;
				...
				if (characterControllerBattleGround && characterControllerBattleGround.GetModel().CombatantFaction == Faction.Pigs && (string.IsNullOrEmpty(targetName) || targetName == characterControllerBattleGround.GetModel().CombatantNameId))
				{
					m_target = characterControllerBattleGround;
				}
			}
			var allies = new List<CharacterControllerBattleGround>();
			for (var j = 0; j < m_characters.Length; j++)
			{
				var characterControllerBattleGround2 = m_characters[j] as CharacterControllerBattleGround;
				if (characterControllerBattleGround2 == null ||
				    characterControllerBattleGround2.GetModel().IsBanner ||
				    characterControllerBattleGround2.GetModel().CombatantFaction != Faction.Birds)
				{
					continue;
				}

				allies.Add(characterControllerBattleGround2);
			}
			if (int.Parse(indexOfAllyToAttack) >= allies.Count)
			{
				FinishStep("less_than_3_birds", new List<string>());
				return;
			}
			for (var k = 0; k < allies.Count; k++)
			{
				var source = allies[k];
				if (k.ToString() == indexOfAllyToAttack)
				{
					m_source = source;
					break;
				}
