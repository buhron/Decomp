...
namespace ABH.GameDatas
{
	public class EquipmentGameData : GameDataBase<EquipmentBalancingData, EquipmentData>, IInventoryItemGameData
	{
		...
		public string ItemLocalizedName
		{
			get
			{
				return GetFinalEquipmentName(BalancingData, Data.Level, Data.ItemSource, Data.IsAncient);
			}
		}
		...
		public static string GetFinalEquipmentName(EquipmentBalancingData balancing, int level, EquipmentSource source, bool isAncient)
		{
			var itemAssetOrLoca = GetItemAssetOrLoca(balancing, level, balancing.LocaBaseId);
			if (balancing.DirectAssetAndLoca)
			{
				return DIContainerInfrastructure.GetLocaService().GetEquipmentName(balancing.LocaBaseId, isAncient);
			}
			switch (source)
			{
			...
			case EquipmentSource.LootBird:
				...
				return DIContainerInfrastructure.GetLocaService().GetEquipmentName(itemAssetOrLoca, isAncient);
		...
		public static float GetItemMainStat(EquipmentGameData gameData, int enchantmentLevel = -1)
		{
			if (gameData == null)
				return 0f;
			...
			var num = 0;
			if (gameData.BalancingData.StatPerQualityPercent != null)
			{
				num = gameData.BalancingData.StatPerQualityPercent.Count <= gameData.Data.Quality
					? gameData.BalancingData.StatPerQualityPercent[gameData.BalancingData.StatPerQualityPercent.Count - 1]
					: gameData.BalancingData.StatPerQualityPercent[gameData.Data.Quality];
			}

			float num2 = num > 0 ? 1 : 0;
			var num3 = Mathf.Round(gameData.BalancingData.BaseStat + gameData.BalancingData.BaseStat * (gameData.BalancingData.StatPerLevel / 100f) * (float)(gameData.Data.Level - 1));
			var num4 = gameData.Data.Quality <= 0 ? 0f : num3 * ((float)num / (float)gameData.Data.Quality / 100f);
			var num5 = DIContainerLogic.EnchantmentLogic.GetBalancing(enchantmentLevel).StatsBoost / 100f;
			var num6 = num3 + gameData.BalancingData.StatPerQuality * (float)gameData.Data.Quality + Mathf.Max(Mathf.Round(num4 * (float)gameData.Data.Quality), num2 * (float)gameData.Data.Quality);
			if (gameData.Data.IsAncient)
			{
				num6 += num6 * (DIContainerBalancing.GameConstantsBalancingDataProvider.AncientEquipmentStatsBoost / 100f);
			}
		...
		{
			var assetBaseId = eqgd.ItemBalancing.AssetBaseId;
			if (Data.IsAncient)
			{
				assetBaseId += "_Ancient";
			}
		...
		private static string GetItemAssetOrLoca(EquipmentBalancingData eqbd, int level, string baseId)
		{
			...
			{
				var assetAndLocaBaseLevel = GetAssetAndLocaBaseLevel(eqbd, level);
		...
		{
			var num = level + eqbd.AssetLevelOffset;
			if (num <= 0)
			{
		...
		private string GetProjectileAssetId(EquipmentGameData eqgd)
		{
			...
			{
				var num = eqgd.ItemData.Level + eqgd.BalancingData.AssetLevelOffset;
				if (num <= 0)
				{
					num = 27 + num;
				}
			...
			{
				var num2 = eqgd.ItemData.Level + eqgd.BalancingData.AssetLevelOffset;
				if (num2 <= 0)
				{
					num2 = 24 + num2;
				}
		...
		public Dictionary<string, int> GetScrapLoot()
		{
			...
			var dictionary = new Dictionary<string, int>(Data.ScrapLoot);
			...
			foreach (var key4 in Data.ScrapLoot.Keys)
			{
