...
namespace ABH.GameDatas
{
	public class PvPSeasonManagerGameData
	{
		...
		public PvPSeasonState CurrentPvPSeasonState
		{
			...
			set
			{
				var currentSeasonState = Data.CurrentSeasonState;
		...
		public void SetCurrentSeasonTurn()
		{
			...
			var currentSeasonTurn = DIContainerLogic.PvPSeasonService.GetCurrentSeasonTurn(Balancing);
		...
		public Requirement GetRerollResultRequirement(int timesRerolled)
		{
			...
			var rerollRequirement = Balancing.RerollResultRequirement.Count < Data.CurrentLeague
				? Balancing.RerollResultRequirement.LastOrDefault()
				: Balancing.RerollResultRequirement[Data.CurrentLeague - 1];

			req.NameId = rerollRequirement.NameId;
			req.RequirementType = rerollRequirement.RequirementType;
			
			var amount = rerollRequirement.Value + timesRerolled * Balancing.RerollResultCostIncrease;
			req.Value = UnityEngine.Mathf.Min(amount, Balancing.RerollResultCostMax);
		...
		{
			var rankedPlayers = CurrentSeasonTurn.GetRankedPlayers(true);
