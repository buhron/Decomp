...
namespace ABH.GameDatas
{
	public class SocialEnvironmentGameData : GameDataBase<SocialEnvironmentBalancingData, SocialEnvironmentData>
	{
		...
		private void InitGameData(SocialEnvironmentData instance)
		{
			...
			{
				foreach (var acceptedFriendId in instance.AcceptedFriendIds)
				{
					var value = 0u;
					...
					Friends.Add(acceptedFriendId, new FriendGameData(acceptedFriendId).SetLastBirdRequest(value));
				}
				instance.PublicPlayerInstances = new List<PublicPlayerData>
				{
				...
				foreach (var publicPlayerInstance in instance.PublicPlayerInstances)
				{
		...
		{
			var socialEnvironmentData = new SocialEnvironmentData();
		...
		{
			foreach (var key in BirdPerFriend.Keys)
			{
				FriendGameData value = null;
				if (Friends.TryGetValue(key, out value))
				{
					var birdGameData = new BirdGameData(BirdPerFriend[key]);
		...
		public void RefreshFriends(List<FriendData> friendDatas)
		{
			...
			list = list.OrderByDescending(f => f.FriendLevel).ToList();
		...
		{
			var dictionary = new Dictionary<string, FriendGameData>(Friends);
			Friends.Clear();
			foreach (var key in dictionary.Keys)
			{
				var friendGameData = dictionary[key];
		...
		public void RefreshPublicPlayerDatas(Dictionary<string, PublicPlayerData> publicPlayerDictionary, List<string> friendsToRemove)
		{
			...
			foreach (var item in friendsToRemove)
			{
				if (Friends != null)
				{
					Friends.Remove(item);
			...
			foreach (var publicPlayerInstance in Data.PublicPlayerInstances)
			{
			...
			foreach (var key in publicPlayerDictionary.Keys)
			{
				var publicPlayerData = publicPlayerDictionary[key];
				var socialId = publicPlayerData.SocialId;
		...
		{
			var friendshipGateLockState = GetFriendshipGateLockState(friendshipGate);
			if (!friendshipGateLockState)
			{
				...
				{
					num = DIContainerInfrastructure.GetCurrentPlayer().SocialEnvironmentGameData.Data.FriendShipGateUnlocks[friendshipGate.BalancingData.NameId].Count(f => f.Contains("NPC_"));
				}
				dictionary.Add("NPCFriendsUsed", num.ToString("0"));
				DIContainerInfrastructure.GetAnalyticsSystem().LogEventWithParameters("FriendshipGateUnlocked", dictionary);
			}
		...
		{
			var mailboxMessageGameData = DIContainerLogic.SocialService.GenerateMessageGameData(message);
			if (MailboxMessages.Values.Any(m => m.IsEqual(message)))
			{
		...
		{
			if (!MailboxMessages.Remove(messageGameData.Id) || !Data.Messages.Remove(Data.Messages.FirstOrDefault(m => m.Id == messageGameData.Id)))
			{
				return false;
			}
			if (this.MessageRemoved != null)
		...
		{
			var mcCoolVisitMinCooldown = DIContainerBalancing.Service.GetBalancingData<SocialEnvironmentBalancingData>("default").McCoolVisitMinCooldown;
			var mcCoolVisitMaxCooldown = DIContainerBalancing.Service.GetBalancingData<SocialEnvironmentBalancingData>("default").McCoolVisitMaxCooldown;
			var presentTime = DIContainerLogic.GetTimingService().GetPresentTime();
			var nextVisitTimestamp = presentTime + new TimeSpan(0, 0, UnityEngine.Random.Range(mcCoolVisitMinCooldown, mcCoolVisitMaxCooldown));
			switch (type)
			{
			case MessageType.ResponseFriendshipEssenceMessage:
				DIContainerInfrastructure.GetCurrentPlayer().Data.SocialEnvironment.McCoolSendsEssenceTimestamp = DIContainerLogic.GetTimingService().GetTimestamp(nextVisitTimestamp);
				break;
			case MessageType.ResponseBirdBorrowMessage:
				DIContainerInfrastructure.GetCurrentPlayer().Data.SocialEnvironment.McCoolLendsBirdTimestamp = DIContainerLogic.GetTimingService().GetTimestamp(nextVisitTimestamp);
				break;
			case MessageType.ResponseGachaUseMessage:
				DIContainerInfrastructure.GetCurrentPlayer().Data.SocialEnvironment.McCoolVisitsGachaTimestamp = DIContainerLogic.GetTimingService().GetTimestamp(nextVisitTimestamp);
				break;
			case MessageType.ResponsePvpGachaUseMessage:
				DIContainerInfrastructure.GetCurrentPlayer().Data.SocialEnvironment.McCoolVisitsPvpGachaTimestamp = DIContainerLogic.GetTimingService().GetTimestamp(nextVisitTimestamp);
				break;
			default:
