	...
	public class ResponseBirdBorrowMessage : BaseMailboxMessage
	{
		...
		public override string ContentDescription
		{
			get
			{
				...
				{
					var bird = DIContainerInfrastructure.GetCurrentPlayer().GetBird(m_Message.Parameter1, true);
					if (bird != null)
					{
						dictionary.Add("{bird_name}", DIContainerInfrastructure.GetLocaService().GetCharacterName(bird.BalancingData.LocaId));
					}
				}
				var inventoryItemGameData = DIContainerLogic.GetLootOperationService().GetItemsFromLoot(Loot).FirstOrDefault();
				if (m_Message != null)
				{
		...
		public override string IconAssetId
		{
			get
			{
				var bird = DIContainerInfrastructure.GetCurrentPlayer().GetBird(m_Message.Parameter1);
				if (bird != null)
				{
