	...
	public class WonInPvpChallengeMessage : BaseMailboxMessage
	{
		...
		public override string ContentDescription
		{
			get
			{
				...
				var balancingData = DIContainerBalancing.GetInventoryItemBalancingDataPovider().GetBalancingData(Loot.FirstOrDefault().Key);
				...
				if (m_Message.Parameter2 <= 1)
				{
					...
					if (Sender.PublicPlayerData != null)
					{
						...
						{
							var array = Sender.PublicPlayerData.SocialPlayerName.Split(' ');
							value = array[0];
						}
					}
					if (string.IsNullOrEmpty(value))
		...
		{
			var parameter = message.m_Message.Parameter2;
