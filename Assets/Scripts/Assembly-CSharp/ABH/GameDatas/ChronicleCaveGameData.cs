...
namespace ABH.GameDatas
{
	public class ChronicleCaveGameData : GameDataBase<ChronicleCaveBalancingData, ChronicleCaveData>
	{
		...
		public ChronicleCaveGameData(ChronicleCaveData instance)
		{
			...
			{
				for (var i = 0; i < instance.CronicleCaveFloors.Count; i++)
				{
					var instance2 = instance.CronicleCaveFloors[i];
					var item = new ChronicleCaveFloorGameData(instance2);
		...
		{
			var chronicleCaveData = new ChronicleCaveData();
			ChronicleCaveFloorGameDatas = new List<ChronicleCaveFloorGameData>();
		...
		{
			var flag = ChronicleCaveFloorGameDatas.Count > floorIndex;
			if (flag)
			{
		...
		public ChronicleCaveFloorGameData GetFloorAndCreateIfNext(int floorIndex)
		{
			...
			var chronicleCaveFloorGameData = new ChronicleCaveFloorGameData((floorIndex % DIContainerBalancing.Service.GetBalancingDataList<ChronicleCaveFloorBalancingData>().Count).ToString("0"));
		...
		{
			var dictionary = new Dictionary<string, HotspotGameData>();
			foreach (var chronicleCaveFloorGameData in ChronicleCaveFloorGameDatas)
			{
				foreach (var hotspotGameData in chronicleCaveFloorGameData.HotspotGameDatas)
				{
