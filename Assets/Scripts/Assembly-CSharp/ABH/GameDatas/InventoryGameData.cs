	...
	public class InventoryGameData : GameDataBase<InventoryBalancingData, InventoryData>
	{
			...
			: base(instance)
		{
			...
			foreach (int value3 in Enum.GetValues(typeof(InventoryItemType)))
			{
				...
				{
					var value2 = new List<IInventoryItemGameData>();
		...
		{
			var list = new List<SkinItemGameData>();
		...
		public void AddNewItemToInventory(IInventoryItemGameData itemGameData, bool birdInventory = false)
		{
			...
			{
				var skinItemGameData = GetClassSkin(itemGameData.ItemBalancing.NameId, 0).FirstOrDefault();
			...
			var craftingRecipeGameData = (CraftingRecipeGameData)itemGameData;
		...
		{
			var list = Items[InventoryItemType.Skin];
		...
		private void GenerateGameDatasFromItemDatas(InventoryData instance, InventoryItemType itemcategory)
		{
			...
			case InventoryItemType.Class:
				...
				{
					foreach (var classItem in instance.ClassItems)
					{
			...
			case InventoryItemType.Consumable:
				...
				{
					foreach (var consumableItem in instance.ConsumableItems)
					{
			...
			case InventoryItemType.Ingredients:
				...
				{
					foreach (var craftingIngredientItem in instance.CraftingIngredientItems)
					{
			...
			case InventoryItemType.MainHandEquipment:
				...
				{
					foreach (var mainHandItem in instance.MainHandItems)
					{
			...
			case InventoryItemType.OffHandEquipment:
				...
				{
					foreach (var offHandItem in instance.OffHandItems)
					{
			...
			case InventoryItemType.CollectionComponent:
				...
				{
					foreach (var collectionComponent in instance.CollectionComponents)
					{
			...
			case InventoryItemType.CraftingRecipes:
				...
				{
					foreach (var craftingRecipesItem in instance.CraftingRecipesItems)
					{
						var craftingRecipeGameData = DIContainerLogic.InventoryService.ReinitNewInventoryItemGameData(this, craftingRecipesItem) as CraftingRecipeGameData;
			...
			case InventoryItemType.Resources:
				...
				{
					foreach (var craftingResourceItem in instance.CraftingResourceItems)
					{
			...
			case InventoryItemType.Trophy:
				...
				{
					foreach (var trophyItem in instance.TrophyItems)
					{
			...
			case InventoryItemType.Story:
				...
				{
					foreach (var storyItem in instance.StoryItems)
					{
			...
			case InventoryItemType.EventBossItem:
				...
				{
					foreach (var eventItem in instance.EventItems)
					{
			...
			case InventoryItemType.BannerEmblem:
				...
				{
					foreach (var bannerItem in instance.BannerItems)
					{
			...
			case InventoryItemType.Mastery:
				...
				{
					foreach (var masteryItem in instance.MasteryItems)
					{
			...
			case InventoryItemType.Skin:
				...
				{
					foreach (var skinItem in instance.SkinItems)
					{
		...
		public bool HasNewItemBird(InventoryItemType currentItemType, BirdGameData selectedBird)
		{
			try
			{
				return (currentItemType == InventoryItemType.Class && Items[InventoryItemType.Skin].FirstOrDefault(i => i.IsValidForBird(selectedBird) && i.ItemData.IsNew) != null) || Items[currentItemType].FirstOrDefault(i => i.IsValidForBird(selectedBird) && i.ItemData.IsNew) != null;
			}
			catch
			{
				return false;
		...
		{
			if (Items[currentItemType].FirstOrDefault(i => i.ItemData.IsNew) != null)
			{
				return true;
			}
			return false;
		...
		{
			var list = new List<IInventoryItemGameData>(Items[inventoryItemType]);
