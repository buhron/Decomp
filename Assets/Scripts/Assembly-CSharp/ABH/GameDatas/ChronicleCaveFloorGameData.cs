...
namespace ABH.GameDatas
{
	public class ChronicleCaveFloorGameData : GameDataBase<ChronicleCaveFloorBalancingData, ChronicleCaveFloorData>
	{
		...
		public ChronicleCaveFloorGameData(string nameId)
			: base(nameId)
		{
			foreach (var chronicleCaveHotspotId in BalancingData.ChronicleCaveHotspotIds)
			{
			...
			: base(instance)
		{
			...
			foreach (var hotSpotInstance in instance.HotSpotInstances)
			{
				var value = new HotspotGameData(DIContainerBalancing.Service.GetBalancingData<ChronicleCaveHotspotBalancingData>(hotSpotInstance.NameId), hotSpotInstance);
		...
		{
			var chronicleCaveFloorData = new ChronicleCaveFloorData();
