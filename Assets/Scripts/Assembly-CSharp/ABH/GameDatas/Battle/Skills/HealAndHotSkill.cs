...
namespace ABH.GameDatas.Battle.Skills
{
	public class HealAndHotSkill : SkillBattleDataBase
	{
		...
		public override IEnumerator DoAction(BattleGameData battle, ICombatant source, ICombatant target, bool shared = false, bool illusion = false)
		{
			...
			{
				var modifiedPercent = m_Percent;
				var referencedHealth = !(m_InvokersHealth >= 0f) ? skillTarget.ModifiedHealth : m_Source.ModifiedHealth;
				...
				if (Random.value <= m_HotChance / 100f)
				{
					...
					var valueList = new List<float> { m_HotPercent, m_InvokersHealth };
