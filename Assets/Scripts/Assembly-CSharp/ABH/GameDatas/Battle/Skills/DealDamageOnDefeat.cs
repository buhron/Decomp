...
{
	public class DealDamageOnDefeat : SkillBattleDataBase
	{
		...
		{
			base.Init(model);
			model.SkillParameters.TryGetValue("damage_in_percent", out m_DamageInPercent);
		}
		...
		public override void DoActionInstant(BattleGameData battle, ICombatant source, ICombatant target)
		{
			...
			var battleEffectGameData = new BattleEffectGameData(source, target, new List<BattleEffect>
			{
				new BattleEffect
				{
					...
					EffectType = BattleEffectType.DealFriendlyFireDamage,
		...
		public override string GetLocalizedDescription(ICombatant invoker)
		{
			...
			dictionary.Add("{value_1}", (m_DamageInPercent / 100f * invoker.ModifiedAttack).ToString("0"));
