...
{
	public class PassiveDispelAura : SkillBattleDataBase
	{
		private float m_DispelChance;

		public override void Init(SkillGameData model)
		{
			base.Init(model);
			model.SkillParameters.TryGetValue("chance_to_dispel", out m_DispelChance);
		}
		...
		public override void DoActionInstant(BattleGameData battle, ICombatant source, ICombatant target)
		{
			...
			foreach (var target2 in m_Targets)
			{
				...
				list.Add(m_DispelChance);
				var values = list;
				BattleEffectGameData battleEffectGameData = null;
				battleEffectGameData = target2.CombatantMainHandEquipment == null || target2.CombatantMainHandEquipment.BalancingData.Perk.Type != PerkType.Dispel ? new BattleEffectGameData(source, target2, new List<BattleEffect>
				{
					new BattleEffect
					{
						EffectTrigger = EffectTriggerType.OnDealDamage,
						EffectType = BattleEffectType.ChanceToDispel,
						Values = values,
				...
				}, base.Model.Balancing.EffectDuration, battle, base.Model.Balancing.AssetId, base.Model.Balancing.EffectType, GetLocalizedName(), base.Model.SkillNameId) : new BattleEffectGameData(source, target2, new List<BattleEffect>
				{
					new BattleEffect
					{
						...
						EffectType = BattleEffectType.ModifyDispel,
						Values = values,
						AfflicionType = base.Model.Balancing.EffectType,
						Duration = base.Model.Balancing.EffectDuration,
						EffectAssetId = base.Model.Balancing.EffectIconAssetId,
		...
		public override string GetLocalizedDescription(ICombatant invoker)
		{
			...
			dictionary.Add("{value_1}", m_DispelChance.ToString("0"));
