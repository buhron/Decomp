...
namespace ABH.GameDatas.Battle.Skills
{
	public class ProtectAndCounterSkill : SkillBattleDataBase
	{
		...
		public override IEnumerator DoAction(BattleGameData battle, ICombatant source, ICombatant target, bool shared = false, bool illusion = false)
		{
			...
			var valueList = new List<float> { m_Percent, m_CounterAttackDamage };
			var effectIdent = base.Model.Balancing.AssetId;
			var possibleHolders = battle.m_CombatantsPerFaction[Faction.Birds];
			for (var i = 0; i < possibleHolders.Count; i++)
			{
				foreach (var effects in possibleHolders[i].CurrrentEffects.Values)
				{
			...
			if (target != m_Source)
			{
				var effect = new BattleEffectGameData(target, source, new List<BattleEffect>
				{
					new BattleEffect
					{
				...
				effect.SetPersistanceAfterDefeat(false).AddEffect();
				var effect2 = new BattleEffectGameData(effectIdent: base.Model.Balancing.AssetId + "_counter", source: source, target: target, effects: new List<BattleEffect>
				{
					new BattleEffect
					{
						EffectTrigger = EffectTriggerType.AfterTargetSelection,
				...
				effect2.SetPersistanceAfterDefeat(false).LinkEffect(effect).AddEffect();
			}
			else
			{
				var effect3 = new BattleEffectGameData(effectIdent: base.Model.Balancing.AssetId + "_counter_only", source: source, target: target, effects: new List<BattleEffect>
				{
					new BattleEffect
					{
				...
				effect3.SetPersistanceAfterDefeat(false).AddEffect();
			}
		}
