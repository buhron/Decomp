...
namespace ABH.GameDatas.Battle.Skills
{
	public class SacrificeUndeadForHealthSkill : SkillBattleDataBase
	{
		...
		public override IEnumerator DoAction(BattleGameData battle, ICombatant source, ICombatant target, bool shared = false, bool illusion = false)
		{
			...
			var source2 = source;
			var toSacrifice = battle.m_CombatantsByInitiative.Where(c => c.KnockOutOnDefeat && c.CombatantFaction == source2.CombatantFaction).ToList();
			toSacrifice.AddRange(battle.m_CombatantsPerFaction[source.CombatantFaction].Where(c => c.IsKnockedOut));
			...
			var healFactor = 0f;
			var oneKnockedOut = false;
			var animationLength = 0f;
			foreach (var undead in toSacrifice)
			{
