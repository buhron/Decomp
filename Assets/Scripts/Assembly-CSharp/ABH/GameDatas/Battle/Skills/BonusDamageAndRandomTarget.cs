...
namespace ABH.GameDatas.Battle.Skills
{
	public class BonusDamageAndRandomTarget : AttackSkillTemplate
	{
		...
		public override IEnumerator DoAction(BattleGameData battle, ICombatant source, ICombatant target, bool shared = false, bool illusion = false)
		{
			...
			var source2 = source;
			var target2 = target;
			var possibleAdditionalTargets = battle.m_CombatantsByInitiative.Where(c => c.CombatantFaction != source2.CombatantFaction && target2 != c).ToList();
			...
			{
				var additionalTarget = possibleAdditionalTargets[UnityEngine.Random.Range(0, possibleAdditionalTargets.Count)];
				possibleAdditionalTargets.Remove(additionalTarget);
				var targets = new List<ICombatant> { additionalTarget };
				var modificationreceived2 = 1f;
				var modificationdealt2 = 1f;
				var attackValue2 = source.ModifiedAttack * (m_SprayDamage / 100f);
				var cachedValue = m_Source.CurrentSkillAttackValue;
				...
				{
					VisualEffectSetting setting = null;
					var gooDamageEffect = "GooBomb_Additional";
