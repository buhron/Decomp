	...
	public class AttackWithCollateralDamageAndStun : AttackSkillTemplate
	{
		...
		public override void Init(SkillGameData model)
		{
			...
			ActionsBeforeDamageDealt.Add(delegate(float damage, BattleGameData battle, ICombatant source, ICombatant target)
			{
				...
				var list = battle.m_CombatantsByInitiative.Where(c => c.CombatantFaction != source.CombatantFaction && target != c).ToList();
				if (list.Count > 0)
				{
					var num = 0;
					while (list.Count > 0 && (float)num < m_AdditionalEnemiesAffected)
					{
						...
						var list2 = new List<ICombatant> { combatant };
						var effectedParam = 1f;
						var effectedParam2 = 1f;
						var num2 = source.ModifiedAttack * m_CollateralDamageMod;
						var currentSkillAttackValue = m_Source.CurrentSkillAttackValue;
		...
		public override void ShareableActionPart(BattleGameData battle, ICombatant source, ICombatant target, bool isShared)
		{
			...
			for (var i = 0; i < m_AdditionalTargets.Count; i++)
			{
				if (m_ChanceToStunExtras > UnityEngine.Random.value * 100f)
				{
					var combatant = m_AdditionalTargets[i];
					Debug.Log("Stun extra target: " + combatant.CombatantNameId);
					var battleEffectGameData2 = new BattleEffectGameData(source, combatant, new List<BattleEffect>
					{
						new BattleEffect
						{
							EffectTrigger = EffectTriggerType.BeforeStartOfTurn,
