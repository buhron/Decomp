...
{
	public class PassiveRageReduce : SkillBattleDataBase
	{
		private float m_RageReduction;

		public override void Init(SkillGameData model)
		{
			base.Init(model);
			model.SkillParameters.TryGetValue("ragereduce", out m_RageReduction);
		}
		...
		public override void DoActionInstant(BattleGameData battle, ICombatant source, ICombatant target)
		{
			...
			var battleEffectGameData = new BattleEffectGameData(source, target, new List<BattleEffect>
			{
				new BattleEffect
				{
					...
					EffectType = BattleEffectType.ReduceRage,
		...
		public override string GetLocalizedDescription(ICombatant invoker)
		{
			...
			dictionary.Add("{value_1}", m_RageReduction.ToString("0"));
