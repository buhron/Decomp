...
{
	public class ReduceDamageReceivedWithEffectsSkill : SkillBattleDataBase
	{
		...
		{
			base.Init(model);
			base.Model.SkillParameters.TryGetValue("damage_reduce", out m_Percent);
		...
		public override IEnumerator DoAction(BattleGameData battle, ICombatant source, ICombatant target, bool shared = false, bool illusion = false)
		{
			...
			var battleEffectlist = new List<BattleEffect>();
			if (base.Model.SkillParameters.TryGetValue("damage_reduce", out effectValue))
			{
				var valueList2 = new List<float> { effectValue };
			...
			if (base.Model.SkillParameters.TryGetValue("stun_chance", out effectValue))
			{
				...
				var valueList = new List<float> { effectValue, stunDuration };
				battleEffectlist.Add(new BattleEffect
				{
					EffectTrigger = EffectTriggerType.OnReceiveDamage,
			...
			{
				var damage = target.ModifiedHealth / (100f / effectValue);
			...
			foreach (var skillTarget in m_Targets)
			{
				...
				var effect = new BattleEffectGameData(source, skillTarget, battleEffectlist, base.Model.Balancing.EffectDuration, battle, base.Model.Balancing.AssetId, base.Model.Balancing.EffectType, GetLocalizedName(), base.Model.SkillNameId);
				effect.AddEffect();
			}
		}
