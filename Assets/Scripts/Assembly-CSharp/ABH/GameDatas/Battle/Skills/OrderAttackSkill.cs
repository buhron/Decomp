	...
	public class OrderAttackSkill : SkillBattleDataBase
	{
		...
		public override IEnumerator DoAction(BattleGameData battle, ICombatant source, ICombatant target, bool shared = false, bool illusion = false)
		{
			...
			if (m_AllButSelf)
			{
				...
				m_Targets.AddRange(battle.m_CombatantsByInitiative.Where(c => c.CombatantFaction == source2.CombatantFaction && !c.IsStunned && c.IsParticipating && c != source2).ToList());
			}
			else if (!m_All)
			{
			...
			{
				m_Targets = new List<ICombatant>();
				m_Targets.AddRange(battle.m_CombatantsByInitiative.Where(c => c.CombatantFaction == source2.CombatantFaction && !c.IsStunned && c.IsParticipating).ToList());
			}
			...
			{
				combatant.CombatantView.m_AllowClick = false;
				combatant.CombatantView.m_AllowDrag = false;
				combatant.CombatantView.ActivateControlHUD(false);
			}
			while (!m_ImpactDone)
			{
			...
			foreach (var skillTarget in m_Targets)
			{
				...
				if (m_SpawnImage)
				{
					...
					case "bird_adventurer":
						if (m_copy != null)
						{
							var sc = m_copy.GetComponent<ScaleController>();
							if (sc != null)
							{
				...
				var previoustarget = skillTarget.AttackTarget;
				...
				skillTarget.CombatantView.targetSheltered = null;
				skillTarget.IsAttacking = false;
				if (m_ShareEffect)
				{
					var otherCombatants = battle.m_CombatantsByInitiative.Where(c => c.IsParticipating && c.CombatantFaction == skillTarget.AttackTarget.CombatantFaction && c != skillTarget.AttackTarget).ToList();
					if (otherCombatants.Count > 0)
					{
						foreach (var otherCombatant in otherCombatants)
						{
				...
				if (pig != null)
				{
					var chargeEffects = pig.CurrrentEffects.Values.Where(e => e.m_Effects.Any(ef => ef.EffectTrigger == EffectTriggerType.OnCharge)).ToList();
					if (pig is PigCombatant && chargeEffects.Count > 0)
					{
						pig.CombatantView.PlayAffectedAnimation();
						for (var i = chargeEffects.Count - 1; i >= 0; i--)
						{
							var effectGameData = chargeEffects[i];
							if (pig.ChargeDone)
							{
				...
				if (m_SpawnImage && m_copy)
				{
					...
					var motion = m_copy.gameObject.AddComponent<CHMotionTween>();
			...
			foreach (var combatant in m_Targets)
			{
				combatant.CombatantView.m_AllowClick = true;
				combatant.CombatantView.m_AllowDrag = true;
				combatant.CombatantView.ActivateControlHUD(true);
			}
		}
