...
namespace ABH.GameDatas.Battle.Skills
{
	public class XPMultiplierSkill : SkillBattleDataBase
	{
		...
		{
			base.Init(model);
			var keyValuePair = base.Model.SkillParameters.FirstOrDefault();
		...
		public override IEnumerator DoAction(BattleGameData battle, ICombatant source, ICombatant target, bool shared = false, bool illusion = false)
		{
			...
			m_Source = source;
			var bal = DIContainerBalancing.GetInventoryItemBalancingDataPovider().GetBalancingData(m_ItemName);
			var source2 = source;
			m_Targets = battle.m_CombatantsByInitiative.Where(c => c.CombatantFaction == source2.CombatantFaction).ToList();
