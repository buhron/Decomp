...
namespace ABH.GameDatas.Battle.Skills
{
	public class DamageFoesWithFriendsHP : AttackSkillTemplate
	{
		...
		public override IEnumerator DoAction(BattleGameData battle, ICombatant source, ICombatant target, bool shared = false, bool illusion = false)
		{
			...
			enemyList.AddRange(battle.m_CombatantsByInitiative.Where(c => c.CombatantFaction != target2.CombatantFaction).ToList());
			yield return new WaitForSeconds(source.CombatantView.PlaySupportAnimation());
			foreach (var skillTarget in m_Targets)
			{
				var bonusValue = m_Percent1 / 100f;
				var effectValue = m_Percent2;
				if (skillTarget.IsBanner)
				{
					bonusValue = m_BannerPercent1 / 100f;
					effectValue = m_BannerPercent2;
				}
				var damageToFriend = skillTarget.ModifiedHealth / (100f / effectValue);
				...
				foreach (var enemy in enemyList)
				{
					...
					var damageToEnemy2 = damageToFriend * bonusValue;
