...
namespace ABH.GameDatas.Battle.Skills
{
	public class FocusAttackSkill : SkillBattleDataBase
	{
		...
		public override IEnumerator DoAction(BattleGameData battle, ICombatant source, ICombatant target, bool shared = false, bool illusion = false)
		{
			...
			var tauntedCharacters = new List<ICombatant>();
			tauntedCharacters = new List<ICombatant>();
			var source2 = source;
			tauntedCharacters.AddRange(battle.m_CombatantsByInitiative.Where(c => c.CombatantFaction == source2.CombatantFaction && (base.Model.Balancing.TargetSelfPossible || c != source2)).ToList());
			...
			foreach (var taunted in tauntedCharacters)
			{
				taunted.CombatantView.PlayLaughAnimation();
				var valueList = new List<float> { 1f };
				var effect2 = new BattleEffectGameData(target, taunted, source, new List<BattleEffect>
				{
					new BattleEffect
					{
				...
				effect2.SetPersistanceAfterDefeat(false).AddEffect();
				var targetingBubble = taunted.CombatantView.m_SpeechBubbles.Values.FirstOrDefault();
			...
			{
				var effect = new BattleEffectGameData(source, target, source, new List<BattleEffect>
				{
					new BattleEffect
					{
						EffectTrigger = EffectTriggerType.OnTarget,
