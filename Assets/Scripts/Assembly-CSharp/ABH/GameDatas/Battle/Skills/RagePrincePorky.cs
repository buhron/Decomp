	...
	public class RagePrincePorky : AttackSkillTemplate
	{
		...
		public override void Init(SkillGameData model)
		{
			...
			ActionsAfterTargetSelection.Add(delegate(BattleGameData battle, ICombatant source, ICombatant target)
			{
				...
				if (m_All)
				{
					list2 = battle.m_CombatantsPerFaction[Faction.Pigs].Where(c => c.IsAlive || c.IsKnockedOut).ToList();
				}
				else
				{
					list2.Add(target);
			...
			{
				var num = 0f;
				foreach (var item in m_Targets.Where(t => t.IsKnockedOut))
				{
			...
			{
				var list = m_Targets.Where(c => c.IsKnockedOut).ToList();
