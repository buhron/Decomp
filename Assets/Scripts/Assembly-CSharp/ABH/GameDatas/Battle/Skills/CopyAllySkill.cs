...
namespace ABH.GameDatas.Battle.Skills
{
	public class CopyAllySkill : SkillBattleDataBase
	{
		...
		public override IEnumerator DoAction(BattleGameData battle, ICombatant source, ICombatant target, bool shared = false, bool illusion = false)
		{
			...
			var waveBalancing2 = DIContainerBalancing.Service.GetBalancingData<BattleParticipantTableBalancingData>("environmental_summon_table_21");
			var waveBalancing = new BattleParticipantTableBalancingData();
			...
			foreach (PigCombatant pig in summonedList)
			{
				...
				var newHealth = pig.ModifiedHealth / (100f / m_HitpointsPercentage);
				pig.SetHealthWithoutLogic(newHealth);
				foreach (var effectGameData in target.CurrrentEffects.Values)
				{
					if (effectGameData.m_EffectType == SkillEffectTypes.Curse && m_IncludeDebuffs == 1f)
					{
						var copyEffect = new BattleEffectGameData(effectGameData);
						copyEffect.m_Target = pig;
						copyEffect.AddEffect();
					}
					else if (effectGameData.m_EffectType == SkillEffectTypes.Blessing && m_IncludeBuffs == 1f)
					{
						var copyEffect2 = new BattleEffectGameData(effectGameData);
						copyEffect2.m_Target = pig;
						copyEffect2.AddEffect();
					}
				}
