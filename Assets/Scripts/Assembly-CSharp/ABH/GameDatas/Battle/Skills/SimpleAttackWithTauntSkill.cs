	...
	public class SimpleAttackWithTauntSkill : AttackSkillTemplate
	{
		...
		public override void ShareableActionPart(BattleGameData battle, ICombatant source, ICombatant target, bool isShared)
		{
			...
			if (m_TauntStrongest)
			{
				...
				list.AddRange(battle.m_CombatantsByInitiative.Where(c => c.CombatantFaction == tauntTarget.CombatantFaction).ToList());
			...
			{
				var list2 = new List<ICombatant>();
				list2.AddRange(battle.m_CombatantsByInitiative.Where(c => c.CombatantFaction == tauntingCharacter.CombatantFaction).ToList());
			...
			list3.Add(1f);
			var values = list3;
			var battleEffectGameData = new BattleEffectGameData(tauntingCharacter, tauntTarget, new List<BattleEffect>
			{
				new BattleEffect
				{
			...
			var characterSpeechBubble = tauntTarget.CombatantView.m_SpeechBubbles.Values.FirstOrDefault();
			...
			foreach (var item3 in battle.m_CombatantsByInitiative.Where(c => c.CombatantFaction == Faction.Birds))
			{
