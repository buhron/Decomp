	...
	public class HealAndCleanseSkill : SkillBattleDataBase
	{
		...
		public override IEnumerator DoAction(BattleGameData battle, ICombatant source, ICombatant target, bool shared = false, bool illusion = false)
		{
			...
			else
			{
				...
				var target2 = target;
				var source2 = source;
				m_Targets.AddRange(battle.m_CombatantsByInitiative.Where(c => c.CombatantFaction == target2.CombatantFaction && c != source2).ToList());
			}
		...
		private void Heal(ICombatant skillTarget, BattleGameData battle, ICombatant source, ICombatant target)
		{
			if (m_Percent != 0f)
			{
				...
				var num2 = m_InvokersHealth ? m_Source.ModifiedHealth : !m_FirsTargetHealth ? skillTarget.ModifiedHealth : target.ModifiedHealth;
			...
			{
				var num3 = DIContainerLogic.GetBattleService().RemoveBattleEffects(skillTarget, SkillEffectTypes.Curse);
				if (num3 > 0)
				{
