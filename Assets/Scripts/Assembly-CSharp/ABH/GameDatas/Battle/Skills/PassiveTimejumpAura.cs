...
{
	public class PassiveTimejumpAura : SkillBattleDataBase
	{
		private float m_TriggerChance;

		public override void Init(SkillGameData model)
		{
			base.Init(model);
			model.SkillParameters.TryGetValue("chance_to_trigger", out m_TriggerChance);
		}
		...
		public override void DoActionInstant(BattleGameData battle, ICombatant source, ICombatant target)
		{
			...
			foreach (var target2 in m_Targets)
			{
				...
				list.Add(m_TriggerChance);
				var values = list;
				var battleEffectGameData = new BattleEffectGameData(source, target2, new List<BattleEffect>
				{
					new BattleEffect
					{
						EffectTrigger = EffectTriggerType.AfterSkillUse,
						EffectType = BattleEffectType.AddBonusTurnPvP,
						Values = values,
		...
		public override string GetLocalizedDescription(ICombatant invoker)
		{
			...
			dictionary.Add("{value_1}", m_TriggerChance.ToString("0"));
