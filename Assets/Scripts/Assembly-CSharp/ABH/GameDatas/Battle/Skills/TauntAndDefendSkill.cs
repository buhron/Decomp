	...
	public class TauntAndDefendSkill : SkillBattleDataBase
	{
		...
		public override IEnumerator DoAction(BattleGameData battle, ICombatant source, ICombatant target, bool shared = false, bool illusion = false)
		{
			...
			else
			{
				...
				m_Targets.AddRange(battle.m_CombatantsByInitiative.Where(c => c.CombatantFaction == source2.CombatantFaction).ToList());
			}
			var effectValue = 0f;
			...
			foreach (var skillTarget2 in m_Targets)
			{
				...
				if (base.Model.SkillParameters.TryGetValue("percent", out effectValue))
				{
					...
					var effect = new BattleEffectGameData(source, skillTarget2, new List<BattleEffect>
					{
						new BattleEffect
						{
			...
			foreach (var skillTarget in m_Targets)
			{
				if (!skillTarget.IsBanner)
				{
					...
					var tauntEffect = new BattleEffectGameData(target, skillTarget, source, new List<BattleEffect>
					{
						new BattleEffect
						{
					...
					tauntEffect.SetPersistanceAfterDefeat(false).AddEffect();
					var targetingBubble = skillTarget.CombatantView.m_SpeechBubbles.Values.FirstOrDefault();
