...
namespace ABH.GameDatas.Battle.Skills
{
	public class AttackAndSelfDestruct : AttackSkillTemplate
	{
		...
		public override void Init(SkillGameData model)
		{
			...
			if (value == 1f)
			{
				m_AttackAnimation = c => c.CombatantView.PlaySupportAnimation();
			}
		}
		...
		public override IEnumerator DoAction(BattleGameData battle, ICombatant source, ICombatant target, bool shared = false, bool illusion = false)
		{
			...
			var modificationreceived = 1f;
			var modificationdealt = 1f;
			var attackValue = source.ModifiedAttack * (m_DamageInPercent / 100f);
			var possibleTargets = new List<ICombatant>();
			if (m_All)
			{
				var source2 = source;
				possibleTargets = battle.m_CombatantsByInitiative.Where(c => c.CombatantFaction != source2.CombatantFaction).ToList();
			}
			else
			{
			...
			{
				var skillTarget = possibleTargets[i];
				...
				var damage = attackValue * modificationreceived;
