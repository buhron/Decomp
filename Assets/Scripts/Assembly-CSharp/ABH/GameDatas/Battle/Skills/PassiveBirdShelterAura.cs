...
namespace ABH.GameDatas.Battle.Skills
{
	public class PassiveBirdShelterAura : SkillBattleDataBase
	{
		...
		public override void DoActionInstant(BattleGameData battle, ICombatant source, ICombatant target)
		{
			...
			foreach (var item in battle.m_CombatantsByInitiative.Where(c => c.CombatantFaction == target.CombatantFaction))
			{
			...
			{
				var source2 = item2;
				var battleEffectGameData = new BattleEffectGameData(source2, source, new List<BattleEffect>
				{
					new BattleEffect
					{
