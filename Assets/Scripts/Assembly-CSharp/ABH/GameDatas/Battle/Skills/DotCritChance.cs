...
{
	public class DotCritChance : SkillBattleDataBase
	{
		private float m_Chance = 100f;

		private float m_Bonus = 50f;
		
		public override void Init(SkillGameData model)
		{
			base.Init(model);
			model.SkillParameters.TryGetValue("chance", out m_Chance);
			model.SkillParameters.TryGetValue("bonus_percent", out m_Bonus);
		}
		...
		public override string GetLocalizedDescription(ICombatant invoker)
		{
			...
			dictionary.Add("{value_2}", m_Bonus.ToString());
