...
namespace ABH.GameDatas.Battle.Skills
{
	public class BirdShieldAfterCoinloss : SkillBattleDataBase
	{
		...
		public override void DoActionInstant(BattleGameData battle, ICombatant source, ICombatant target)
		{
			...
			var duration = source.CombatantFaction != Faction.Pigs ? (int)m_Duration : (int)m_Duration - 1;
			foreach (var target2 in m_Targets)
			{
				var battleEffectGameData = new BattleEffectGameData(m_Source, target2, new List<BattleEffect>
				{
					new BattleEffect
					{
