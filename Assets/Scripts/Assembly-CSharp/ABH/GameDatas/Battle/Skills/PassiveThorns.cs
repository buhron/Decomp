...
{
	public class PassiveThorns : SkillBattleDataBase
	{
		private float m_DamagePerLevel;

		public override void Init(SkillGameData model)
		{
			base.Init(model);
			model.SkillParameters.TryGetValue("damage_per_level_to_attacker", out m_DamagePerLevel);
		}
		...
		public override void DoActionInstant(BattleGameData battle, ICombatant source, ICombatant target)
		{
			...
			list.Add(m_DamagePerLevel * (float)source.CharacterModel.Level);
			var values = list;
		...
		public override string GetLocalizedDescription(ICombatant invoker)
		{
			...
			dictionary.Add("{value_1}", (m_DamagePerLevel * (float)invoker.CharacterModel.Level).ToString("0"));
