...
namespace ABH.GameDatas.Battle
{
	public class BattleGameData
	{
		...
		public bool AllDeadOfFaction(Faction faction)
		{
			...
			foreach (var item in m_CombatantsByInitiative)
			{
		...
		public void RegisterRageDecreaseByEnemy(float rageDecrease, ICombatant victim)
		{
			if (this.RageDecreasedByOpponent != null)
			{
				this.RageDecreasedByOpponent(rageDecrease, victim);
			}
		}
		...
		{
			if (IsPvP && source.CombatantFaction == Faction.Birds)
			{
