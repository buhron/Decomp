	...
	public abstract class BaseCombatant<T> : ICombatant where T : ICharacter
	{
		...
		public float CurrentHealth
		{
			...
			set
			{
				...
				RaiseHealthChanged(oldHealth, m_currentHealth);
				m_currentHealth = Math.Min(m_currentHealth, ModifiedHealth);
		...
		public bool IsStunned
		{
			get
			{
				return CurrrentEffects.Values.Any(e => e.m_Effects.Any(ei => ei.EffectType == BattleEffectType.Stun));
			}
		}
		...
		public float Depth
		{
			get
			{
				if (CombatantView && CombatantView.m_CachedTransform)
				{
		...
		protected SkillBattleDataBase GenerateSkillBattleData(SkillGameData data)
		{
			...
			var skillBattleDataBase = Activator.CreateInstance(Type.GetType("ABH.GameDatas.Battle.Skills." + data.Balancing.SkillTemplateType, true, true)) as SkillBattleDataBase;
		...
		public void ReceiveDamage(float damage, ICombatant attacker)
		{
			...
			var num = DIContainerLogic.GetBattleService().ApplyEffectsOfTypeOnTriggerType(damage, new List<BattleEffectType> { BattleEffectType.DoNotGetKilledByBird }, EffectTriggerType.OnReceiveDamageImmediately, this, attacker);
		...
		public void RemoveAppliedEffects(bool forceremoval)
		{
			if (CurrrentEffects != null)
			{
				...
				if (forceremoval)
					list = CurrrentEffects.Values.ToList();
				else
					list = CurrrentEffects.Values.Where(egd => egd.m_Effects.All(e => e.AfflicionType != SkillEffectTypes.Passive)).ToList();
		...
		public SkillBattleDataBase GetSkill(string name)
		{
			...
			return skills.FirstOrDefault(s => s.Model.SkillNameId == name);
		}
