...
namespace ABH.GameDatas.Battle
{
	public class BattleEffectGameData
	{
		...
		public BattleEffectGameData(BattleEffectGameData original, ICombatant source = null, ICombatant target = null)
		{
			...
			foreach (var effect in original.m_Effects)
			{
		...
		{
			if (m_Effects.Any(e => e.EffectType == BattleEffectType.Taunt) || !m_PersistIfDefeated)
			{
				RemoveEffect();
				return;
			}
		...
		{
			if (m_Target != null && (!m_Target.IsStunned || !m_Effects.Any(e => e.AfflicionType == SkillEffectTypes.None)))
			{
				if (!m_HasManualTurns)
				{
		...
		private void UpdateEffect(int turn)
		{
			...
			{
				var characterSpeechBubble = m_Target.CombatantView.m_SpeechBubbles[m_EffectIdent];
		...
		public bool AddEffect(bool withVisuals = true)
		{
			...
			{
				foreach (var key in m_Target.CurrrentEffects.Keys)
				{
		...
		public void RemoveEffect(bool onlyLogical = false, bool onlyVisual = false)
		{
			...
			if (m_Effects.Count(e => e.EffectType == BattleEffectType.Taunt) > 0)
			{
				m_Target.CombatantView.ResetBubbleTargetIcon();
				var characterControllerBattleGround = m_Target.CombatantView as CharacterControllerBattleGround;
			...
			{
				var gameObject = m_Target.CombatantView.LastingVisualEffects[m_EffectIdent];
				if (gameObject != null)
				{
					var t = gameObject.PlayAnimationOrAnimatorState("End");
		...
		public void EvaluateEffect(ICombatant target = null)
		{
			...
			{
				var gameObject = target.CombatantView.LastingVisualEffects[m_EffectIdent];
