	...
	public class BannerGameData : ICharacter
	{
		...
		public float BaseHealth
		{
			get
			{
				return CharacterHealth + 
				       (BannerCenter == null ? 0f : BannerItemGameData.GetItemMainStat(BannerCenter)) + 
				       (BannerEmblem == null ? 0f : BannerItemGameData.GetItemMainStat(BannerEmblem)) + 
				       (BannerTip == null ? 0f : BannerItemGameData.GetItemMainStat(BannerTip));
			}
		}
		...
		public BannerItemGameData BannerTip
		{
			get
			{
				...
				{
					var bannerItemGameData = InventoryGameData.Items[InventoryItemType.BannerTip].FirstOrDefault() as BannerItemGameData;
		...
		public BannerItemGameData BannerCenter
		{
			get
			{
				...
				{
					var bannerItemGameData = InventoryGameData.Items[InventoryItemType.Banner].FirstOrDefault() as BannerItemGameData;
		...
		public BannerItemGameData BannerEmblem
		{
			get
			{
				...
				{
					var bannerItemGameData = InventoryGameData.Items[InventoryItemType.BannerEmblem].FirstOrDefault() as BannerItemGameData;
		...
		public List<SkillGameData> Skills
		{
			get
			{
				if (m_Skills == null)
				{
					...
					{
						for (var i = 0; i < BannerTip.BalancingData.SkillNameIds.Count; i++)
						{
							var nameId = BannerTip.BalancingData.SkillNameIds[i];
		...
		private List<SkillGameData> BannerEmblemSkills
		{
			get
			{
				if (m_BannerEmblemSkills == null && BannerEmblem != null)
				{
					...
					{
						for (var i = 0; i < BannerEmblem.BalancingData.SkillNameIds.Count; i++)
						{
							var nameId = BannerEmblem.BalancingData.SkillNameIds[i];
		...
		private List<SkillGameData> BannerCenterSkills
		{
			get
			{
				if (m_BannerCenterSkills == null && BannerCenter != null)
				{
					...
					{
						for (var i = 0; i < BannerCenter.BalancingData.SkillNameIds.Count; i++)
						{
							var nameId = BannerCenter.BalancingData.SkillNameIds[i];
