...
namespace ABH.GameDatas
{
	public class BirdGameData : GameDataBase<BirdBalancingData, BirdData>, ICharacter
	{
		...
		public SkillGameData PassiveSkill
		{
			get
			{
				if ((m_PassiveSkill == null || m_ClassChanged) && ClassSkin != null && !string.IsNullOrEmpty(ClassSkin.BalancingData.PassiveSkillNameId))
				{
		...
		public float BaseAttack
		{
			get
			{
				var classIndependentAttack = ClassIndependentAttack;
		...
		public void RaiseClassItemChanged()
		{
			...
			{
				var item = player.InventoryGameData.Items[InventoryItemType.Skin].Where(s => s.ItemBalancing.NameId == player.Data.EquippedSkins[ClassItem.BalancingData.NameId]).FirstOrDefault() as SkinItemGameData;
		...
		{
			var flag = (from cb in DIContainerBalancing.Service.GetBalancingDataList<ClassSkinBalancingData>()
				where cb.OriginalClass == ClassItem.BalancingData.NameId && cb.ShowPreview
				select cb).Count() > 1;
			var flag2 = GetClassSkins().Length > 1;
			return flag || flag2;
		}

		public SkinItemGameData GetNextSkin()
		{
			var classSkins = GetClassSkins();
			if (classSkins.Length == 1)
			{
				return null;
			}
			var sortPriority = ClassSkin.BalancingData.SortPriority;
		...
		{
			var list = DIContainerInfrastructure.GetCurrentPlayer().InventoryGameData.Items[InventoryItemType.Skin].Where(i => i.IsValidForBird(this)).ToList();
			var list2 = new List<SkinItemGameData>();
			for (var j = 0; j < list.Count; j++)
			{
				var skinItemGameData = list[j] as SkinItemGameData;
			...
			var array = new SkinItemGameData[list2.Count];
			for (var k = 0; k < list2.Count; k++)
			{
				var skinItemGameData2 = list2[k];
