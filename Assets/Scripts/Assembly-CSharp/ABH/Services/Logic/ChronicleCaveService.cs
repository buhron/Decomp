...
namespace ABH.Services.Logic
{
	public class ChronicleCaveService
	{
		...
		public bool SetupHotspotBattle(PlayerGameData playerGameData, HotspotGameData hotspot, List<BirdGameData> battleBirdList, BattleParticipantTableBalancingData addition, ChronicleCaveFloorBalancingData floor)
		{
			...
			var dictionary = new Dictionary<Faction, Dictionary<string, float>>();
			...
			var firstPossibleBattle = DIContainerLogic.GetBattleService().GetFirstPossibleBattle(hotspot.BalancingData.BattleId, playerGameData, true);
			var backgroundAssetId = DIContainerBalancing.Service.GetBalancingData<ChronicleCaveBattleBalancingData>(firstPossibleBattle).BackgroundAssetId;
