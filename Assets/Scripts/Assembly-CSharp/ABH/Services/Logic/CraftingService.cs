...
namespace ABH.Services.Logic
{
	public class CraftingService
	{
		...
		public CraftingService SetResourceCostPerLevelBalancingData()
		{
			...
			var balancingDataList = DIContainerBalancing.Service.GetBalancingDataList<ResourceCostPerLevelBalancingData>();
			...
			foreach (var item in balancingDataList)
			{
				...
				var flag = false;
				for (var i = 0; i < m_resourceCostPerItemNameOrderedByLevel[item.AppliedItemNameId].Count; i++)
				{
		...
		{
			var key = recipe.ResultLoot.FirstOrDefault().Key;
		...
		public bool IsCraftAble(CraftingRecipeGameData recipe, InventoryGameData inventoryGameData, int amount = 1)
		{
			...
			var dictionary = m_lootService.GenerateLoot(recipe.GetResultLoot(), recipe.ItemData.Level);
		...
		public bool IsCraftingPossible(CraftingRecipeGameData recipe, InventoryGameData inventoryGameData, out List<IInventoryItemGameData> failedItems, int amount = 1)
		{
			...
			var result = true;
			failedItems = new List<IInventoryItemGameData>();
			for (var i = 0; i < craftingCosts.Count; i++)
			{
				var keyValuePair = craftingCosts[i];
		...
		{
			return DIContainerBalancing.GameConstantsBalancingDataProvider.RerollCraftingRequirement == null || (float)m_inventoryService.GetItemValue(inventoryGameData, DIContainerBalancing.GameConstantsBalancingDataProvider.RerollCraftingRequirement.NameId) >= DIContainerBalancing.GameConstantsBalancingDataProvider.RerollCraftingRequirement.Value;
		}

		public bool ExecuteRerollCost(int timesRerolled, InventoryGameData inventoryGameData, int amount = 1, bool isHardMode = false, bool isBossBattle = false)
		{
			Requirement requirement = null;
			requirement = amount <= 1 ? GetRerollRequirement(timesRerolled) : GetMultiRerollRequirement(timesRerolled);
			if (isHardMode)
			{
				var rerollCostMultiplicator = isBossBattle
				...
				return requirement == null || m_inventoryService.RemoveItem(inventoryGameData, requirement.NameId, (int)(rerollCostMultiplicator * requirement.Value), "crafting_reroll");
			}
		...
		public Requirement GetRerollRequirement(int timesRerolled)
		{
			var rerollCraftingReq = DIContainerBalancing.GameConstantsBalancingDataProvider.RerollCraftingRequirement;
			var req = new Requirement
			{
				NameId = rerollCraftingReq.NameId,
				RequirementType = rerollCraftingReq.RequirementType
			};
			var rerollIncrease = DIContainerBalancing.GameConstantsBalancingDataProvider.RerollCraftingCostIncrease;
			var amount = rerollCraftingReq.Value + timesRerolled * rerollIncrease;
			req.Value = UnityEngine.Mathf.Min(amount, DIContainerBalancing.GameConstantsBalancingDataProvider.RerollCraftingCostMax);
			
			return req;
		}

		public Requirement GetMultiRerollRequirement(int timesRerolled)
		{
			var rerollCraftingReq = DIContainerBalancing.GameConstantsBalancingDataProvider.RerollMultiCraftingRequirement;
			var req = new Requirement
			{
				NameId = rerollCraftingReq.NameId,
				RequirementType = rerollCraftingReq.RequirementType
			};
			var rerollIncrease = DIContainerBalancing.GameConstantsBalancingDataProvider.RerollMultiCraftingCostIncrease;
			var amount = rerollCraftingReq.Value + timesRerolled * rerollIncrease;
			req.Value = UnityEngine.Mathf.Min(amount, DIContainerBalancing.GameConstantsBalancingDataProvider.RerollMultiCraftingCostMax);
		...
		public List<IInventoryItemGameData> CraftItem(CraftingRecipeGameData recipe, InventoryGameData inventoryGameData, bool isAlchemy, int craftAmount = 1)
		{
			...
			var list = m_lootService.RewardLoot(inventoryGameData, num, dictionary, "crafting", EquipmentSource.Crafting, craftAmount);
			foreach (var item2 in list)
			{
				if (item2 is EquipmentGameData)
				{
					var equipmentGameData = item2 as EquipmentGameData;
					var data2 = equipmentGameData.Data;
					object firstCraftingBal;
					if (craftingCosts.Count >= 1)
					{
						var balancingData = DIContainerBalancing.GetInventoryItemBalancingDataPovider().GetBalancingData(craftingCosts[0].Key);
					...
					var firstCraftingCost = craftingCosts.Count >= 1 ? craftingCosts[0].Value : 0;
					object secondCraftingBal;
					if (craftingCosts.Count >= 2)
					{
						var balancingData = DIContainerBalancing.GetInventoryItemBalancingDataPovider().GetBalancingData(craftingCosts[1].Key);
					...
					var secondCraftingCost = craftingCosts.Count >= 2 ? craftingCosts[1].Value : 0;
					object thirdCraftingBal;
					if (craftingCosts.Count >= 3)
					{
						var balancingData = DIContainerBalancing.GetInventoryItemBalancingDataPovider().GetBalancingData(craftingCosts[2].Key);
					...
					data2.ScrapLoot = GenerateScrapLoot((IInventoryItemBalancingData)firstCraftingBal, firstCraftingCost, (IInventoryItemBalancingData)secondCraftingBal, secondCraftingCost, (IInventoryItemBalancingData)thirdCraftingBal, craftingCosts.Count >= 3 ? craftingCosts[2].Value : 0, DIContainerBalancing.GameConstantsBalancingDataProvider.CostToScrapLootRateCrafting, equipmentGameData.Data.ItemSource);
				}
			}
			...
			DIContainerInfrastructure.GetAnalyticsSystem().LogEventWithParameters(ABHAnalyticsEvents.ItemCrafted, dictionary2);
			return list;
		}
		...
		public int GetQuality(bool isAlchemy)
		{
			...
			{
				return QualityStrategy(DIContainerBalancing.GameConstantsBalancingDataProvider.StandardDiceWeights);
			}
			if (data.ItemData.Level == 2)
			{
				return QualityStrategy(DIContainerBalancing.GameConstantsBalancingDataProvider.GoldDiceWeights);
			}
			if (data.ItemData.Level == 3)
			{
				return QualityStrategy(DIContainerBalancing.GameConstantsBalancingDataProvider.CrystalDiceWeights);
			}
			return 0;
		}
		...
		public void RerollItemQuality(InventoryGameData owner, ref IInventoryItemGameData item, int craftingAmount = 1)
		{
			...
			var quality2 = GetQuality(item.ItemBalancing.ItemType == InventoryItemType.Consumable || item.ItemBalancing.ItemType == InventoryItemType.Ingredients);
			if (item.ItemBalancing.ItemType == InventoryItemType.Resources || item.ItemBalancing.ItemType == InventoryItemType.Consumable || item.ItemBalancing.ItemType == InventoryItemType.Ingredients)
			{
				...
				var amountByQuality = GetAmountByQuality(item.ItemData.Quality);
			...
			DIContainerInfrastructure.GetAnalyticsSystem().LogEventWithParameters("CraftResultRerolled", dictionary);
		}
		...
		public Dictionary<string, int> GenerateScrapLoot(IInventoryItemBalancingData firstCraftingBal, int firstCraftingCost, IInventoryItemBalancingData secondCraftingBal, int secondCraftingCost, IInventoryItemBalancingData thirdCraftingBal, int thirdCraftingCost, float costToScrapLootRate, EquipmentSource source)
		{
			...
			if (source != EquipmentSource.SetItem)
			{
				...
				{
					var craftingItemBalancingData = firstCraftingBal as CraftingItemBalancingData;
				...
				{
					var craftingItemBalancingData2 = secondCraftingBal as CraftingItemBalancingData;
				...
				{
					var craftingItemBalancingData3 = thirdCraftingBal as CraftingItemBalancingData;
			...
			var num4 = (int)Math.Floor((float)num * costToScrapLootRate);
			...
			var num5 = (int)Math.Floor((float)num2 * costToScrapLootRate);
			if (num5 > 0)
			{
				dictionary.Add(key, num5);
			}
			var num6 = (int)Math.Floor((float)num3 * costToScrapLootRate);
			if (num6 > 0)
			{
		...
		public List<IInventoryItemGameData> ScrapEquipment(InventoryGameData owner, EquipmentGameData equipment)
		{
			...
			ABHAnalyticsHelper.AddPlayerStatusToTracking(dictionary);
			DIContainerInfrastructure.GetAnalyticsSystem().LogEventWithParameters(ABHAnalyticsEvents.ItemScrapped, dictionary);
			return list;
		}

		public bool IsScrapPossible(InventoryGameData owner, IInventoryItemGameData equipment)
		{
			return equipment != null && equipment is EquipmentGameData && owner.Items[equipment.ItemBalancing.ItemType].Where(e => (e as EquipmentGameData).BalancingData.RestrictedBirdId == (equipment as EquipmentGameData).BalancingData.RestrictedBirdId).ToList().Count > 1;
		}
		...
		public float GetScrapResultFactor(EquipmentSource source)
		{
			...
			case EquipmentSource.Gatcha:
				return DIContainerBalancing.GameConstantsBalancingDataProvider.CostToScrapLootRateGacha;
			case EquipmentSource.SetItem:
				return DIContainerBalancing.GameConstantsBalancingDataProvider.CostToScrapLootRateSet;
			default:
				return DIContainerBalancing.GameConstantsBalancingDataProvider.CostToScrapLootRateCrafting;
			}
		}
