...
namespace ABH.Services.Logic
{
	public class PvPSeasonService
	{
		...
		public bool IsPvpUnderConstruction(PlayerGameData currentPlayer = null)
		{
			if (currentPlayer == null)
				currentPlayer = DIContainerInfrastructure.GetCurrentPlayer();

			return !IsCurrentPvPTurnAvailable(currentPlayer) || currentPlayer.CurrentPvPSeasonGameData.CurrentPvPSeasonState == PvPSeasonState.Pending;
		}
		...
		public void TriggerSeasonEnd()
		{
			...
			var text = DIContainerInfrastructure.GetLocaService().Tr(currentPlayer.CurrentPvPSeasonGameData.Balancing.LocaBaseId + "_name");
			if (currentPlayer.Data.m_CachedSeasonName == text) 
				return;

			currentPlayer.Data.m_CachedPvpTrophyId = currentPlayer.CurrentPvPSeasonGameData.Balancing.TrophyId;
		...
		public EventManagerState FinishCurrentPvPTurn(PvPSeasonManagerGameData currentPvPManagerGameData)
		{
			...
			var currentPvPTurnManagerState = currentPvPManagerGameData.CurrentSeasonTurn.CurrentPvPTurnManagerState;
			...
			var newState = currentPvPTurnManagerState == EventManagerState.Finished ? CurrentGlobalEventState.FinishedEvent : CurrentGlobalEventState.NoEvent;
			...
			var num = currentPvPTurnManagerState != EventManagerState.FinishedWithoutPoints ? GetRankChangesOnPvPTurnFinish(currentPvPManagerGameData, false) : GetRankChangesOnPvPTurnFinish(currentPvPManagerGameData, true);
			var num2 = currentPvPManagerGameData.Data.CurrentLeague + num;
		...
		public bool ConfirmCurrentPvPTurn(PlayerGameData currentPlayer)
		{
			...
			{
				for (var i = 0; i < currentPlayer.CurrentPvPSeasonGameData.CurrentSeasonTurn.Data.CurrentOpponents.Count; i++)
				{
					var value = currentPlayer.CurrentPvPSeasonGameData.CurrentSeasonTurn.Data.CurrentOpponents[i];
		...
		{
			var num = Math.Max(0u, m_timingService.GetCurrentTimestamp() - pvPSeasonManagerBalancingDataData.SeasonStartTimeStamp);
			var num2 = (uint)Mathf.RoundToInt((pvPSeasonManagerBalancingDataData.SeasonEndTimeStamp - pvPSeasonManagerBalancingDataData.SeasonStartTimeStamp) / pvPSeasonManagerBalancingDataData.SeasonTurnAmount);
		...
		public int GetRankChangesOnPvPTurnFinish(PvPSeasonManagerGameData pvpManager, bool applyChanges)
		{
			...
			var rankChangeByRanking = GetRankChangeByRanking(pvpManager);
			var rankChangeByMissedSeasons = GetRankChangeByMissedSeasons(pvpManager);
			if (!applyChanges)
			{
		...
		public int GetRankChangeByRanking(PvPSeasonManagerGameData pvpManager)
		{
			...
			{
				num = PromotePlayer(pvpManager.CurrentSeasonTurn.GetCurrentRank) ? 1 : 0;
			}
			if (pvpManager.Data.CurrentLeague + num > pvpManager.Balancing.MaxLeague || pvpManager.Data.CurrentLeague + num <= 0)
			{
				return 0;
			}
			return num;
		...
		public void SubmitPvPTurnScore(PvPSeasonManagerGameData pvpManager, ScoreSourceType sourceType, int score, Action<RESTResultEnum> UiCallback = null)
		{
			var player = DIContainerInfrastructure.GetCurrentPlayer();
			var currentSeasonTurn = pvpManager.CurrentSeasonTurn;
			...
			var luckyCoins = DIContainerLogic.InventoryService.GetItemValue(player.InventoryGameData, "lucky_coin");
			DIContainerLogic.BackendService.AddPvpScore(
				pvpManager.Balancing.NameId,
				currentSeasonTurn.Data.CurrentSeason,
				(PvpLeague)pvpManager.Data.CurrentLeague,
				score,
				luckyCoins,
				player.Data.Level,
				sourceType,
				OnPvpScoreSubmittedSuccess,
				OnErrorsubmittingPvPTurnScore);
		}
		...
		{
			var offlineMatchmakingAttributes = GetOfflineMatchmakingAttributes(player, pvpTurnManagerId);
		...
		private Dictionary<string, object> GetOfflineMatchmakingAttributes(PlayerGameData player, string pvpTurnManagerId)
		{
			...
			var num2 = (int)(TimeZone.CurrentTimeZone.GetUtcOffset(DateTime.Now).TotalHours + 13.0);
			LogDebug("Timezone Indicator: " + num2);
			var num3 = (int)((DIContainerLogic.GetDeviceTimingService().GetPresentTime() - new DateTime(2014, 1, 1)).TotalDays + (double)player.Data.ActivityIndicator);
		...
		public float GetAverageEquipmentLevel(PlayerGameData player)
		{
			...
			foreach (var bird in player.Birds)
			{
		...
		internal void UpdatePvpLeaderboard(string leaderboardId, Dictionary<string, bool> lbPlayerIds, Action<RESTResultEnum> uiCallback = null)
		{
			...
			var currentSeasonTurn = currentPlayer.CurrentPvPSeasonGameData.CurrentSeasonTurn;
		...
		{
			this.DebugLog("GetLeaderboardScores: getting scores for season pvp season");
			...
			{
				this.OnSuccessfulGotPublicPlayers(player, publicPlayers);
			}, new Action<string>(this.OnErrorGetPublicPlayers));
			...
			{
				this.OnSuccessfullFetchedScore(player, scores);
				if (uiCallback != null)
				{
					uiCallback(RESTResultEnum.Success);
				}
			...
			{
				this.OnErrorFechingScores(errorCode);
				if (uiCallback != null)
				{
					uiCallback(RESTResultEnum.Fail);
				}
		...
		{
			this.LogError("Error getting profiles of other players: " + error);
		}

		private void OnSuccessfullFetchedScore(PlayerGameData player, Dictionary<string, int> scores)
		{
			this.LogDebug("successful got scores of other players");
			if (player.CurrentPvPSeasonGameData == null || player.CurrentPvPSeasonGameData.CurrentSeasonTurn == null)
			{
				return;
			}
			// var leaderboardResults = new LeaderboardResults(scores.Count);
			// foreach (var keyValuePair in scores)
			// {
			// 	Leaderboard.Score score = new Leaderboard.Score(string.Empty, keyValuePair.Key);
			// 	score.SetPoints((long)keyValuePair.Value);
			// 	Leaderboard.Result result = new Leaderboard.Result(0L, score);
			// 	leaderboardResults.Add(result);
			// }
			// player.CurrentPvPSeasonGameData.CurrentSeasonTurn.UpdateOpponentScores(leaderboardResults.ToList());
			player.RegisterGlobalPvPScoresUpdated();
			if (this.IsWaitingForConfirmation(player.CurrentPvPSeasonGameData) && !player.CurrentPvPSeasonGameData.CurrentSeasonTurn.IsResultValid)
			{
				this.LogDebug("PvP turn is over and valid to show result!");
				this.SetPvPResultValid(player, player.CurrentPvPSeasonGameData);
		...
		{
			var rerollRequirement = pvPSeasonManagerGame.GetRerollResultRequirement(1);
		...
		public bool ExecuteResultRerollCost(PvPSeasonManagerGameData pvPSeasonManagerGame, InventoryGameData inventoryGameData, int timesRerolled)
		{
			var rerollRequirement = pvPSeasonManagerGame.GetRerollResultRequirement(timesRerolled);
		...
		public void RerollPvPSeasonResultLoot(PvPSeasonManagerGameData seasonManagerGameData, PlayerGameData playerGameData)
		{
			...
			var seasonTurnLootTableWheel = seasonManagerGameData.GetSeasonTurnLootTableWheel();
			if (string.IsNullOrEmpty(seasonTurnLootTableWheel))
			{
				this.LogError("PvPRewardLootTablesPerLeague is missing!");
				return;
			}
			if (!DIContainerBalancing.LootTableBalancingDataPovider.TryGetBalancingData(seasonTurnLootTableWheel, out lootTableBalancingData))
			{
			...
			var cachedRolledResultWheelIndex = seasonManagerGameData.CurrentSeasonTurn.Data.CachedRolledResultWheelIndex;
			seasonManagerGameData.CurrentSeasonTurn.RolledResultLoot = DIContainerLogic.GetLootOperationService().GenerateLoot(new Dictionary<string, int> { { lootTableBalancingData.NameId, 1 } }, playerGameData.Data.Level, seasonManagerGameData.CurrentSeasonTurn.ResultStars, ref cachedRolledResultWheelIndex);
			seasonManagerGameData.CurrentSeasonTurn.Data.CachedRolledResultWheelIndex = cachedRolledResultWheelIndex;
		}
		...
		{
			var pvPSeasonManagerGameData = new PvPSeasonManagerGameData().CreateNewInstance(seasonManager.NameId);
		...
		{
			this.LogDebug("Offline Matchmake a pvp opponent: With used function " + pvpManagerGameData.Balancing.OfflineGetBattleFunction);
			var offlineMatchmakingAttributes = this.GetOfflineMatchmakingAttributes(player, pvpManagerGameData.CurrentSeasonTurn.Data.NameId);
			var stringBuilder = new StringBuilder("MatchmakeForSinglePvPOpponentOffline: matchmaking parameters sent to server are " + offlineMatchmakingAttributes);
			foreach (var keyValuePair in offlineMatchmakingAttributes)
			{
			...
			{
				this.OnSinglePvPOpponentMatchedOffline(player, result, ids);
			}, 1);
		}
		...
		private void OnErrorMatchmakingSingleOpponentOffline(PlayerGameData player, string error)
		{
			...
			if (IsCurrentPvPTurnAvailable(player))
			{
				var fallbackPvPOpponent = GetFallbackPvPOpponent(player);
				player.CurrentPvPSeasonGameData.CurrentSeasonTurn.CurrentPvPOpponent = fallbackPvPOpponent;
				player.CurrentPvPSeasonGameData.CurrentSeasonTurn.FallbackOpponentReason = PvpFallbackOpponentReason.MATCHMAKING_ERROR;
		...
		private void OnSuccessfullyMatchedSingleOpponentOffline(PlayerGameData player, string[] ids)
		{
			...
			if (list.Count <= 0)
			{
				...
				var fallbackPvPOpponent = GetFallbackPvPOpponent(player);
				player.CurrentPvPSeasonGameData.CurrentSeasonTurn.CurrentPvPOpponent = fallbackPvPOpponent;
				player.CurrentPvPSeasonGameData.CurrentSeasonTurn.FallbackOpponentReason = PvpFallbackOpponentReason.NO_OPPONENT_ID;
				player.CurrentPvPSeasonGameData.CurrentSeasonTurn.CurrentBirdIndices = fallbackPvPOpponent.PvPIndices;
				TrackPvpOpponentFound();
				ResetIndices(player);
			}
		...
		private void OnErrorGetPublicOpponent(PlayerGameData player, string error)
		{
			if (IsCurrentPvPTurnAvailable(player))
			{
				var fallbackPvPOpponent = GetFallbackPvPOpponent(player);
				player.CurrentPvPSeasonGameData.CurrentSeasonTurn.CurrentPvPOpponent = fallbackPvPOpponent;
				player.CurrentPvPSeasonGameData.CurrentSeasonTurn.FallbackOpponentReason = PvpFallbackOpponentReason.NO_PLAYER_PROFILE;
				player.CurrentPvPSeasonGameData.CurrentSeasonTurn.CurrentBirdIndices = fallbackPvPOpponent.PvPIndices;
				TrackPvpOpponentFound();
			}
		}
		...
		private void TrackPvpOpponentFound()
		{
			...
			{
				var flag = currentPvPSeasonGameData.CurrentSeasonTurn.FallbackOpponentReason.Contains("CHEAT");
				var flag2 = currentPvPSeasonGameData.CurrentSeasonTurn.FallbackOpponentReason != PvpFallbackOpponentReason.NONE;
				var dictionary = new Dictionary<string, string>();
		...
		private void OnSuccessfullGotPublicOpponent(PlayerGameData player, Dictionary<string, PublicPlayerData> publicPlayers)
		{
			...
			else
			{
				...
				{
					var fallbackPvPOpponent2 = GetFallbackPvPOpponent(player);
				...
				{
					player.CurrentPvPSeasonGameData.CurrentSeasonTurn.FallbackOpponentReason = PvpFallbackOpponentReason.NONE;
		...
		public bool IsCheating(PublicPlayerData opponent)
		{
			...
			{
				global::DebugLog.Error(base.GetType(), "IsCheating - opponent null or pvpIndices not set");
				return false;
			}
			var pvPIndices = opponent.PvPIndices;
			var currentPlayer = DIContainerInfrastructure.GetCurrentPlayer();
			var list = new List<BirdGameData>();
			for (var i = 0; i < pvPIndices.Count; i++)
			{
				var giantEnemyBird = new BirdGameData(opponent.Birds[pvPIndices[i]]);
				if (giantEnemyBird.ClassItem == null || !giantEnemyBird.ClassItem.IsValidForBird(giantEnemyBird))
				{
					var inventoryGameData = new InventoryGameData(opponent.Inventory);
					DIContainerLogic.InventoryService.EquipBirdWithItem(new List<IInventoryItemGameData> { inventoryGameData.Items[InventoryItemType.Class].FirstOrDefault(item => item.IsValidForBird(giantEnemyBird)) }, InventoryItemType.Class, giantEnemyBird.InventoryGameData);
				}
				...
				if (giantEnemyBird.Level > DIContainerLogic.PlayerOperationsService.GetPlayerMaxLevel())
				{
					...
					currentPlayer.CurrentPvPSeasonGameData.CurrentSeasonTurn.FallbackOpponentReason = PvpFallbackOpponentReason.CHEAT_LEVEL;
					return true;
				}
				if (giantEnemyBird.ClassItem.Data.Level > giantEnemyBird.ClassItem.MasteryMaxRank())
				{
					...
					currentPlayer.CurrentPvPSeasonGameData.CurrentSeasonTurn.FallbackOpponentReason = PvpFallbackOpponentReason.CHEAT_MASTERY;
					return true;
				}
				if (giantEnemyBird.MainHandItem.Data.Level > giantEnemyBird.Level + 2)
				{
					...
					currentPlayer.CurrentPvPSeasonGameData.CurrentSeasonTurn.FallbackOpponentReason = PvpFallbackOpponentReason.CHEAT_EQUIPMENT;
					return true;
				}
				...
				if (giantEnemyBird.MainHandItem.EnchantmentLevel > DIContainerLogic.EnchantmentLogic.GetMaxEnchantmentLevel(giantEnemyBird.MainHandItem))
				{
					global::DebugLog.Log("IsCheating - mainhand enchant = " + giantEnemyBird.MainHandItem.EnchantmentLevel);
					currentPlayer.CurrentPvPSeasonGameData.CurrentSeasonTurn.FallbackOpponentReason = PvpFallbackOpponentReason.CHEAT_ENCHANTMENT;
					return true;
				}
				if (giantEnemyBird.OffHandItem.EnchantmentLevel > DIContainerLogic.EnchantmentLogic.GetMaxEnchantmentLevel(giantEnemyBird.OffHandItem))
				{
					global::DebugLog.Log("IsCheating - offhand enchant = " + giantEnemyBird.OffHandItem.EnchantmentLevel);
					currentPlayer.CurrentPvPSeasonGameData.CurrentSeasonTurn.FallbackOpponentReason = PvpFallbackOpponentReason.CHEAT_ENCHANTMENT;
					return true;
				}
			}
			var ownTeamPowerLevel = DIContainerInfrastructure.GetPowerLevelCalculator().GetPvPTeamPowerLevel(DIContainerInfrastructure.GetCurrentPlayer().PublicPlayer, null);
			var opponentTeamPowerLevel = DIContainerInfrastructure.GetPowerLevelCalculator().GetPvPTeamPowerLevel(opponent, null) * 2;
			var plDiff = (float)Mathf.Abs(opponentTeamPowerLevel - ownTeamPowerLevel);
			var plDiffPercent = plDiff / ownTeamPowerLevel;
			if (plDiffPercent > DIContainerBalancing.GameConstantsBalancingDataProvider.PvpMaxPowerlevelDiff / 100f)
			{
				global::DebugLog.Log("IsCheating - plDiff = " + plDiff + " - that's more than " + DIContainerBalancing.GameConstantsBalancingDataProvider.PvpMaxPowerlevelDiff + "%");
				currentPlayer.CurrentPvPSeasonGameData.CurrentSeasonTurn.FallbackOpponentReason = PvpFallbackOpponentReason.POWERLEVEL_DIFFERENCE;
				return true;
			}
			return false;
		}
		...
		private void ResetIndices(PlayerGameData player)
		{
			...
			for (var i = 0; i < player.CurrentPvPSeasonGameData.CurrentSeasonTurn.CurrentPvPOpponent.Birds.Count; i++)
			{
				list.Add(i);
			}
			list.Shuffle();
		...
		public PublicPlayerData GetFallbackPvPOpponent(PlayerGameData player)
		{
			...
			var balancingData = DIContainerBalancing.Service.GetBalancingData<SocialEnvironmentBalancingData>("default");
			var pvPFallbackHard = balancingData.PvPFallbackMedium;
			var num = Mathf.RoundToInt((GetAverageEquipmentLevel(player) + (float)player.Data.Level) / 2f);
			var value = 0;
			var val = DIContainerBalancing.Service.GetBalancingDataList<ExperienceLevelBalancingData>().Count + 1;
			...
			var fallbackPlayer = DIContainerLogic.SocialService.GetFallbackPlayer(value2, num, (EquipmentState)value4, player.CurrentPvPSeasonGameData.Data.CurrentLeague);
		...
		private List<int> GetRandomBirdIndices()
		{
			...
			var num2 = UnityEngine.Random.Range(0, 5);
			if (num == num2)
			{
		...
		public TimeSpan GetDailyPvpRefreshTimeLeft(PlayerGameData player, PvPSeasonManagerGameData seasonManager)
		{
			...
			var dateTimeFromTimestamp = DIContainerLogic.GetServerOnlyTimingService().GetDateTimeFromTimestamp(seasonManager.CurrentSeasonTurn.Data.LastUsedPvpEnergy);
			var todayWithOffset = GetTodayWithOffset(trustedTime, new TimeSpan(0, seasonManager.Balancing.HourOfDayToRefreshEnergyAndObjectives, 0, 0));
			...
			var targetServerTime = todayWithOffset.AddDays(1.0);
		...
		public bool IsDailyPvpRefreshed(PlayerGameData player, PvPSeasonManagerGameData seasonManager)
		{
			...
			var dateTimeFromTimestamp = DIContainerLogic.GetServerOnlyTimingService().GetDateTimeFromTimestamp(seasonManager.CurrentSeasonTurn.Data.LastUsedPvpEnergy);
			var todayWithOffset = GetTodayWithOffset(trustedTime, new TimeSpan(0, seasonManager.Balancing.HourOfDayToRefreshEnergyAndObjectives, 0, 0));
			var dateTime = GetTodayWithOffset(dateTimeFromTimestamp, new TimeSpan(0, seasonManager.Balancing.HourOfDayToRefreshEnergyAndObjectives, 0, 0));
		...
		public bool CheckPvpUnderConstructionPopupShown()
		{
			var player = DIContainerInfrastructure.GetCurrentPlayer();
			return IsCurrentPvPTurnAvailable(player) && IsPvpUnderConstruction(player) && !player.CurrentPvPSeasonGameData.Data.UnderConstructionPopupShown;
		}

		public void SetPvpUnderConstructionPopupShown(bool wasShown, PlayerGameData player)
		{
			if (IsCurrentPvPTurnAvailable(player))
			{
				player.CurrentPvPSeasonGameData.Data.UnderConstructionPopupShown = wasShown;
			}
		}
		
		public ScoreSourceType MapPvPScoreReasonToScoreSourceType(Dictionary<string, string> addreason)
		{
			var stringBuilder = new StringBuilder(GetType().ToString());
			stringBuilder.Append("Mapping pvp addReason Dict:");
			...
			string type;
			if (addreason.TryGetValue("TypeOfUse", out type))
			{
				string objectiveDifficulty;
				if (addreason.TryGetValue("ObjectiveDifficulty", out objectiveDifficulty) && objectiveDifficulty != null)
				{
					if (objectiveDifficulty == "easy" || objectiveDifficulty == "not_used")
					{
						return ScoreSourceType.PvpObjectiveEasy;
					}
					if (objectiveDifficulty == "normal")
					{
						return ScoreSourceType.PvpObjectiveMedium;
					}
					if (objectiveDifficulty == "hard")
					{
						return ScoreSourceType.PvpObjectiveHard;
					}
				}
			}
			else if (addreason.TryGetValue("TypeOfGain", out type))
			{
				if (type == "Battle_Won" && addreason.ContainsKey("BattlerewardBonusMultiplier") && float.Parse(addreason["BattlerewardBonusMultiplier"]) > 1f)
				{
					return ScoreSourceType.PvpBattleBoosted;
				}
			}
			
			return ScoreSourceType.PvpBattle;
		}
		
		public int CountCurrentLootTablesPerRank(PvPSeasonManagerGameData seasonManagerGameData = null)
		{
			if (seasonManagerGameData == null)
				seasonManagerGameData = DIContainerInfrastructure.GetCurrentPlayer().CurrentPvPSeasonGameData;
			
			var startingPlayerLevel = seasonManagerGameData.CurrentSeasonTurn.Data.StartingPlayerLevel;
			...
			var pvpBonusLootTablePrefix = seasonManagerGameData.Balancing.PvPBonusLootTablesPerRank.FirstOrDefault().Replace("{rank}", string.Empty);
			var pvpBonusLootTablePostfix = "levels_" + levelRange.ToString("00");

			return DIContainerBalancing.LootTableBalancingDataPovider.GetBalancingDataList().Count(ltb =>
					ltb.NameId.StartsWith(pvpBonusLootTablePrefix) && 
					ltb.NameId.EndsWith(pvpBonusLootTablePostfix));
		}
		
		public PvPSeasonManagerBalancingData GetNextSeason()
		{
			var currentTimeStamp = DIContainerLogic.GetTimingService().GetCurrentTimestamp();
			foreach (var season in DIContainerBalancing.EventBalancingService.GetBalancingDataList<PvPSeasonManagerBalancingData>())
			{
				if (currentTimeStamp < season.SeasonStartTimeStamp)
				{
					return season;
				}
			}
		...
		public bool PromotePlayer(int rank)
		{
			return rank <= DIContainerBalancing.GameConstantsBalancingDataProvider.NumberOfPromotedPlayers;
		}
	}
}
