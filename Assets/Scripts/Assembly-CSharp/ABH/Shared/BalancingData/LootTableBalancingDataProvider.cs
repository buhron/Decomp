...
namespace ABH.Shared.BalancingData
{
	public class LootTableBalancingDataProvider
	{
		...
		{
			var list = new List<LootTableBalancingData>();
			foreach (var balancingData in DIContainerBalancing.Service.GetBalancingDataList<LootTableBalancingData>())
			{
		...
		{
			for (var i = 0; i < m_lootTableCacheList.Count; i++)
			{
				var lootTableBalancingData = m_lootTableCacheList[i];
