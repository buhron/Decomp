...
public class ContentLoader : MonoBehaviourContainerBase
{
	public class FakeProgress
	{
		...
		public void Update(float timeDelta)
		{
			...
			var num = Math.Min(1f, Math.Max(0f, m_timeMax - m_timeRemaining)) / m_timeMax;
	...
	private IEnumerator Start()
	{
		...
		var abhLocaService = DIContainerInfrastructure.GetStartupLocaService().InjectLocaAsset(m_startUpLocaLinks);
		ClientInfo.AddLoadingTracking("02_FirstStart");
		abhLocaService.InitDefaultLoca();
		...
		StartCoroutine(LeaveChimeraLogo(0f)); //m_ChimeraLogo.PlayAnimationOrAnimatorState("SplashScreen_Chimera") ));
		...
		#endif
			StartWithDeviceCompatibilityCheck(); // keep indentation for the else statement
	...
	private IEnumerator RequestStoragePermission()
	{
		...
		if (GetAPIVersion() >= 23 && (EtceteraAndroid.shouldShowRequestPermissionRationale("android.permission.WRITE_EXTERNAL_STORAGE") || PlayerPrefs.GetInt("PermissionRequested", 0) == 0) && !EtceteraAndroid.checkSelfPermission("android.permission.WRITE_EXTERNAL_STORAGE"))
		{
			...
			{
				var permissions = new string[2] { "android.permission.WRITE_EXTERNAL_STORAGE", "android.permission.READ_EXTERNAL_STORAGE" };
	...
	{
		OnInternetConnectivityReceived += HandleInternetConnectivityStatusForContentLoader;
	...
	private IEnumerator DoInitialInternetConnectivityCheckAndGetServerTime(Action continueAfterPreparation)
	{
		...
		var continueAfterPreparation2 = continueAfterPreparation;
	...
	{
		m_networkStatusService.InternetAvailabilityResponseReceived -= OnInternetAvailabilityResponseReceived;
		var num = new ABHTimingHandler().ProcessTimeFromSkynestTimeService(jsonTime);
		if (num != 0)
		{
	...
	private void HandleInternetConnectivityStatusForContentLoader(bool internetAvailable)
	{
		if (!internetAvailable)
		{
			if (m_reconnectButton)
			{
				...
				var componentInChildren = m_reconnectButton.GetComponentInChildren<UILabel>();
	...
	private void ContinueWithOBBCheck(Action doWhenObbLoadingFinished)
	{
		...
		var androidExpansionFileMgr = m_androidExpansionFileMgr;
		...
		var androidExpansionFileMgr2 = m_androidExpansionFileMgr;
	...
	private void DisplayRovioId()
	{
		var rovioId = DIContainerInfrastructure.GetPlayerPrefsService().GetString("rovioid");
		
		if (string.IsNullOrEmpty(rovioId))
			return;
		
		m_rovioIdLabel.gameObject.SetActive(true);
	...
	private void VerificationLogoutInputTriggerClicked()
	{
		...
		m_BeaconConnectionMgr.Identity.Login(Identity.LoginMethod.LoginGuest, SkynestIdentityOnLoggedIn, SkynestIdentityOnLoginError);
	}
	...
	private void ZeroTheGuestProfile()
	{
		...
		var assetData = DIContainerInfrastructure.GetAssetData();
    ...
    private void SkynestIdentityOnLoggedIn()
    {
        ...
        DIContainerInfrastructure.IdentityService.OnLoggedIn -= SkynestIdentityOnLoggedIn;
        if (m_verificationRoot)
        {
            UnityEngine.Object.Destroy(m_verificationRoot);
        }
        // never fetch profile because it causes a softlock since we are offline
        if (false) //if (DIContainerInfrastructure.GetAssetData().FetchAndResetProfileOnNextLogin)
        {
	...
	{
		var languageAssetProviderName = GetLanguageAssetProviderName();
		if (string.IsNullOrEmpty(languageAssetProviderName))
		{
			DoVersionCheckThenContinue();
			return;
		}
		var text = DIContainerInfrastructure.GetTargetBuildGroup() + "_" + languageAssetProviderName.ToLower() + ".assetbundle";
	...
	{
		var component = GetComponent<FontInitializer>();
		if (!component)
		{
			return string.Empty;
		}
		foreach (var fontSet in component.m_FontSets)
		{
			if (fontSet.m_Language == DIContainerInfrastructure.GetStartupLocaService().CurrentLanguageKey)
			{
				...
				{
					if (fontPair.m_FontReplacementAssetProvider)
					{
    ...
    private void OnProfileFetched(ABH.Shared.Models.PlayerData playerData)
    {
    	...
    	if (playerData != null)
    	{
    		...
    		DIContainerInfrastructure.GetProfileMgr().SaveProfile(playerData, 0);
    	}
	...
	private IEnumerator Hatch2_ContinueWithAssetLoading()
	{
		...
		yield return StartCoroutine(m_gdprManager.EvaluateTermsOfServiceAgreement());
		yield return StartCoroutine(m_gdprManager.CheckForErasurePending());
	...
	private void Hatch2_LoadAllAssets()
	{
		...
		DIContainerInfrastructure.GetAppConfigurationService(); // ok
	}
	...
	private void Hatch2_OnAssetLoadingError(string[] missingAssets, int errorCode)
	{
		...
		case -100:
			...
			if (m_allAssetsDownloadRetryCount <= MaxRetriesDownloadAssets)
			{
				Hatch2_LoadAllAssets();
				return;
			}
	...
	{
		SetDownloadProgressText(DIContainerInfrastructure.GetStartupLocaService().Tr("startup_syncing_version", "Synchronizing Version..."));
		var downloadHappening = false;
		var continueAction2 = continueAction;
	...
	private void ContinueWithLocaLoading()
	{
		...
		var text = DIContainerInfrastructure.GetTargetBuildGroup() + "_" + DIContainerInfrastructure.GetStartupLocaService().CurrentLanguageKey + ".bytes";
		if (!DIContainerInfrastructure.GetAssetsService().NeedToDownloadAsset(text))
		{
			DIContainerInfrastructure.GetStartupLocaService().Tr("startup_syncing_loca", "Syncing Loca...");
			ContinueWithAssetLoading();
			return;
		}
	...
	private void ContinueWithBattlegroundAssetLoading(AssetbundleIndex bundleIndex = AssetbundleIndex.A, int retryCount = 0)
	{
		...
		var text = DIContainerInfrastructure.GetTargetBuildGroup() + "_battlegroundassetprovider" + bundleIndex.ToString().ToLower();
		if (!DIContainerInfrastructure.GetAssetsService().NeedToDownloadAsset(text))
		{
	...
	private void ContinueWithAssetLoading(AssetbundleIndex prio = AssetbundleIndex.A, int retryCount = 0)
	{
		...
		if (retryCount >= MaxRetriesDownloadAssets)
		{
			DebugLog.Log(GetType(), string.Concat("ContinueWithAssetLoading: max retry count for prio ", prio, " of ", MaxRetriesDownloadAssets, " reached!"));
		...
		var audioAsset = DIContainerInfrastructure.GetTargetBuildGroup() + "_audioassetproviderpriority" + prio.ToString().ToLower() + ".assetbundle";
	...
	{
		var text = ".mp4";
		var text2 = DIContainerInfrastructure.GetTargetBuildGroup() + "_videoclipintro" + text;
	...
	{
		if (!showAsError && m_downloadTextUiLabel && !string.IsNullOrEmpty(localizedText))
		{
		...
		if (showAsError && m_ErrorText && !string.IsNullOrEmpty(localizedText))
		{
	...
	{
		DebugLog.Log(GetType(), "WaitAndGotoRootScene: waiting");
		...
		CancelInvoke("HasNetworkConnectionCoroutine");
		InvokeRepeating("HasNetworkConnectionCoroutine", m_alwaysOnCheckFrequency, m_alwaysOnCheckFrequency);
		var timePassed = Time.realtimeSinceStartup - m_startLoadTime;
		if (timePassed < 2f)
		{
		...
		if (DIContainerConfig.GetClientConfig().UseChimeraLeaderboards)
		{
			...
			// }); disable auth because decomp is offline
		}
		InitialTracking();
		m_rovioIdLabel.gameObject.SetActive(false);
	...
	private void InitialTracking()
	{
		...
		DIContainerInfrastructure.GetAnalyticsSystem().LogEventWithParameters("GameLoadInit", dictionary);
	}
	...
	public void SetDownloadProgress(float loadingProgress)
	{
		if (m_downloadProgressBar)
		{
			...
			if (m_loadingProgressPanel)
			{
				var num = Mathf.Max(m_downloadProgressBar.transform.localScale.x * m_downloadProgressBar.fillAmount, 1f);
				var x = num / 2f;
	...
	private IEnumerator ReloadEventBalancingAfterPlayerRestart()
	{
		...
		if (DIContainerInfrastructure.LocationStateMgr)
		{
