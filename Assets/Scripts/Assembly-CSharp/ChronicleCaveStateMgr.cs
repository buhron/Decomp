...
public class ChronicleCaveStateMgr : BaseLocationStateManager
{
	...
	private const int m_completeCavesForAchievement = 10;
	...
	private IEnumerator Start()
	{
		...
		for (var i = 1; i < m_Floors.Count; i++)
		{
			var lastFloor = m_Floors[i - 1];
			var floor = m_Floors[i];
			if (floor.m_EnteringHotSpot != null)
			{
				floor.m_EnteringNode.m_litePath = lastFloor.m_LeavingPath;
				if (lastFloor.m_LeavingNode)
				{
		...
		for (var j = 0; j < m_Floors.Count; j++)
		{
			var floor2 = m_Floors[j];
		...
		for (var index = 0; index < DIContainerInfrastructure.GetCurrentPlayer().Data.PendingFeatureUnlocks.Count; index++)
		{
			var pendingUnlocks = DIContainerInfrastructure.GetCurrentPlayer().Data.PendingFeatureUnlocks[index];
			...
			var introCutsene = m_Floors[Mathf.Max(0, DIContainerInfrastructure.GetCurrentPlayer().ChronicleCaveGameData.Data.CurrentBirdFloorIndex)].GetIntroCutscene();
			if (!introCutsene)
			{
		...
		if (m_Floors[Mathf.Max(0, DIContainerInfrastructure.GetCurrentPlayer().ChronicleCaveGameData.Data.CurrentBirdFloorIndex)])
		{
		...
		var playerRecord = DIContainerInfrastructure.GetCurrentPlayer().Data.AchievementTracking;
		if (!playerRecord.ChronicleCavesCompletedAchieved && DIContainerInfrastructure.GetCurrentPlayer().ChronicleCaveGameData.Data.CronicleCaveFloors.Count > m_completeCavesForAchievement)
		{
			var achievementId = DIContainerInfrastructure.GetAchievementService().GetAchievementIdForStoryItemIfExists("completeCaves");
	...
	{
		foreach (var value in DIContainerInfrastructure.GetCurrentPlayer().SocialEnvironmentGameData.PublicPlayerDatas.Values)
		{
	...
	private void InstatiateHotspotFriendProgress(PublicPlayerData publicPlayerData)
	{
		...
		var list = DIContainerBalancing.Service.GetBalancingDataList<ChronicleCaveFloorBalancingData>().ToList();
		ChronicleCaveFloorBalancingData chronicleCaveFloorBalancingData = null;
		int i;
		for (i = 0; i < list.Count; i++)
		{
			var flag = false;
			for (var j = 0; j < list[i].ChronicleCaveHotspotIds.Count; j++)
			{
				var nameId = list[i].ChronicleCaveHotspotIds[j];
		...
		{
			var friendProgressIndicator = UnityEngine.Object.Instantiate(m_FriendProgressIndicatorPrefab);
	...
	private void OnMenuWorldMapLoaded()
	{
		...
		var @object = UnityEngine.Object.FindObjectOfType(typeof(ChronicleCaveMenuUI));
	...
	private void LoadBirdsIntoScene()
	{
		if (DIContainerInfrastructure.GetCurrentPlayer().Birds != null)
		{
			for (var i = 0; i < DIContainerInfrastructure.GetCurrentPlayer().Birds.Count; i++)
			{
				...
				var characterControllerWorldMap = UnityEngine.Object.Instantiate(m_WorldMapCharacterController, m_CharacterRoot.position, m_CharacterRoot.rotation) as CharacterControllerWorldMap;
				characterControllerWorldMap.SetModel(birdGameData);
				var gameObject = new GameObject(birdGameData.BalancingData.AssetId);
	...
	public IEnumerator InitOnlyThree()
	{
		...
		var levelToLoad = m_CurrentCronicleCaveFloor.BalancingData.BackgroundId + "_" + m_CurrentCronicleCaveFloor.BalancingData.TemplateId.ToString("00");
		...
		levelToLoad = m_CurrentCronicleCaveFloor.BalancingData.BackgroundId + "_" + m_CurrentCronicleCaveFloor.BalancingData.TemplateId.ToString("00");
		yield return SceneManager.LoadSceneAsync(levelToLoad, LoadSceneMode.Additive);
		...
		var i = 0;
		m_CurrentCronicleCaveFloor = m_Model.GetFloorAndCreateIfNext(m_Model.CurrentFloorIndex);
		for (var index = 0; index < m_Model.ChronicleCaveFloorGameDatas.Count; index++)
		{
			var cave = m_Model.ChronicleCaveFloorGameDatas[index];
			DebugLog.Log("ChronicleCave #" + i + " Balancing#" + cave.BalancingData.NameId);
			foreach (var hotspot in cave.HotspotGameDatas.Values)
			{
	...
	public IEnumerator FloorUp()
	{
		...
		m_CurrentFloorIndex++;
		var newFloor = m_Model.GetFloorAndCreateIfNext(m_Model.CurrentFloorIndex + 1);
		if (newFloor == null)
		{
			m_Loading = false;
			yield break;
		...
		var floorToDestroy = m_LowerFloor;
		...
		var levelToLoad = newFloor.BalancingData.BackgroundId + "_" + newFloor.BalancingData.TemplateId.ToString("00");
		yield return SceneManager.LoadSceneAsync(levelToLoad, LoadSceneMode.Additive);
		yield return Resources.UnloadUnusedAssets();
		var timeDone = Time.realtimeSinceStartup - currentTime;
	...
	public IEnumerator FloorDown()
	{
		...
		var floorToDestroy = m_UpperFloor;
		...
		{
			m_CurrentCronicleCaveFloor = newFloor;
			var sceneNameToLoad = newFloor.BalancingData.BackgroundId + "_" + newFloor.BalancingData.TemplateId.ToString("00");
	...
	{
		var gate = m_Floors[m_Model.ChronicleCaveFloorGameDatas.Count - 1].GetGate();
		if (gate)
		{
			if (opened)
			{
	...
	public IEnumerator PlayGotoNextCaveAnimation()
	{
		...
		var nextCave = DIContainerInfrastructure.GetCurrentPlayer().ChronicleCaveGameData.Data.CronicleCaveFloors.Count;
		var playerRecord = DIContainerInfrastructure.GetCurrentPlayer().Data.AchievementTracking;
		if (!playerRecord.ChronicleCavesCompletedAchieved && nextCave > m_completeCavesForAchievement)
		{
			var achievementId = DIContainerInfrastructure.GetAchievementService().GetAchievementIdForStoryItemIfExists("completeCaves");
			if (!string.IsNullOrEmpty(achievementId))
			{
		...
		var tween = m_CamRoot.GetComponentInChildren<CHMotionTween>();
		...
		var targetFloor = m_Floors[nextCave - 1];
