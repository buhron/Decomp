...
public class WorldMapMenuButtonStates : MonoBehaviour
{
	...
	private void Awake()
	{
		...
		foreach (var allButton in m_allButtons)
		{
	...
	{
		if (!DIContainerLogic.RequirementService.CheckGenericRequirements(
			    DIContainerInfrastructure.GetCurrentPlayer(), 
			    new List<Requirement> { DIContainerBalancing.GameConstantsBalancingDataProvider.ShowCampButtonIndicatorsRequirement }))
		{
			return;
		}
		...
		foreach (var item in m_allButtons.Where(b => b.m_Priority > m_currentActivePrio))
		{
