...
public class UICamera : MonoBehaviour
{
	...
	public KeyCode cancelKey0 = KeyCode.Joystick8Button19; // Keycode.Escape
	...
	public static Vector2 lastEventPosition
	{
		get
		{
			var controlScheme = currentScheme;
			if (controlScheme == ControlScheme.Controller)
			{
				var gameObject = hoveredObject;
				if (gameObject != null)
				{
					var bounds = NGUIMath.CalculateAbsoluteWidgetBounds(gameObject.transform);
					var camera = NGUITools.FindCameraForLayer(gameObject.layer);
	...
	public static bool inputHasFocus
	{
		get
		{
			if (mInputFocus && mSelected && mSelected.activeInHierarchy)
			{
				return true;
			}
			return false;
	...
	public static bool isOverUI
	{
		get
		{
			...
			var i = 0;
			for (var count = activeTouches.Count; i < count; i++)
			{
				var mouseOrTouch = activeTouches[i];
	...
	public static GameObject hoveredObject
	{
		get
		{
			...
			if (mHover && mHover.activeInHierarchy)
			{
				return mHover;
			}
			mHover = null;
		...
		set
		{
			...
			if (mHover)
			{
				...
				if (flag)
				{
					var uICamera2 = !(mHover != null) ? list[0] : FindCameraForLayer(mHover.layer);
					if (uICamera2 != null)
					{
						current = uICamera2;
	...
	public static GameObject controllerNavigationObject
	{
		get
		{
			...
			if (currentScheme == ControlScheme.Controller && current != null && current.useController && !ignoreControllerInput && UIKeyNavigation.list.size > 0)
			{
				...
				{
					var uIKeyNavigation = UIKeyNavigation.list[i];
				...
				if (mHover == null)
				{
					for (var j = 0; j < UIKeyNavigation.list.size; j++)
					{
						var uIKeyNavigation2 = UIKeyNavigation.list[j];
	...
	public static GameObject selectedObject
	{
		get
		{
			if (mSelected && mSelected.activeInHierarchy)
			{
				return mSelected;
			}
			mSelected = null;
		...
		set
		{
			...
			if (value != null)
			{
				var component = value.GetComponent<UIKeyNavigation>();
				if (component != null)
				{
					controller.current = value;
				}
			...
			if (mSelected && flag)
			{
				var uICamera2 = !(mSelected != null) ? list[0] : FindCameraForLayer(mSelected.layer);
				if (uICamera2 != null)
				{
					current = uICamera2;
					currentCamera = uICamera2.cachedCamera;
	...
	static UICamera()
	{
		list = new BetterList<UICamera>();
		GetKeyDown = key => (key < KeyCode.JoystickButton0 || !ignoreControllerInput) && Input.GetKeyDown(key);
		GetKeyUp = key => (key < KeyCode.JoystickButton0 || !ignoreControllerInput) && Input.GetKeyUp(key);
		GetKey = key => (key < KeyCode.JoystickButton0 || !ignoreControllerInput) && Input.GetKey(key);
		GetAxis = axis => ignoreControllerInput ? 0f : Input.GetAxis(axis);
		GetMouse = button => mMouse[button];
		GetTouch = delegate(int id, bool createIfMissing)
		{
			if (id < 0)
			{
			...
			var j = 0;
			for (var count2 = mTouchIDs.Count; j < count2; j++)
			{
				if (mTouchIDs[j] == id)
				{
			...
			if (createIfMissing)
			{
				var mouseOrTouch = new MouseOrTouch
				{
		...
		RemoveTouch = delegate(int id)
		{
			var i = 0;
			for (var count = mTouchIDs.Count; i < count; i++)
			{
				if (mTouchIDs[i] == id)
				{
		...
		current = null;
		currentCamera = null;
		mLastScheme = ControlScheme.Mouse;
		currentTouchID = -100;
		mCurrentKey = KeyCode.Alpha0;
		currentTouch = null;
		mInputFocus = false;
		mMouse = new MouseOrTouch[3]
		{
			new MouseOrTouch(),
			new MouseOrTouch(),
			new MouseOrTouch()
		};
		controller = new MouseOrTouch();
		activeTouches = new List<MouseOrTouch>();
		...
		mTooltipTime = 0f;
		isDragging = false;
		mHit = default(DepthEntry);
		mHits = new BetterList<DepthEntry>();
	...
	public static bool IsPressed(GameObject go)
	{
		...
		var j = 0;
		for (var count = activeTouches.Count; j < count; j++)
		{
			var mouseOrTouch = activeTouches[j];
	...
	private static Rigidbody FindRootRigidbody(Transform trans)
	{
		...
		{
			var component = trans.GetComponent<Rigidbody>();
			if (component != null)
			{
				return component;
			}
	...
	private static Rigidbody2D FindRootRigidbody2D(Transform trans)
	{
		while (trans != null && !(trans.GetComponent<UIPanel>() != null))
		{
			var component = trans.GetComponent<Rigidbody2D>();
			if (component != null)
			{
				return component;
			}
	...
	public static bool Raycast(Vector3 inPos)
	{
		for (var i = 0; i < list.size; i++)
		{
			...
			var layerMask = currentCamera.cullingMask & (int)uICamera.eventReceiverMask;
			var enter = !(uICamera.rangeDistance > 0f) ? currentCamera.farClipPlane - currentCamera.nearClipPlane : uICamera.rangeDistance;
			if (uICamera.eventType == EventType.World_3D)
			{
				...
				{
					var rigidbody = FindRootRigidbody(mRayHitObject.transform);
					if (rigidbody != null)
					{
			...
			{
				var array = Physics.RaycastAll(ray, enter, layerMask);
				if (array.Length > 1)
				{
					for (var j = 0; j < array.Length; j++)
					{
						var gameObject = array[j].collider.gameObject;
						var component = gameObject.GetComponent<UIWidget>();
						if (component != null)
						{
						...
						{
							var uIRect = NGUITools.FindInParents<UIRect>(gameObject);
					...
					mHits.Sort((r1, r2) => r2.depth.CompareTo(r1.depth));
					for (var k = 0; k < mHits.size; k++)
					{
				...
				else
				{
					...
					var gameObject2 = array[0].collider.gameObject;
					var component2 = gameObject2.GetComponent<UIWidget>();
					if (component2 != null)
					{
					...
					{
						var uIRect2 = NGUITools.FindInParents<UIRect>(gameObject2);
			...
			else
			{
				if (uICamera.eventType == EventType.World_2D)
				{
					...
					var collider2D = Physics2D.OverlapPoint(point, layerMask);
					if (!collider2D)
					{
						continue;
					}
					...
					if (!uICamera.eventsGoToColliders)
					{
						var rigidbody2D = FindRootRigidbody2D(mRayHitObject.transform);
						if (rigidbody2D != null)
						{
				...
				var array2 = Physics2D.OverlapPointAll(lastWorldPosition, layerMask);
				if (array2.Length > 1)
				{
					for (var l = 0; l < array2.Length; l++)
					{
						var gameObject3 = array2[l].gameObject;
						var component3 = gameObject3.GetComponent<UIWidget>();
						if (component3 != null)
						{
						...
						{
							var uIRect3 = NGUITools.FindInParents<UIRect>(gameObject3);
				...
				else
				{
					...
					var gameObject4 = array2[0].gameObject;
					var component4 = gameObject4.GetComponent<UIWidget>();
					if (component4 != null)
					{
					...
					{
						var uIRect4 = NGUITools.FindInParents<UIRect>(gameObject4);
	...
	{
		var uIPanel = NGUITools.FindInParents<UIPanel>(go);
		while (uIPanel != null)
		{
	...
	{
		var uIPanel = NGUITools.FindInParents<UIPanel>(de.go);
		while (uIPanel != null)
		{
	...
	public static UICamera FindCameraForLayer(int layer)
	{
		...
		{
			var uICamera = list.buffer[i];
			var camera = uICamera.cachedCamera;
	...
	private void Start()
	{
		...
		if (Application.isPlaying)
		{
			if (fallThrough == null)
			{
				var uIRoot = NGUITools.FindInParents<UIRoot>(base.gameObject);
	...
	private void ProcessEvents()
	{
		...
		{
			var num = string.IsNullOrEmpty(scrollAxisName) ? 0f : GetAxis(scrollAxisName);
			if (num != 0f)
			{
				if (onScroll != null)
				{
	...
	public void ProcessMouse()
	{
		...
		var sqrMagnitude = currentTouch.delta.sqrMagnitude;
		...
		var flag4 = currentTouch.last != currentTouch.current;
		var flag5 = currentTouch.pressed != null;
		if (!flag5)
		{
		...
		for (var l = 0; l < 3; l++)
		{
			var mouseButtonDown = Input.GetMouseButtonDown(l);
			var mouseButtonUp = Input.GetMouseButtonUp(l);
	...
	{
		var num = GetInputTouchCount != null ? GetInputTouchCount() : Input.touchCount;
		for (var i = 0; i < num; i++)
		{
			TouchPhase phase;
			int fingerId;
			...
			var flag = phase == TouchPhase.Began || currentTouch.touchBegan;
			var flag2 = phase == TouchPhase.Canceled || phase == TouchPhase.Ended;
	...
	public void ProcessOthers()
	{
		...
		if (useController && !ignoreControllerInput)
		{
			...
			{
				var direction = GetDirection(verticalAxisName);
				if (direction != 0)
				{
					ShowTooltip(null);
			...
			{
				var direction2 = GetDirection(horizontalAxisName);
				if (direction2 != 0)
				{
					ShowTooltip(null);
					currentScheme = ControlScheme.Controller;
			...
			var num = string.IsNullOrEmpty(horizontalPanAxisName) ? 0f : GetAxis(horizontalPanAxisName);
			var num2 = string.IsNullOrEmpty(verticalPanAxisName) ? 0f : GetAxis(verticalPanAxisName);
			if (num != 0f || num2 != 0f)
			{
				ShowTooltip(null);
				currentScheme = ControlScheme.Controller;
		...
		{
			var i = 0;
			for (var num3 = NGUITools.keys.Length; i < num3; i++)
			{
				var keyCode2 = NGUITools.keys[i];
	...
	private void ProcessPress(bool pressed, float click, float drag)
	{
		if (pressed)
		{
			...
			{
				var component = currentTouch.pressed.GetComponent<UIKeyNavigation>();
				if (component != null)
				{
		...
		else
		{
			...
			var sqrMagnitude = currentTouch.totalDelta.sqrMagnitude;
			var flag = false;
	...
	private bool HasCollider(GameObject go)
	{
		...
		var component2 = go.GetComponent<Collider2D>();
	...
	public void ProcessTouch(bool pressed, bool released)
	{
		...
		var flag = currentScheme == ControlScheme.Mouse;
