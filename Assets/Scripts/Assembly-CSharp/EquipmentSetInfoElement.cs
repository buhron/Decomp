...
public class EquipmentSetInfoElement : MonoBehaviour
{
	...
	public void Init(IInventoryItemGameData item, EquipmentSetInfoElement partner, GachaSetItemInfoPopup parentWindow)
	{
		...
		{
			var equipmentBalancingData = item.ItemBalancing as EquipmentBalancingData;
			m_IsSecondary = equipmentBalancingData.ItemType == InventoryItemType.OffHandEquipment;
			flag = equipmentBalancingData.ShowAsNew;
			var gameObject = DIContainerInfrastructure.GetEquipmentAssetProvider().InstantiateObject(item.ItemAssetName, m_ItemSpawnRoot, Vector3.zero, Quaternion.identity, false);
		...
		else
		{
			...
			var bannerItemBalancingData = item.ItemBalancing as BannerItemBalancingData;
			m_IsSecondary = bannerItemBalancingData.ItemType == InventoryItemType.Banner;
			flag = bannerItemBalancingData.FlagAsNew;
			var gameObject2 = DIContainerInfrastructure.GetBannerAssetProvider().InstantiateObject(item.ItemAssetName, m_ItemSpawnRoot, Vector3.zero, Quaternion.identity);
	...
	{
		var equipmentBalancingData = m_Item.ItemBalancing as EquipmentBalancingData;
		var itemMainStat = EquipmentGameData.GetItemMainStat(m_Item as EquipmentGameData);
		var num = 0f;
		var bird = DIContainerInfrastructure.GetCurrentPlayer().GetBird(equipmentBalancingData.RestrictedBirdId);
		if (bird != null)
		{
		...
		var comparisionFooter = itemMainStat - num;
	...
	{
		var bannerItemBalancingData = m_Item.ItemBalancing as BannerItemBalancingData;
		var itemMainStat = BannerItemGameData.GetItemMainStat(m_Item as BannerItemGameData, m_Item.ItemData.Quality);
		var num = 0f;
		m_BaseStatType.spriteName = "Character_Health_Small";
