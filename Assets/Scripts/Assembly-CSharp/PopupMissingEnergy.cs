...
public class PopupMissingEnergy : MonoBehaviour
{
	...
	private void RefreshContent()
	{
		...
		var itemValue2 = DIContainerLogic.InventoryService.GetItemValue(DIContainerInfrastructure.GetCurrentPlayer().InventoryGameData, "potion_energy");
	...
	private void OpenShop()
	{
		...
		for (var i = DIContainerLogic.InventoryService.GetItemValue(DIContainerInfrastructure.GetCurrentPlayer().InventoryGameData, "event_energy"); m_energyRequirement > (float)i; i++)
		{
			ConsumePotion();
		}
		CloseAndRefresh();
	...
	private void ConsumePotion()
	{
		...
		{
			var consumableItemGameData = data as ConsumableItemGameData;
