...
public class GachaSetItemInfoPopup : MonoBehaviour
{
	...
	[SerializeField]
	private GameObject m_MythicEmblemChance;

	[SerializeField] 
	private UILabel[] m_ChanceLabels;
	
	[SerializeField]
	private GameObject m_SetChanceBonus;
	...
	private void OpenInfoScreen()
	{
		...
		var rainbowRiot1Multi = DIContainerBalancing.GameConstantsBalancingDataProvider.RainbowRiot1Multi;
		var rainbowRiot2Multi = DIContainerBalancing.GameConstantsBalancingDataProvider.RainbowRiot2Multi;
		
		m_MythicEmblemChance.SetActive(!m_isNormalGacha);
		SetupGachaChances();
		
		m_InfoPopupRainbowRiotText.text = DIContainerInfrastructure.GetLocaService()
			.Tr("set_info_desc_03")
			.Replace("{value_1}", rainbowRiot1Multi)
			.Replace("{value_2}", rainbowRiot2Multi);
		m_InfoScreenAnimation.gameObject.SetActive(true);
		m_InfoScreenAnimation.Play("Popup_Enter");
	}

	private void SetupGachaChances()
	{
		var localGachaLogic = new GachaLogic(!m_isNormalGacha);
		localGachaLogic.GetRainbowBarProgress(false);

		var advancedGachaValue = DIContainerLogic.InventoryService.GetItemValue(
			DIContainerInfrastructure.GetCurrentPlayer().InventoryGameData, 
			"story_goldenpig_advanced");
		
		m_SetChanceBonus.SetActive(advancedGachaValue > 0);

		for (var i = 0; i < m_ChanceLabels.Length; i++)
		{
			var chanceToWin = localGachaLogic.GetChanceToWin(i + 1);
			
			if (advancedGachaValue > 0 && i == 3)
				chanceToWin += -0.01f;

			var chancePercent = chanceToWin * 100;
			m_ChanceLabels[i].text = chancePercent.ToString("0") + "% " + DIContainerInfrastructure.GetLocaService().Tr("gacha_chance_basic");
		}
	}
	...
	public void Show(bool normalGacha, Animation gachaPopupUiAnimation)
	{
		...
		m_Header.text = DIContainerInfrastructure.GetLocaService().Tr("setpreview_header");
	...
	private IEnumerator EnterCoroutine()
	{
		...
		var componentsInChildren = GetComponentsInChildren<UIPanel>();
	...
	private void CreateBirdPreview()
	{
		var flag = false;
		var flag2 = false;
		var allBirds = DIContainerInfrastructure.GetCurrentPlayer().AllBirds;
		var list = new List<BirdGameData>();
		m_allOriginalBirds = new Dictionary<int, BirdGameData>();
		for (var i = 0; i < allBirds.Count; i++)
		{
			var birdGameData = allBirds[i];
	...
	{
		var bannerAsset = m_BannerEquipmentPreviewUI.m_CharacterController.m_AssetController as BannerAssetController;
		var root1 = bannerAsset.m_BannerFlagRoot;
		var root2 = bannerAsset.m_BannerTipRoot;
	...
	{
		var root1 = m_BirdEquipmentPreviewUI.m_CurrentCharacterController.m_AssetController.MainHandBone;
		var root2 = m_BirdEquipmentPreviewUI.m_CurrentCharacterController.m_AssetController.OffHandBone;
		yield return StartCoroutine(SelectNewButton(button, root1, root2));
	...
	public IEnumerator SetupItems(string charName)
	{
		...
		m_instantiatedFrameA.transform.localScale = Vector3.one;
		m_instantiatedFrameB.transform.localScale = Vector3.one;
		var anyNewFound = false;
		for (var i = 0; i < m_setItemsList[charName].Count; i++)
		{
			var setItem = m_setItemsList[charName].Values.ToList()[i];
			...
			var setItemId = Convert.ToInt32(Regex.Match(setItem.ItemBalancing.NameId, "\\d+").Value);
			...
			var panel1 = infoItem2.GetComponent<UIPanel>();
			var panel2 = correspondingItem2.GetComponent<UIPanel>();
			if (panel1 != null)
			{
			...
			var componentsInChildren = m_ItemGrid.GetComponentsInChildren<EquipmentSetInfoElement>();
	...
	private void FillItemList()
	{
		...
		if (m_isNormalGacha)
		{
			var list = DIContainerBalancing.Service.GetBalancingDataList<EquipmentBalancingData>()
				.Where(e => !string.IsNullOrEmpty(e.CorrespondingSetItemId))
				.ToList();
		...
		{
			var list2 = DIContainerBalancing.Service.GetBalancingDataList<BannerItemBalancingData>()
				.Where(e => !string.IsNullOrEmpty(e.CorrespondingSetItem) && e.ItemType != InventoryItemType.BannerEmblem)
				.ToList();
	...
	private Dictionary<string, IInventoryItemGameData> SortEquipment(Dictionary<string, IInventoryItemGameData> m_setItemsList)
	{
		...
		var dictionary = new Dictionary<string, IInventoryItemGameData>();
		foreach (var key in m_setItemsList.Keys)
		{
			var inventoryItemGameData = m_setItemsList[key];
			var showAsNew = ((EquipmentBalancingData)inventoryItemGameData.ItemBalancing).ShowAsNew;
			var equipmentBalancingData = inventoryItemGameData.ItemBalancing as EquipmentBalancingData;
			var i = Convert.ToInt32((float)DIContainerBalancing.Service.GetBalancingData<BirdBalancingData>(equipmentBalancingData.RestrictedBirdId).SortPriority * ((float)m_setItemsList.Count / 5f));
			if (showAsNew)
			{
				i--;
			}
	...
	private IInventoryItemGameData CreateItemFromLoot(string nameId)
	{
		...
		var level = DIContainerInfrastructure.GetCurrentPlayer().Data.Level + 2;
		var inventoryItemGameData = DIContainerLogic.GetLootOperationService().GetItemsFromLoot(DIContainerLogic.GetLootOperationService().GenerateLoot(dictionary, level), EquipmentSource.Gatcha).FirstOrDefault();
	...
	private void SetFooterItem()
	{
		...
		var skillBattleDataBase = (m_currentSelectedButton.m_Item as EquipmentGameData).SetItemSkill.GenerateSkillBattleData();
	...
	private void SetFooterBanner()
	{
		...
		m_ItemInfoAnimation.Play("ItemInfo_Change_Out");
		var skillBattleDataBase = (m_currentSelectedButton.m_Item as BannerItemGameData).SetItemSkill.GenerateSkillBattleData();
