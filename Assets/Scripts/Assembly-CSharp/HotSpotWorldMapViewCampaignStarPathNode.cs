...
public class HotSpotWorldMapViewCampaignStarPathNode : HotSpotWorldMapViewPathNode
{
	...
	private void Start()
	{
		if (base.Model != null)
		{
			...
			var requirement = base.Model.BalancingData.EnterRequirements.FirstOrDefault();
			var flag = DIContainerLogic.RequirementService.CheckGenericRequirements(DIContainerInfrastructure.GetCurrentPlayer(), base.Model.BalancingData.EnterRequirements);
			var itemValue = DIContainerLogic.InventoryService.GetItemValue(DIContainerInfrastructure.GetCurrentPlayer().InventoryGameData, requirement.NameId);
	...
	public override void ShowTooltip()
	{
		...
		{
			var localizedText = DIContainerInfrastructure.GetLocaService().Tr(m_overrideTooltip).Replace("{value_1}", GetNumberOfStarsRequired().ToString());
