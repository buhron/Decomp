...
public class ArenaBattlePreperationUI : MonoBehaviour
{
	...
	private void OnAdWatchedForBuffRoll()
	{
		...
		currentPlayer.Data.CurrentPvPBuff = DIContainerBalancing.GameConstantsBalancingDataProvider.SponsoredAdBuffName;
		m_SponsoredBuffButton.Clicked -= OnSponsoredBuffButtonClicked;
		...
		var pvPTeamPowerLevel = DIContainerInfrastructure.GetPowerLevelCalculator().GetPvPTeamPowerLevel(DIContainerInfrastructure.GetCurrentPlayer().PublicPlayer, DIContainerInfrastructure.GetCurrentPlayer().SelectedBirdIndices);
	...
	public void OnStartButtonClicked()
	{
		...
		if (DIContainerLogic.InventoryService.GetItemValue(DIContainerInfrastructure.GetCurrentPlayer().InventoryGameData, "pvp_energy") <= 0)
		{
			IInventoryItemGameData data = null;
	...
	public void Enter(ArenaCampStateMgr mapMgr, PublicPlayerData opponent, List<int> enemySelectedBirdsIndices, FriendGameData friend = null)
	{
		if (enemySelectedBirdsIndices == null || enemySelectedBirdsIndices.Count == 0)
		{
			...
			for (var i = 0; i < Mathf.Min(3, opponent.Birds.Count); i++)
			{
		...
		var currentPlayer = DIContainerInfrastructure.GetCurrentPlayer();
		var list = new List<BirdGameData>();
		for (var j = 0; j < m_EnemySelectedBirds.Count; j++)
		{
			var giantEnemyBird = new BirdGameData(m_opponent.Birds[m_EnemySelectedBirds[j]]);
		...
		battleStartGameData.m_BattleBalancingNameId = "battle_pvp_" + DIContainerInfrastructure.GetCurrentPlayer().CurrentPvPSeasonGameData.Data.CurrentLeague.ToString("00");
		...
		m_EnergyCharges.ForEach(energy => energy.gameObject.SetActive(true));
	...
	private IEnumerator EnterCoroutine()
	{
		...
		m_animator.SetBool("IsUnrankedBattle", false);
	...
	public IEnumerator LeaveCoroutine()
	{
		...
		m_Entered = false;
		yield return new WaitForSeconds(0.317f); // wait until hide animation finishes (idk how to get the anim time from the animator so i hardcoded the wait time)
	...
	private void FillInfos()
	{
		...
		CreateOpponentInfo();
		var eventPlayerName = player.SocialEnvironmentGameData.Data.EventPlayerName;
		...
		var list = player.SelectedBirdIndices.Where(i => player.Birds.Count >= i || player.Birds[i] == null).ToList();
		...
		{
			var num = m_MaxSelectableBirdsCount - player.SelectedBirdIndices.Count;
			for (var l = 0; l < player.Birds.Count; l++)
			{
				if (num <= 0)
				{
					break;
	...
	private void SetTopLoot()
	{
		...
		var num = balancing.LootTableEntries[0].BaseValue;
		...
		var mainItem = DIContainerLogic.InventoryService.GenerateNewInventoryItemGameData(DIContainerInfrastructure.GetCurrentPlayer().InventoryGameData, m_BattleLevel, 1, balancing.LootTableEntries[0].NameId, num, EquipmentSource.LootBird);
		m_MajorLoot.SetModel(mainItem, new List<IInventoryItemGameData>(), LootDisplayType.Major);
	}

	private IEnumerator UpdateEnergy()
	{
		var pvpEnergyCharges = DIContainerLogic.InventoryService.GetItemValue(DIContainerInfrastructure.GetCurrentPlayer().InventoryGameData, "pvp_energy");
		for (var i = 0; i < m_EnergyCharges.Length; i++)
		{
			m_EnergyCharges[i].spriteName = pvpEnergyCharges <= i ? "PvPCharge_Empty" : "PvPCharge_Full";
		}
		if (pvpEnergyCharges <= 0)
		{
			m_SponsoredBuffRoot.SetActive(false);
			var playAnimation = !m_PvpEnergyRoot.activeInHierarchy;
			var pvpEnergyOffer = DIContainerLogic.GetShopService().GetShopOffers(DIContainerInfrastructure.GetCurrentPlayer(), "shop_global_pvp_energy").FirstOrDefault();
			var cost = DIContainerLogic.GetShopService().GetBuyResourcesRequirements(1, pvpEnergyOffer, false).FirstOrDefault();
	...
	{
		var locaString = DIContainerInfrastructure.GetLocaService().Tr(m_OutOfEnergyText.GetComponent<LocaScript>().m_locaIdent, string.Empty);
	...
	private void CreateOpponentInfo()
	{
		...
		var num = 0;
		foreach (var pvPBird in ClientInfo.CurrentBattleStartGameData.m_PvPBirds)
		{
			foreach (Transform item in m_EnemyCharacterRoots[num])
			{
				UnityEngine.Object.Destroy(item.gameObject);
			}
			var characterControllerCamp = UnityEngine.Object.Instantiate(m_CharacterCampPrefab);
		...
		var pigBanner = ClientInfo.CurrentBattleStartGameData.m_PigBanner;
		var characterControllerCamp2 = UnityEngine.Object.Instantiate(m_CharacterCampPrefab);
	...
	private void CreateBirds()
	{
		...
		{
			m_birdSelectionFirst = true;
			for (var i = 0; i < currentPlayer.Birds.Count; i++)
			{
		...
		var bannerGameData = currentPlayer.BannerGameData;
		var characterControllerCamp = UnityEngine.Object.Instantiate(m_CharacterCampPrefab);
	...
	{
		var num = (float)(m_opponentPowerlevel - m_ownPowerLevel) / (float)m_ownPowerLevel;
		if (num < 0f)
		{
