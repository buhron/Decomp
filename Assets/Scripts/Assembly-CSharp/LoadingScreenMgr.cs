...
public class LoadingScreenMgr : MonoBehaviour
{
	...
	public float Show(LoadingArea target)
	{
		...
		base.gameObject.SetActive(true);
		m_Animation.Play("LoadingScreen_Enter");
		var list = new List<LoadingHintBalancingData>();
		var list2 = DIContainerBalancing.Service.GetBalancingDataList<LoadingHintBalancingData>().ToList();
		var num = 0f;
	...
	{
		var flag = true;
		foreach (var loadingBackground in m_loadingBackgrounds)
		{
			var flag2 = loadingBackground.m_Target == target;
	...
	public float Hide()
	{
		if (!notIgnoreFirstShow)
		{
			notIgnoreFirstShow = true;
			base.gameObject.SetActive(true);
			var num = m_Animation["LoadingScreen_CloseIris"].length + m_Animation["LoadingScreen_OpenIris"].length;
