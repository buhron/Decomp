...
public class SkinInfoPopup : MonoBehaviour
{
	...
	public void Show(ClassItemGameData birdClass, ClassManagerUi classManager)
	{
		...
		m_upgradeSkin = DIContainerBalancing.Service.GetBalancingDataList<ClassSkinBalancingData>().FirstOrDefault(sb => sb.OriginalClass == birdClass.BalancingData.NameId && sb.SortPriority == 1);
	...
	private void SpawnHeadgear(ClassSkinBalancingData UpgradeSkin)
	{
		...
		var gameObject = DIContainerInfrastructure.GetClassAssetProvider().InstantiateObject(UpgradeSkin.AssetBaseId, m_HeadgearSlot, Vector3.zero, Quaternion.identity, false);
		gameObject.transform.localPosition = Vector3.zero;
		gameObject.gameObject.layer = LayerMask.NameToLayer("Interface");
		gameObject.transform.localScale = Vector3.one;
		if (m_HeadgearSlotSorting)
		{
	...
	{
		return DIContainerBalancing.Service.GetBalancingDataList<BuyableShopOfferBalancingData>().Any(o => o.OfferContents != null && o.OfferContents.Keys.Contains(m_upgradeSkin.NameId) && o.Category != "none" && !DIContainerLogic.GetShopService().IsExclusiveOfferHidden(o));
	}

	private void GoToShop()
	{
		ClosePopup();
		var birdWindowUI = UnityEngine.Object.FindObjectOfType(typeof(BirdWindowUI)) as BirdWindowUI;
