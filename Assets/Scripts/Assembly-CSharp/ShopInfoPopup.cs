...
public class ShopInfoPopup : MonoBehaviour
{
	...
	{
		var shopOfferContent = DIContainerLogic.GetShopService().GetShopOfferContent(DIContainerInfrastructure.GetCurrentPlayer(), offer, DIContainerLogic.GetSalesManagerService().GetOfferSaleDetails(offer.NameId));
		...
		{
			var masteryItemGameData = shopOfferContent[0] as MasteryItemGameData;
	...
	private void SetItemListContent(List<IInventoryItemGameData> items)
	{
		...
		for (var i = 0; i < m_LootDisplays.Count; i++)
		{
			if (list.Count > i)
			{
	...
	private void SetupIcon(GameObject icon)
	{
		...
		var gameObject = Object.Instantiate(icon);
