...
public class GenericAssetProvider : MonoBehaviour
{
	...
	public IEnumerator InitializeCoroutine()
	{
		...
		m_initializing = true;
		var assetInfo = GetAssetInfoFor(m_AssetbundleFileName);
		if (assetInfo != null)
		{
			if (assetInfo.FileExistsCheck())
			{
				using (var www = WWW.LoadFromCacheOrDownload(assetInfo.GetFilePathWithPixedFileTripleSlashes(), assetInfo.AssetVersion))
				{
					while (!www.isDone)
					{
	...
	public List<string> GetAssetsToDeletePaths()
	{
		...
		{
			var editorAssetInfo = AssetInfos[i];
	...
	public void Initialize(bool clear = true)
	{
		...
		for (var i = 0; i < AssetInfos.Count; i++)
		{
			...
			var key = editorAssetInfo.NameId.ToLower();
			switch (editorAssetInfo.AssetLoadingType)
			{
			case LoadingType.FromResources:
			{
				var startIndex = editorAssetInfo.Path.LastIndexOf("/", StringComparison.Ordinal);
				var loadPath = editorAssetInfo.Path.Insert(startIndex, PathAddition);
	...
	public UnityEngine.Object GetObject(string nameId)
	{
		...
		case LoadingType.FromResources:
			...
			break;
		case LoadingType.FromBundle:
			value.Asset = this.Bundle.LoadAsset(key);
			break;
	...
	public void PreCacheObject(string nameId)
	{
		...
		{
			var gameObject = InstantiateObject(nameId, CachedInstancesRoot, Vector3.zero, Quaternion.identity, false);
	...
	{
		foreach (var item in UsedAssets.Where(i => i.Value.AssetLoadingType != LoadingType.FromMemory && i.Value.loaded && DIContainerLogic.GetTimingService().IsAfter(i.Value.LastRequestTime.AddSeconds(MinTimeForAssetUnload))))
		{
	...
	private void OnDestroy()
	{
		...
		foreach (var value in CachedObjects.Values)
		{
