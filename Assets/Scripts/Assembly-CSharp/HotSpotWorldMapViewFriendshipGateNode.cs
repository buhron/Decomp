...
public class HotSpotWorldMapViewFriendshipGateNode : HotSpotWorldMapViewBase
{
	...
	public override bool IsCompleted()
	{
		...
		foreach (var outgoingHotSpot in m_outgoingHotSpots)
		{
			...
			foreach (var outgoingHotSpot2 in outgoingHotSpot.m_outgoingHotSpots)
			{
	...
	public override void ShowContentView()
	{
		...
		if (base.Model.BalancingData.EnterRequirements != null && base.Model.BalancingData.EnterRequirements.Any(r => r.RequirementType == RequirementType.UsedFriends))
		{
		...
		{
			var inventoryItemGameData = DIContainerLogic.InventoryService.GenerateNewInventoryItemGameData(1, 1, requirement.NameId, (int)requirement.Value);
