...
public class CoreStateMgr : MonoBehaviourContainerBase
{
	...
	[HideInInspector]
	public BundleSalePopup m_BundleSalePopup;

	[HideInInspector]
	public ChainSalePopup m_ChainSalePopup;
	...
	[HideInInspector]
	public FeatureLockedPopup m_DungeonsLockedPopup;

	[HideInInspector]
	public FeatureLockedPopup m_ArenaUnderConstructionPopup;
	...
	[HideInInspector]
	public ChainChestRewardUI m_ChainChestRewardUi;

	[HideInInspector]
	public EliteChestInfoPopup m_EliteChestPreviewPopup;

	[HideInInspector]
	public SunsetModePopup m_SunsetModePopup;
	...
	public bool m_UseSwipeInteractionInBirdManager;
	...
	public int m_ZoomToDungeonId;

	public bool m_GoToDojo;

	public bool m_AllowSalesPopup;

	public CampStateMgr m_CampStateMgr;

	public ArenaCampStateMgr m_ArenaCampStateMgr;

	public DeveloperConsole m_Console;

	[SerializeField]
	private float m_friendRefreshCheckFrequency;

	[SerializeField]
	private List<GenericAssetProvider> m_LongLoadingAssetProviders;

	protected Dictionary<string, bool> m_LoadedLevels;
	...
	public SceneLoadingMgr SceneLoadingMgr
	{
		get
		{
			if (this.m_SceneLoadingMgr == null)
				this.m_SceneLoadingMgr = base.GetComponent<SceneLoadingMgr>();

			return m_SceneLoadingMgr;
		}
	}
	...
	public bool IsAnyPopupActive
	{
		get
		{
			if (DIContainerInfrastructure.LocationStateMgr && DIContainerInfrastructure.LocationStateMgr.FeatureUnlocksRunning)
			{
				return true;
			}
	...
	protected virtual void Awake()
	{
		...
		var array = UnityEngine.Object.FindObjectsOfType(typeof(AnimationManager));
	...
	{
		var useDebug = !DIContainerInfrastructure.GetCoreStateMgr().UseDebug;
		DIContainerInfrastructure.GetCoreStateMgr().UseDebug = useDebug;
		
		if (m_Console != null)
			m_Console.gameObject.SetActive(useDebug);
		
		var fpsDisplay = FindObjectOfType(typeof(FPSDisplay)) as FPSDisplay;
		if (fpsDisplay != null)
			fpsDisplay.enabled = useDebug;

		var timeDisplay = FindObjectOfType(typeof(TimeDisplay)) as TimeDisplay;
		if (timeDisplay != null)
	...
	public void BlockAllInput(bool block, bool ignoreBackButton = false)
	{
		...
		var component = m_InterfaceCamera.GetComponent<UICamera>();
		if (component)
		{
			component.enabled = !block;
		}
		var component2 = GetComponent<TouchInputHandler>();
		if (component2)
		{
	...
	protected virtual IEnumerator Start()
	{
		...
		var tutmgr = DIContainerInfrastructure.TutorialMgr;
		...
		StartCoroutine(DIContainerInfrastructure.GetCustomMessageService().Init(delegate { }, delegate { }, delegate { }));
	...
	{
		var notloadedAudioCount = m_LongLoadingAssetProviders.Count(e => !e.m_Initialized);
		...
		{
			yield return new WaitForEndOfFrame();
			notloadedAudioCount = m_LongLoadingAssetProviders.Count(e => !e.m_Initialized);
			if (ContentLoader.Instance != null)
			{
				var downloadProgress = (float)(m_LongLoadingAssetProviders.Count - notloadedAudioCount) / (float)m_LongLoadingAssetProviders.Count;
	...
	{
		DIContainerInfrastructure.InitCurrentPlayerIfNecessary();
	...
	private IEnumerator InitializeLocalization()
	{
		...
		var fakeProgress = new ContentLoader.FakeProgress(5f, ContentLoader.Instance.SetDownloadProgress);
	...
	public bool TryLoginOnFacebook()
	{
		if (!DIContainerInfrastructure.GetFacebookWrapper().IsUserAuthenticated())
		{
			...
			DIContainerInfrastructure.GetAnalyticsSystem().LogEventWithParameters("FacebookLoginTry", dictionary);
			return true;
		}
		return false;
	}
	...
	public virtual void GoToBattleResultWon(bool isPvP)
	{
		if (!SceneLoadingMgr.IsLoading())
		{
			...
			SceneLoadingMgr.AddUILevel(isPvP ? "Popup_BattleWon_PvP" : "Popup_BattleWon");
		}
	}
	...
	public virtual void GoToBattleResultLost(bool isPvP)
	{
		...
		{
			m_InfoOverlays.HideAllTooltips();
			SceneLoadingMgr.AddUILevel(isPvP ? "Popup_BattleLost_PvP" : "Popup_BattleLost");
		}
	}
	...
	public virtual void GotoWorldMap()
	{
		try
		{
			var list = DIContainerInfrastructure.GetCurrentPlayer().InventoryGameData.Items[InventoryItemType.Story].Where(i => i.ItemBalancing.NameId.StartsWith("comic_cutscene")).ToList();
			for (var j = 0; j < list.Count; j++)
			{
				var inventoryItemGameData = list[j];
	...
	public virtual void GotoFirendArenaScreen(PublicPlayerData player, FriendGameData friend)
	{
		...
		DIContainerInfrastructure.GetAnalyticsSystem().LogEventWithParameters("FriendInspect", dictionary);
	...
	public virtual ITutorialMgr InstantiateTutorialMgr()
	{
		...
		var tutorialMgr = UnityEngine.Object.Instantiate(m_TutorialMgr);
	...
	{
		var tutorialMgrNullImpl = UnityEngine.Object.Instantiate(m_TutorialNullMgr);
	...
	public bool ShowFeatureLockedPopup(FeatureLockedType type)
	{
		switch (type)
		{
			case FeatureLockedType.DungeonsLocked:
				if (m_DungeonsLockedPopup != null)
					m_DungeonsLockedPopup.Show();
				return false;
			case FeatureLockedType.ArenaUnderConstruction:
				if (m_ArenaUnderConstructionPopup == null || !DIContainerLogic.PvPSeasonService.CheckPvpUnderConstructionPopupShown()) 
					return false;
				
				DIContainerLogic.PvPSeasonService.SetPvpUnderConstructionPopupShown(true, DIContainerInfrastructure.GetCurrentPlayer());
				m_ArenaUnderConstructionPopup.Show();
				return true;
			default:
				return false;
		}
	...
	public void ShowShop(string category, Action reEnterAction, int startIndex = 0, bool arena = false, string enterSource = "Standard")
	{
		...
		DIContainerInfrastructure.GetAnalyticsSystem().LogEventWithParameter(ABHAnalyticsEvents.ShopEntered, "ShopCategory", category);
		if (IsShopOpen())
		{
		...
		{
			SetInitialShopCategory(category, reEnterAction, enterSource, startIndex);
		});
		DIContainerInfrastructure.GetAsynchStatusService().ShowLocalLoading("Loading..", false);
	}

	private void SetInitialShopCategory(string category, Action reEnterAction, string enterSource, int startIndex = 0)
	{
		...
		DIContainerInfrastructure.GetAsynchStatusService().HideLocalLoading();
		if ((m_LeaveShopAfterLoading || SceneLoadingMgr.IsLoading()) && m_ShopWindow != null)
		{
		...
		else
		{
			...
			m_ShopIsLoading = false;
			if (!DIContainerBalancing.GameConstantsBalancingDataProvider.ActivateShopSunset)
				return;
			DIContainerInfrastructure.GetCoreStateMgr().m_SunsetModePopup.Show(SunsetModePopup.SunsetModeTarget.Shop);
		}
	}
	...
	public void UnloadUnusedAssets()
	{
		...
		if (DIContainerInfrastructure.GetCharacterAssetProvider(true))
		{
			DIContainerInfrastructure.GetCharacterAssetProvider(true).RemoveAssetLinks();
		}
		if (DIContainerInfrastructure.GetCharacterAssetProvider(false))
		{
		...
		if (DIContainerInfrastructure.GetBattleEffectAssetProvider())
		{
			DIContainerInfrastructure.GetBattleEffectAssetProvider().RemoveAssetLinks();
		}
		if (DIContainerInfrastructure.GetGenericIconAtlasAssetProvider())
		{
		...
		if (DIContainerInfrastructure.ProjectileAssetProvider)
		{
			DIContainerInfrastructure.ProjectileAssetProvider.RemoveAssetLinks();
		}
		if (DIContainerInfrastructure.GetComicCutsceneAssetProvider())
		{
	...
	{
		yield return new WaitForEndOfFrame();
		while (!m_isInitialized || SceneLoadingMgr.IsLoading())
		{
			yield return new WaitForEndOfFrame();
		}
	...
	private IEnumerator ResetProfileAfterMessageCoroutine(PlayerData remotePlayerData)
	{
		...
		while (SceneLoadingMgr == null || SceneLoadingMgr.IsLoading())
		{
			yield return new WaitForEndOfFrame();
		}
	...
	internal void EvalulateAndShowHintPopup(BasicItemGameData basicItemGameData)
	{
		...
		case "hint_defeat_cabinboy":
		{
			...
			var bird3 = DIContainerInfrastructure.GetCurrentPlayer().GetBird("bird_red");
		...
		case "hint_defeat_monty":
		{
			...
			var bird4 = DIContainerInfrastructure.GetCurrentPlayer().GetBird("bird_yellow");
		...
		case "hint_defeat_porky_ship":
		{
			...
			var bird = DIContainerInfrastructure.GetCurrentPlayer().GetBird("bird_red");
			var bird2 = DIContainerInfrastructure.GetCurrentPlayer().GetBird("bird_white");
	...
	{
		var dictionary = new Dictionary<string, string>();
		foreach (var key in ClientInfo.LoadingTracking.Keys)
		{
			var num = ClientInfo.LoadingTracking[key];
