...
public class CharacterChunkHealthBar : CharacterHealthBar
{
	...
	public override void SetModel(ICombatant model)
	{
		...
		var num = (int)((m_Model.ModifiedHealth - m_Model.CurrentHealth) / (m_Model.ModifiedHealth / (float)m_NumberOfChunks));
		for (var i = 0; i < m_NumberOfChunks; i++)
		{
			var animation = m_HealthChunks[m_HealthChunks.Count - 1 - i];
	...
	{
		var chunkSize = maxHealth / (float)m_NumberOfChunks;
		var formerChunksGone = (int)((maxHealth - oldHealth) / chunkSize);
		if (oldHealth == newHealth)
		{
			for (var i = 0; i < m_NumberOfChunks; i++)
			{
				if (i < formerChunksGone)
				{
					m_HealthChunks[m_NumberOfChunks - 1 - i].Play(m_aniForChunkInactive);
				}
				else
				{
					m_HealthChunks[m_NumberOfChunks - 1 - i].Play(m_aniForChunkActive);
				}
			}
			yield break;
		}
		var totalChunksGone = (int)((maxHealth - newHealth) / chunkSize);
		var nrOfPlops = totalChunksGone - formerChunksGone;
		yield return new WaitForSeconds(DIContainerLogic.GetPacingBalancing().TimeForShowHealthBarChangeInSec);
		for (var j = 0; j < nrOfPlops; j++)
		{
			var cChunk = m_HealthChunks[m_HealthChunks.Count - 1 - formerChunksGone - j];
