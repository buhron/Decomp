...
public class ClientUpdateService
{
	...
	{
		var message = DIContainerInfrastructure.GetLocaService().Tr("conf_popup_new_clientversion_available", "There is a new version is available!");
	...
	public void OpenAppropriateReviewPage()
	{
		...
		#elif UNITY_IOS
		var reviewLink = "https://userpub.itunes.apple.com/WebObjects/MZUserPublishing.woa/wa/addUserReview?id=" + DIContainerConfig.GetClientConfig().AppleAppId + "&type=Purple+Software";
		DebugLog.Log("[ClientUpdateService] Review Link: " + reviewLink);
		Application.OpenURL(reviewLink);
		#endif
	}
	...
	public void OpenAppropriateAppStoreForNewVersion()
	{
		...
		#elif UNITY_IOS
		DebugLog.Log("Opening Apple App Store..");
		Application.OpenURL(string.Format("http://itunes.apple.com/app/id{0}", DIContainerConfig.GetClientConfig().AppleAppId));
		#endif
	}
	...
	private bool ParseFileAndCheckVersion(string assetPath)
	{
		...
		var array = FileHelper.ReadAllBytes(assetPath);
		var @string = Encoding.UTF8.GetString(array, 0, array.Length);
		...
		{
			var chimeraVersionNumber2 = new ChimeraVersionNumber();
			...
			var flag = chimeraVersionNumber2.IsOlderThan(chimeraVersionNumber);
		...
		var flag2 = DIContainerInfrastructure.GetVersionService().FullVersion.IsOlderThan(chimeraVersionNumber);
	...
	private IEnumerator CheckForNewVersionAvailable(MonoBehaviour syncObj, Action<bool> callback, Action onDownloadStarted = null)
	{
		...
		var versionAssetInfo = DIContainerInfrastructure.GetAssetData().GetAssetInfoFor(StoreVersionsFileName);
		if (versionAssetInfo != null)
		{
			var path = versionAssetInfo.FilePath;
			if (File.Exists(path))
			{
				var storeVersionBytes = FileHelper.ReadAllBytes(path);
				var version = Encoding.UTF8.GetString(storeVersionBytes, 0, storeVersionBytes.Length);
				if (string.IsNullOrEmpty(version))
				{
				}
				var remoteVersion = new ChimeraVersionNumber
				{
					ReportError = Debug.LogError
				}.FromString(version);
				if (!string.IsNullOrEmpty(DIContainerInfrastructure.GetVersionService().StoreVersion))
				{
					var localVersion2 = new ChimeraVersionNumber
					{
						ReportError = Debug.LogError
					};
					localVersion2 = remoteVersion.FromString(DIContainerInfrastructure.GetVersionService().StoreVersion);
					var isNew = localVersion2.IsOlderThan(remoteVersion);
					DebugLog.Log(string.Concat("[VersionCheck] Remote Version: ", remoteVersion, " Local Version: ", localVersion2, ", is store version newer: ", isNew));
					if (isNew)
					{
						callback(true);
					}
				...
				{
					var isNew2 = DIContainerInfrastructure.GetVersionService().FullVersion.IsOlderThan(remoteVersion);
					DebugLog.Log(string.Concat("[VersionCheck] Remote Version: ", remoteVersion, " Local Version: ", DIContainerInfrastructure.GetVersionService().FullVersion, ", is store version newer: ", isNew2));
					if (isNew2)
					{
						callback(true);
					}
	...
	private IEnumerator CheckForNewVersionInITunesStore(Action<string> versionCallback)
	{
		return null;
	}
}
