...
public class TouchInputHandler : MonoBehaviour
{
	...
	public void EvaluateMouse()
	{
		...
		var flag = false;
		for (var num = m_SecondaryInputHandlers.Count - 1; num >= 0; num--)
		{
			RaycastHit hit;
		...
		else if (Input.GetMouseButtonUp(0))
		{
			if (flag)
			{
				if (targetedObject && targetedObject != transform)
				{
					targetedObject.SendMessage("OnTouchReleased", SendMessageOptions.DontRequireReceiver);
					targetedObject = null;
				}
				else if (targetedObject)
				{
					targetedObject.SendMessage("OnTouchReleased", SendMessageOptions.DontRequireReceiver);
	...
	public void EvaluateTouches()
	{
		...
		else if ((touch.phase == TouchPhase.Moved || touch.phase == TouchPhase.Stationary) && targetedObject)
		{
			targetedObject.SendMessage("OnTouchDrag", SendMessageOptions.DontRequireReceiver);
		}
		else
	...
	public void SetTutorialLayers(bool activate)
	{
		...
		foreach (var secondaryInputHandler in m_SecondaryInputHandlers)
		{
