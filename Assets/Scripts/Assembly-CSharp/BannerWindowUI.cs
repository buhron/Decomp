...
public class BannerWindowUI : BirdWindowUIBase
{
	...
	{
		var flag = false;
		var bannerItemGameData = m_SelectedItem as BannerItemGameData;
		...
		if (flag)
		{
			...
			m_EnchantmentLabel.text = bannerItemGameData.EnchantmentLevel.ToString();
			m_EnchantmentProgress.fillAmount = bannerItemGameData.EnchantmentProgress;
			var flag2 = bannerItemGameData.IsMaxEnchanted();
			if (flag2 && bannerItemGameData.EnchantmentLevel == 0)
			{
	...
	{
		var equippedItem = m_Banner.InventoryGameData.Items[m_CurrentItemType].FirstOrDefault();
		...
		SelectSlot(inventoryItemSlot, false);
	}
	...
	private void ScrapItem(InventoryItemSlot slot)
	{
		...
		var scrapLoot = DIContainerLogic.CraftingService.ScrapBannerItem(DIContainerInfrastructure.GetCurrentPlayer().InventoryGameData, slot.GetModel() as BannerItemGameData);
	...
	private bool IsBestItemSlot(InventoryItemSlot slot)
	{
		...
		{
			var bannerItemGameData = inventoryItemSlot.GetModel() as BannerItemGameData;
			if (bannerItemGameData == null)
			{
				return false;
			}
	...
	private void SetItemListContent()
	{
		...
		for (var i = 0; i < m_GameDatas.Count; i++)
		{
			...
			var inventoryItemSlot2 = InstantiateItemSlot(inventoryItemGameData);
	...
	private void TrackBannerSetAchievement(BannerItemGameData bannerItem)
	{
		...
		{
			var achievementIdForStoryItemIfExists = DIContainerInfrastructure.GetAchievementService().GetAchievementIdForStoryItemIfExists("completeBannerSet");
