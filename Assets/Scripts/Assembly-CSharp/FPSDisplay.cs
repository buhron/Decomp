...
public class FPSDisplay : MonoBehaviour
{
	...
	public void Awake()
	{
		...
		base.enabled = DIContainerInfrastructure.GetCoreStateMgr() && DIContainerInfrastructure.GetCoreStateMgr().UseDebug;
	}
	...
	private void Update()
	{
		...
		{
			var fps = m_AccumulatedTimePassedForTenSec / (float)m_FramesPassedForTenSec;
	...
	{
		var num = m_AccumulatedTimePassed / (float)m_FramesPassed;
		m_FPSText.text = string.Format("{0:F2}", num);
		if (num < 30f)
		{
			m_FPSText.color = new Color(0f, 0.8f, 1f);
		}
		else if (num < 10f)
		{
	...
	{
		_LowestFPSEver = fps;
		m_LowestFPSEver.text = string.Format("{0:F2}", _LowestFPSEver);
		if (_LowestFPSEver < 30f)
		{
			m_LowestFPSEver.color = new Color(0f, 0.8f, 1f);
		}
	...
	{
		m_LowestFPSInLast10Sec.text = string.Format("{0:F2}", _LowestFPSInLast10Sec);
		if (_LowestFPSInLast10Sec < 30f)
		{
			m_LowestFPSInLast10Sec.color = new Color(0f, 0.8f, 1f);
		}
