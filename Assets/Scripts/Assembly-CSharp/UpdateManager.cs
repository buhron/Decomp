...
public class UpdateManager : MonoBehaviour
{
	...
	private static void CreateInstance()
	{
		if (mInst == null)
		{
			...
			{
				var gameObject = new GameObject("_UpdateManager");
	...
	private bool CoroutineUpdate()
	{
		...
		var num = realtimeSinceStartup - mTime;
		if (num < 0.001f)
		{
			return true;
		}
		...
		while (num2 > 0)
		{
			var destroyEntry = mDest.buffer[--num2];
	...
	private void Add(MonoBehaviour mb, int updateOrder, OnUpdate func, List<UpdateEntry> list)
	{
		...
		{
			var updateEntry = list[i];
			if (updateEntry.func == func)
			{
				return;
			}
		}
		var updateEntry2 = new UpdateEntry();
	...
	public static void AddDestroy(Object obj, float delay)
	{
		...
		if (Application.isPlaying)
		{
			...
			{
				CreateInstance();
				var destroyEntry = new DestroyEntry();
