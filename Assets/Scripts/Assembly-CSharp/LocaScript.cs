...
public class LocaScript : MonoBehaviour
{
	...
	public void SetLoca()
	{
		...
		{
			var aBHLocaService = !m_startupLoca ? DIContainerInfrastructure.GetLocaService() : DIContainerInfrastructure.GetStartupLocaService();
			var text = aBHLocaService.Tr(m_locaIdent, m_defaultText);
			if (label != null)
			{
				label.text = text;
			}
