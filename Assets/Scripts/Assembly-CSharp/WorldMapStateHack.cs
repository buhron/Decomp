...
public class WorldMapStateHack : MonoBehaviour
{
	...
	private void ExecuteHack()
	{
		if (IsUnlockedHack("hotspot_001_battleground"))
		{
			...
			if (!IsUnlockedHack("hotspot_003_battleground"))
			{
				var characterControllerWorldMap = Object.Instantiate(m_CharacterControllerWorldmap, m_RoguePigAnvil.position, Quaternion.identity) as CharacterControllerWorldMap;
				...
				var gameObject = (GameObject)Object.Instantiate(m_AnvilPrefab, m_Anvil.position, Quaternion.identity);
				gameObject.transform.parent = m_Anvil;
			}
			if (!IsUnlockedHack("hotspot_005_battleground"))
			{
				var characterControllerWorldMap2 = Object.Instantiate(m_CharacterControllerWorldmap, m_BirdCage.position, Quaternion.identity) as CharacterControllerWorldMap;
			...
			if (!IsUnlockedHack("hotspot_007_battleground"))
			{
				var characterControllerWorldMap3 = Object.Instantiate(m_CharacterControllerWorldmap, m_RoguePigBoss.position, Quaternion.identity) as CharacterControllerWorldMap;
	...
	{
		var componentsInChildren = base.gameObject.GetComponentsInChildren<HotSpotWorldMapViewBase>(true);
