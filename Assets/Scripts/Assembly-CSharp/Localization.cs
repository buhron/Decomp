...
public static class Localization
{
	...
	private static bool LoadDictionary(string value)
	{
		...
		if (loadFunction == null)
		{
			var textAsset2 = Resources.Load<TextAsset>(value);
			if (textAsset2 != null)
			{
	...
	{
		var byteReader = new ByteReader(asset);
	...
	{
		var i = 0;
		for (var num = mLanguages.Length; i < num; i++)
		{
	...
	private static bool LoadCSV(byte[] bytes, TextAsset asset, bool merge = false)
	{
		...
		var byteReader = new ByteReader(bytes);
		var betterList = byteReader.ReadCSV();
		...
		if (!localizationHasBeenSet || (!merge && !mMerging) || mLanguages == null || mLanguages.Length == 0)
		{
			...
			for (var i = 0; i < betterList.size; i++)
			{
		...
		for (var l = 0; l < mLanguages.Length; l++)
		{
			dictionary2.Add(mLanguages[l], l);
		}
		while (true)
		{
			var betterList2 = byteReader.ReadCSV();
		...
		{
			mMerging = true;
			var onLocalizeNotification = onLocalize;
			onLocalize = null;
	...
	private static void AddCSV(BetterList<string> newValues, string[] newLanguages, Dictionary<string, int> languageIndices)
	{
		...
		var value = ExtractStrings(newValues, newLanguages, languageIndices);
	...
	private static string[] ExtractStrings(BetterList<string> added, string[] newLanguages, Dictionary<string, int> languageIndices)
	{
		if (newLanguages == null)
		{
			var array = new string[mLanguages.Length];
			var i = 1;
			for (var num = Mathf.Min(added.size, array.Length + 1); i < num; i++)
			{
				array[i - 1] = added[i];
			}
			return array;
		...
		var j = 0;
		for (var num2 = newLanguages.Length; j < num2; j++)
		{
			var key2 = newLanguages[j];
			var num3 = languageIndices[key2];
	...
	public static string Get(string key)
	{
		...
		if (mLanguageIndex != -1 && mDictionary.TryGetValue(key, out value2))
		{
			...
			{
				var text2 = value2[mLanguageIndex];
