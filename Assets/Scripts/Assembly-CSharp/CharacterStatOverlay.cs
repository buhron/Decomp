...
public class CharacterStatOverlay : MonoBehaviour
{
	...
	internal void ShowStatOverlay(Transform root, BirdGameData bird, StatType stattype, Camera orientatedCamera)
	{
		...
		var birdCombatant = new BirdCombatant(bird).SetPvPBird(DIContainerInfrastructure.GetCoreStateMgr().m_IsWithinPvP);
	...
	{
		var dictionary = new Dictionary<string, string>();
		var bannerCombatant = new BannerCombatant(banner);
		var currentStatBuffs = new Dictionary<string, float>();
		bannerCombatant.CurrentStatBuffs = currentStatBuffs;
