...
public class DailyLoginUI : MonoBehaviour
{
	...
	{
		m_DailyRewards = new GameObject[31];
		for (var num = m_ContainerGrid.transform.childCount - 1; num >= 0; num--)
		{
	...
	private IEnumerator LeaveCoroutine()
	{
		...
		var sponsoredRollLeaveTime = m_videoRewardRoot.gameObject.PlayAnimationOrAnimatorState("SponsoredRoll_Leave");
		...
		if (DIContainerInfrastructure.LocationStateMgr != null)
		{
			if (DIContainerInfrastructure.LocationStateMgr.WorldMenuUI != null)
			{
				...
				var worldMenu = DIContainerInfrastructure.LocationStateMgr.WorldMenuUI as WorldMapMenuUI;
				if (worldMenu != null)
				{
	...
	{
		var claimedToday = DIContainerLogic.DailyLoginLogic.m_ClaimedToday;
		var giftsClaimed = (int)DIContainerInfrastructure.GetCurrentPlayer().Data.GiftsClaimedThisMonth;
		var dailyRewards = DIContainerLogic.DailyLoginLogic.m_RewardItems;
		for (var i = 0; i < 31; i++)
		{
			dailyRewards[i].m_State = GetRewardItemState(i, giftsClaimed, claimedToday);
			var rewardButton = UpdateDailyRewardItemButton(dailyRewards[i], i, giftsClaimed);
	...
	private DailyLoginButton UpdateDailyRewardItemButton(DailyRewardItem dailyRewardItem, int dayIndex, int giftsClaimed)
	{
		...
		var loot = dailyRewardItem.m_Loot;
		...
		if (m_DailyRewards[dayIndex] == null)
		{
			...
			var buttonPrefab = GetButtonPrefab(dailyRewardItem);
		...
		else
		{
			...
			if (dailyLoginButton != null && dailyRewardItem.m_State != dailyLoginButton.m_State)
			{
				...
				var buttonPrefab2 = GetButtonPrefab(dailyRewardItem);
		...
		var itemsFromLoot = DIContainerLogic.GetLootOperationService().GetItemsFromLoot(loot, EquipmentSource.DailyGift);
		...
		{
			list = itemsFromLoot;
			var equipmentGameData = itemsFromLoot.FirstOrDefault() as EquipmentGameData;
			if (equipmentGameData != null && equipmentGameData.IsSetItem)
			{
				text = equipmentGameData.BalancingData.RestrictedBirdId;
			}
		}
		var displayType = dailyRewardItem.m_State == DailyLoginButtonState.CURRENT ? LootDisplayType.Minor : LootDisplayType.None;
		Transform transform = null;
		var componentInChildren = gameObject.GetComponentInChildren<LootDisplayContoller>();
		...
		if (!string.IsNullOrEmpty(text) && componentInChildren != null)
		{
			...
			var calendarChestLootWon = DIContainerInfrastructure.GetCurrentPlayer().Data.CalendarChestLootWon;
			...
			{
				var equipmentGameData2 = new EquipmentGameData(value);
	...
	{
		var oldItem = m_DailyRewards[itemIndex];
	...
	{
		var flag = DIContainerLogic.DailyLoginLogic.HighlightDay(dailyRewardItem.m_Day);
	...
	private void SetFacebookState()
	{
		if (DIContainerInfrastructure.GetFacebookWrapper().IsUserAuthenticated())
		{
			var componentsInChildren = m_FacebookButton.GetComponentsInChildren<UIPlayAnimation>();
		...
		{
			var componentsInChildren2 = m_FacebookButton.GetComponentsInChildren<UIPlayAnimation>();
			var array2 = componentsInChildren2;
			foreach (var uIPlayAnimation2 in array2)
			{
	...
	public void ClaimGift()
	{
		...
		foreach (var item in DIContainerLogic.DailyLoginLogic.GetTodaysReward())
		{
		...
		DIContainerInfrastructure.GetAnalyticsSystem().LogEventWithParameters("DailyRewardClaimed", dictionary);
		if (DIContainerInfrastructure.GetFacebookWrapper().IsUserAuthenticated())
		{
			DIContainerLogic.GetLootOperationService().RewardLoot(DIContainerInfrastructure.GetCurrentPlayer().InventoryGameData, 0, DIContainerLogic.GetLootOperationService().GenerateLoot(DIContainerInfrastructure.GetCurrentPlayer().SocialEnvironmentGameData.BalancingData.FacebookDailyBonus, 1), "daily_facebook_bonus");
			DIContainerInfrastructure.GetCurrentPlayer().SavePlayerData();
	...
	private void RewardSponsoredAdResult(string placement, Ads.RewardResult result, string voucherId)
	{
		...
		case Ads.RewardResult.RewardConfirmed:
			...
			{
				var gameObject = m_DailyRewards[DIContainerInfrastructure.GetCurrentPlayer().Data.GiftsClaimedThisMonth];
	...
	{
		var giftsClaimedThisMonth = (int)DIContainerInfrastructure.GetCurrentPlayer().Data.GiftsClaimedThisMonth;
		if (m_DailyRewards.Count() <= giftsClaimedThisMonth)
		{
			return null;
		}
		var component = m_DailyRewards[giftsClaimedThisMonth].GetComponent<DailyLoginButton>();
		if (component == null)
		{
			return null;
		}
