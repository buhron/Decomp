...
public class RulesOverlayInvoker : MonoBehaviour
{
	...
	{
		if (DIContainerInfrastructure.GetCoreStateMgr() && DIContainerInfrastructure.GetCoreStateMgr().m_InfoOverlays)
		{
	...
	{
		var localizedText = DIContainerInfrastructure.GetLocaService().Tr("rules_bird_amount_tt").Replace("{value_1}", m_AllowedBirdsNum.ToString());
	...
	{
		var localizedText = DIContainerInfrastructure.GetLocaService().Tr("rules_bird_illegal_tt").Replace("{value_1}", m_RestrictedBirdName);
		DIContainerInfrastructure.GetCoreStateMgr().m_InfoOverlays.ShowGenericOverlay(base.transform, localizedText, true);
	}

	public void ShowBirdRequiredTooltip()
	{
		var localizedText = DIContainerInfrastructure.GetLocaService().Tr("rules_bird_needed_tt").Replace("{value_1}", m_RestrictedBirdName);
		DIContainerInfrastructure.GetCoreStateMgr().m_InfoOverlays.ShowGenericOverlay(base.transform, localizedText, true);
	}
