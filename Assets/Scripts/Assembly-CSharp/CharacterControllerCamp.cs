...
public class CharacterControllerCamp : MonoBehaviour
{
	...
	private void OnTapBegin()
	{
		...
		{
			var birdCombatant = new BirdCombatant(m_Model as BirdGameData).SetPvPBird(DIContainerInfrastructure.GetCoreStateMgr().m_IsWithinPvP);
		...
		{
			var bannerCombatant = new BannerCombatant(m_Model as BannerGameData);
			var currentStatBuffs2 = new Dictionary<string, float>();
			bannerCombatant.CurrentStatBuffs = currentStatBuffs2;
	...
	public void SetModel(ICharacter model, bool useScaleController = true, bool isLightWeight = true, bool showItems = true)
	{
		...
		if (audioSource) audioSource.spatialBlend = 0f; // fix sound bug
	}
	...
	{
		var a = new Vector3(0f, character.m_AssetController.BubblePosition.y, -5f);
