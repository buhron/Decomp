...
public class ContainerControl : MonoBehaviour
{
	...
	private void CheckStartupOptions()
	{
		...
		{
			var componentsInChildren = base.gameObject.GetComponentsInChildren<MeshRenderer>();
	...
	{
		var componentsInChildren = GetComponentsInChildren<LayoutControl>(true);
		foreach (var layoutControl in componentsInChildren)
		{
	...
	private void AdjustListeSizeByCount(int newCount, List<TableLine> list)
	{
		...
		if (list.Count < newCount)
		{
			for (var i = 0; i < m_newColumnCount - list.Count; i++)
			{
