...
public class EventPreviewCharacterSlot : MonoBehaviour
{
	...
	public void SetModel(CharacterControllerCamp characterController)
	{
		...
		var birdGameData = characterController.GetModel() as BirdGameData;
		if (birdGameData != null && birdGameData.InventoryGameData.Items[InventoryItemType.Skin].Count > 1)
		{
			var equippedSkin = birdGameData.ClassSkin;
			var item = birdGameData.InventoryGameData.Items[InventoryItemType.Skin].FirstOrDefault(s => s != equippedSkin) as SkinItemGameData;
