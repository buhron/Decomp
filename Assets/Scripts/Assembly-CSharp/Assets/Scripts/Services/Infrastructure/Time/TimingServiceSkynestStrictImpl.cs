...
namespace Assets.Scripts.Services.Infrastructure.Time
{
	internal class TimingServiceSkynestStrictImpl : ITimingService
	{
		...
		private uint m_UpcomingEventTime;
		...
		public TimingServiceSkynestStrictImpl()
		{
			...
			OnAppResumed();
			var @string = PlayerPrefs.GetString(PlayerPrefsName, string.Empty);
		...
		static TimingServiceSkynestStrictImpl()
		{
			MinDateTime = new DateTime(1970, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc);
		}
		...
		{
			var timeFromServer = new ABHTimingHandler().ProcessTimeFromSkynestTimeService(serverTimeJson);
		...
		private DateTime GetCurrentTime(out bool isTrusted)
		{
			...
			var dateTime = QuerySystemTime();
			long num = DateTimeToUnixTimestamp(dateTime);
			var num2 = (long)m_data.ServerSyncTime - (long)m_data.ClientSyncTime + num;
			string timeDescription;
			if (Math.Abs(num2 - m_data.ServerSyncTime) > Tolerance)
			{
				if (m_unityTimeFromOutOfSync != float.MinValue)
				{
					m_cachedTimeIsTrusted = isTrusted = true;
					var num3 = (int)(UnityEngine.Time.time - m_unityTimeFromOutOfSync);
			...
			var result = m_cachedTime = UnixTimeStampToDateTime(m_data.LatestClientTimeReturned);
		...
		public IEnumerator GetTrustedTime(Action<DateTime> callback)
		{
			...
			{
				bool isTrusted;
				var time = GetCurrentTime(out isTrusted);
				if (isTrusted)
				{
					callback(time);
					yield break;
			...
			var fallbackTime = GetCurrentTime(out isFallbackTrusted);
		...
		{
			var differenceInDays = GetDifferenceInDays(d1, d2);
		...
		public DateTime ConvertFacebookDayToDateTimeGraphApi(string dayAsString)
		{
			...
			try
			{
				result = new DateTime(int.Parse(array[2]), int.Parse(array[0]), int.Parse(array[1]));
			}
			catch
			{
				return DateTime.MinValue;
		...
		public bool SetTimeFromServer(int currServerTime)
		{
			...
			var num = DateTimeToUnixTimestamp(QuerySystemTime());
			if (m_lastDelta != int.MinValue)
			{
				var num2 = currServerTime - m_data.ServerSyncTime + m_data.ClientSyncTime;
				if (Math.Abs(num - num2) > Tolerance)
				{
		...
		private void UpdateTime(int clientTime, int serverTime)
		{
			...
			m_lastDelta = m_data.ClientSyncTime - m_data.ServerSyncTime;
			if (UnityEngine.Time.time - m_lastPersistTime > PersistIntervalTime || m_noServerTimeReceivedSinceAppStart)
			{
		...
		{
			var value = DIContainerInfrastructure.GetStringSerializer().Serialize(m_data);
			DIContainerInfrastructure.GetPlayerPrefsService().SetString(PlayerPrefsName, value);
		...
		{
			var dateTime = previousDay.AddDays(1.0);
			return new DateTime(dateTime.Year, dateTime.Month, dateTime.Day, 0, 0, 0);
		}

		public bool ResetEventTime(int maxTime, uint eventStartTime)
		{
			if (GetCurrentTimestamp() <= eventStartTime)
			{
				return false;
			}
			var num = GetCurrentTimestamp() - eventStartTime;
			var num2 = (ulong)(num - m_UpcomingEventTime) >= (ulong)maxTime;
			if (num2 && DIContainerInfrastructure.GetPlayerPrefsService().HasKey(PlayerPrefsName))
			{
				m_data.ClientSyncTime = m_data.ServerSyncTime;
				m_data.LatestClientTimeReturned = m_data.ServerSyncTime;
				m_lastDelta = m_data.ClientSyncTime - m_data.ServerSyncTime;
				m_cachedTime = UnixTimeStampToDateTime(m_data.ServerSyncTime);
			}
			m_UpcomingEventTime += num;
			return num2;
		}
	}
}
