...
namespace Assets.Scripts.Services.Infrastructure
{
	public class RovioAccSyncState
	{
		private enum AsyncOperation
		{
			...
			Login = 3,
			Logout = 4,
			LoginRetrievingProfile = 5,
			LogoutRetrievingProfile = 6,
			MoveToCloudQuestion = 7,
			ResettingGame = 8,
			LoginErrorSignBackInAsGuest = 9
		}
		...
		public void SignInSignOutButtonPressed()
		{
			...
			DebugLog.Log(GetType(), "SignInSignOutButtonPressed: ContentLoader.Instance: " + (ContentLoader.Instance == null ? "null" : "not null"));
			DebugLog.Log(GetType(), "SignInSignOutButtonPressed: ContentLoader.Instance.m_BeaconConnectionMgr: " + (ContentLoader.Instance.m_BeaconConnectionMgr == null ? "null" : "not null"));
		...
		{
			var currentProfile = DIContainerInfrastructure.GetProfileMgr().CurrentProfile;
		...
		private void EnterAsyncOp(AsyncOperation asyncOperation)
		{
			...
			var flag = DIContainerInfrastructure.IdentityService.IsGuest();
			if (flag)
			{
