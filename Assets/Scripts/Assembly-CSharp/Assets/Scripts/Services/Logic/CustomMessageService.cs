...
namespace Assets.Scripts.Services.Logic
{
	public class CustomMessageService
	{
		...
		public void AcknowledgedCustomMessage(IMailboxMessageGameData msg)
		{
			...
			{
				var msgCustom = msg as CustomMessageGameData;
				var num = m_mails.RemoveAll(md => md.Equals(msgCustom));
				if (num > 0 && m_messages.ContainsKey(msgCustom.Id))
				{
					var customMessage = m_messages[msgCustom.Id];
		...
		public IEnumerator Init(Action onDownloadStarted, Action<float> onDownloadProgress, Action<bool> onIsSlowCallback)
		{
			...
			var msgs = new List<CustomMessage>();
		...
		private void GenerateSingleCustomMessageGameData(CustomMessage msg)
		{
			...
			if (!m_mails.Contains((CustomMessageGameData)mailboxMessageGameData))
				m_mails.Add((CustomMessageGameData)mailboxMessageGameData);
			if (!m_messages.ContainsKey(msg.Key))
		...
		private static void ParseCustomMessages(List<CustomMessage> msgs, string path)
		{
			...
			var array = FileHelper.ReadAllBytes(path);
			var @string = Encoding.UTF8.GetString(array, 0, array.Length);
			var source = from line in @string.Split(new char[1] { '\n' }, StringSplitOptions.RemoveEmptyEntries)
				select line.Trim();
			var allMessages = (from msg in source.Select(ParseCustomMessage)
				where msg != null
				select msg).ToList();
		...
		{
			var array = (from part in line.Split(new char[1] { ';' }, 2, StringSplitOptions.RemoveEmptyEntries)
			...
			var text = !string.IsNullOrEmpty(array[0]) ? array[0] : "default";
			var text2 = array[1];
