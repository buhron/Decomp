...
public class NotificationMgr : MonoBehaviour
{
	...
	public void ManageNotifications(bool paused)
	{
		...
		if (DIContainerInfrastructure.GetCoreStateMgr() != null && DIContainerInfrastructure.GetCoreStateMgr().m_isInitialized)
		{
			...
			else if (DIContainerInfrastructure.GetCurrentPlayer().Data.NotificationUsageState == NotificationUsageStateAccepted)
			{
	...
	private void ReportDeviceTokenToOmniataIfNeeded()
	{
		...
		var deviceTokenAsString = DIContainerInfrastructure.NotificationService().GetDeviceTokenAsString();
		if (DIContainerInfrastructure.NotificationService().deviceToken != null && !string.IsNullOrEmpty(deviceTokenAsString))
		{
			...
			DIContainerInfrastructure.GetAnalyticsSystem().LogEventWithParameter("om_" + text + "_enable", "om_device_token", deviceTokenAsString);
			m_HasReportedDeviceTokenToOmniata = true;
			return;
		}
		var parameterValue = "disabled";
		...
		DIContainerInfrastructure.GetAnalyticsSystem().LogEventWithParameter("om_" + text + "_disable", "om_device_token", parameterValue);
		m_HasReportedDeviceTokenToOmniata = true;
	}
	...
	private void FireEventNotifications()
	{
		...
		{
			var teasingEndTime = DIContainerLogic.EventSystemService.GetTeasingEndTime(currentEventManagerGameData.Balancing);
			LocalNotificationPlatformIndependent localNotificationPlatformIndependent = null;
			var localNotificationPlatformIndependent2 = new LocalNotificationPlatformIndependent();
		...
		{
			var dateTime = DIContainerLogic.GetTimingService().GetPresentTime() + DIContainerLogic.EventSystemService.GetTimeTillEventEnd(currentEventManagerGameData.Balancing);
			...
			var dateTime2 = dateTime - new TimeSpan(2, 0, 0);
	...
	private void FireEventEnergyFullNotification()
	{
		...
		{
			var num = DIContainerBalancing.GameConstantsBalancingDataProvider.EventItemMaxCap;
			...
			var num2 = (float)(num - itemValue) * energyRefreshTimeInSeconds;
			if (num2 > 0f)
			{
				LocalNotificationPlatformIndependent localNotificationPlatformIndependent = null;
				var localNotificationPlatformIndependent2 = new LocalNotificationPlatformIndependent();
	...
	private void FireArenaLeagueNotifications()
	{
		...
		{
			var pvpTurnEndTime = DIContainerLogic.PvPSeasonService.GetPvpTurnEndTime(currentPvPSeasonGameData);
			var dateTime = pvpTurnEndTime - new TimeSpan(2, 0, 0);
			LocalNotificationPlatformIndependent localNotificationPlatformIndependent2;
