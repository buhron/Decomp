...
public class PvPAITargeting
{
	...
	public PvPAITargeting(PvPAIService service, BattleGameData battleData, CachedCombatats cachedCombatants)
	{
		m_service = service;
	...
	public SkillBattleDataBase GetSkillForBird(ICombatant bird)
	{
		...
		var nameId = bird.CombatantClass.BalancingData.NameId;
		if (nameId == "class_samurai" && m_battleData.IsRageFull(Faction.Birds))
		{
			if (m_showLog)
			{
		...
		if (nameId == "class_mage" && TauntingAllyWithoutDebuffExists("ShockShield"))
		{
			if (m_showLog)
			{
		...
		if (nameId == "class_wizard" && TauntingAllyWithoutDebuffExists("Energize"))
		{
			if (m_showLog)
			{
		...
		if (nameId == "class_druid" && (OneAllyBelowPercentageHP(80f, true) || AmountofAlliesDamaged(3)))
		{
			if (m_showLog)
			{
		...
		if (nameId == "class_princess" && (OneAllyBelowPercentageHP(80f, true) || TargetWithDebuffAmountsExists(2)))
		{
			if (m_showLog)
			{
		...
		if (nameId == "class_cannoneer" && TauntingAllyWithoutDebuffExists("Counter", false))
		{
			if (m_showLog)
			{
		...
		if (nameId == "class_berserk" && AmountOfLivingEnemies(3))
		{
			if (m_showLog)
			{
		...
		if (nameId == "class_tricksters" && AmountOfDebuffsOnOwnTeam(3))
		{
			if (m_showLog)
			{
		...
		if (nameId == "class_marksmen" && CheckMarksmenCondition(bird))
		{
			if (m_showLog)
			{
		...
		if (nameId == "class_avenger" && m_battleData.m_CombatantsByInitiative.Where(c => c.CombatantFaction == Faction.Birds).Count() <= 1)
		{
			if (m_showLog)
			{
	...
	{
		foreach (var item in m_battleData.m_CombatantsByInitiative.Where(c => c.CombatantFaction == Faction.Birds))
		{
			var tauntTarget = item.GetTauntTarget();
			if (tauntTarget != null)
			{
	...
	{
		var num = 0;
		foreach (var item in m_battleData.m_CombatantsByInitiative)
		{
	...
	private bool TargetWithDebuffAmountsExists(int amountOfDebuffs)
	{
		foreach (var item in m_battleData.m_CombatantsByInitiative)
		{
			...
			var num = 0;
			foreach (var value in item.CurrrentEffects.Values)
			{
	...
	{
		var list = m_battleData.m_CombatantsByInitiative.Where(c => c.CombatantFaction == Faction.Birds).ToList();
	...
	public ICombatant GetWeakestCombatantOfFaction(Faction team, bool includeBanner, string assetIdOfBuff, bool ignoreDyingTarget = false)
	{
		...
		foreach (var item in m_battleData.m_CombatantsByInitiative.Where(c => c.CombatantFaction != team && !c.IsBanner))
		{
		...
		foreach (var item2 in m_battleData.m_CombatantsByInitiative)
		{
			...
			var totalHealthIncludingBuffs = GetTotalHealthIncludingBuffs(item2, purgeAvailable);
	...
	private ICombatant GetMostDamagedAlly(ICombatant caster, string assetIdOfBuff)
	{
		...
		foreach (var item in m_battleData.m_CombatantsByInitiative)
		{
			...
			{
				var num2 = item.CurrentHealth / item.ModifiedHealth;
				if (num2 < num || num == 0f)
				{
					num = num2;
					result = item;
	...
	public ICombatant GetStrongestBird(Faction team, string assetIdOfBuff)
	{
		...
		foreach (var item in m_battleData.m_CombatantsByInitiative)
		{
			...
			{
				var totalHealthIncludingBuffs = GetTotalHealthIncludingBuffs(item, false);
				if (totalHealthIncludingBuffs > num || num == 0f)
				{
					num = totalHealthIncludingBuffs;
					result = item;
	...
	private ICombatant GetSkulkerSupportTarget(ICombatant skulker)
	{
		foreach (var item in m_battleData.m_CombatantsByInitiative.Where(c => c.CombatantFaction == Faction.Pigs && !c.IsBanner))
		{
			...
			if (nameId == "class_thunderbird" && CountBirdsWithoutDebuff("RageOfThunder") > 0)
			{
				return item;
			}
			if (nameId == "class_guardian" && CountBirdsWithoutDebuff("Overpower") > 0)
			{
				return item;
			}
			if (nameId == "class_cannoneer" && CountBirdsWithoutDebuff("CoverFire") > 0)
			{
				return item;
			}
			if (nameId == "class_rainbird" && CountBirdsWithoutDebuff("AcidRain") > 0)
			{
				return item;
			}
			if (nameId == "class_skulkers" && CountBirdsWithoutDebuff("WeakSpot") > 0)
			{
				return item;
			}
		}
	...
	{
		var num = bird.CurrentHealth;
		foreach (var value in bird.CurrrentEffects.Values)
		{
			if (value.m_EffectType == SkillEffectTypes.Curse)
			{
			...
			else
			{
				...
				foreach (var effect2 in value.m_Effects)
				{
	...
	{
		var num = 0;
		var num2 = PvPAIConstants.BasicBannerWeight;
		foreach (var item in m_battleData.m_BirdsByInitiative)
		{
			if (item.CombatantFaction != 0)
			{
				continue;
			}
			var num3 = item.CurrentHealth / item.ModifiedHealth;
			if (item.IsBanner)
			{
				if (num3 < 0.8f)
				{
					num2 += PvPAIConstants.AddBannerWeightBelow80;
				}
				if (num3 < 0.6f)
				{
					num2 += PvPAIConstants.AddBannerWeightBelow60;
				}
				if (num3 < 0.4f)
				{
					num2 += PvPAIConstants.AddBannerWeightBelow40;
				}
				if (num3 < 0.2f)
				{
					num2 += PvPAIConstants.AddBannerWeightBelow20;
				}
			}
			else
			{
				num += PvPAIConstants.BasicBirdWeight;
				if (num3 < 0.5f)
				{
					num2 += PvPAIConstants.AddBirdWeightBelow50;
				}
				if (num3 < 0.3f)
				{
					num2 += PvPAIConstants.AddBirdWeightBelow30;
				}
			}
		}
		if (m_showLog)
