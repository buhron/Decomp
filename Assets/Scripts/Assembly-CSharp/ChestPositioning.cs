...
public class ChestPositioning : MonoBehaviour
{
	...
	public bool SpawnChest(string chestPrefabId)
	{
		if (m_Chest != null)
			return true;

		if (string.IsNullOrEmpty(chestPrefabId))
		{
			m_Chest = Instantiate(m_ChestPrefab, base.transform.position + m_Offset, Quaternion.identity);
		}
		else
		{ 
			if (DIContainerInfrastructure.PropLiteAssetProvider().ContainsAsset(chestPrefabId))
			{
				m_Chest = Instantiate(DIContainerInfrastructure.PropLiteAssetProvider().GetObject(chestPrefabId)) as GameObject;
			}
			else if (DIContainerInfrastructure.GetShopIconAtlasAssetProvider().ContainsAsset(chestPrefabId))
			{
				m_Chest = Instantiate(DIContainerInfrastructure.GetShopIconAtlasAssetProvider().GetObject(chestPrefabId)) as GameObject;
			}
			UnityHelper.SetLayerRecusively(m_Chest, LayerMask.NameToLayer("Scenery"));
		}
		
		if (m_Chest != null)
		{
			m_Chest.transform.parent = transform;
			...
			m_Chest.transform.localScale = Vector3.one;
			m_Chest.transform.localPosition = Vector3.zero;
			m_Chest.GetComponent<BoxCollider>().size += new Vector3(0f, 0f, 80f);
		}
	...
	{
		var animLength = m_Chest.PlayAnimationOrAnimatorState("TreasureChest_Open");
	...
	{
		if (m_Chest && !m_AllowTooltipIfHotspotActive)
		{
