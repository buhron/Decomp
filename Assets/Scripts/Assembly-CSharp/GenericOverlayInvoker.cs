...
public class GenericOverlayInvoker : MonoBehaviour
{
	...
	public void OnTapBegin()
	{
		...
		{
			m_IsTapping = true;
			var localizedText = !string.IsNullOrEmpty(m_DefaultText) ? DIContainerInfrastructure.GetLocaService().Tr(m_LocaIdent, m_DefaultText) : DIContainerInfrastructure.GetLocaService().Tr(m_LocaIdent);
			if (m_UseHeader)
			{
				var text = !string.IsNullOrEmpty(m_HeaderDefaultText) ? DIContainerInfrastructure.GetLocaService().Tr(m_LocaIdent, m_HeaderLocaIdent) : DIContainerInfrastructure.GetLocaService().Tr(m_HeaderLocaIdent);
			}
			else
			{
