...
public class ByteReader
{
	...
	public static ByteReader Open(string path)
	{
		...
		if (fileStream != null)
		{
			...
			var array = new byte[fileStream.Position];
	...
	public string ReadLine(bool skipEmptyLines)
	{
		...
		if (num2 < num)
		{
			...
			var result = ReadLine(mBuffer, mOffset, num2 - mOffset - 1);
			mOffset = num2;
			return result;
		}
		mOffset = num;
	...
	public Dictionary<string, string> ReadDictionary()
	{
		...
		while (canRead)
		{
			...
			if (!text.StartsWith("//"))
			{
				var array = text.Split(separator, 2, StringSplitOptions.RemoveEmptyEntries);
				if (array.Length == 2)
				{
					var key = array[0].Trim();
					var value = array[1].Trim().Replace("\\n", "\n");
	...
	public BetterList<string> ReadCSV()
	{
		...
		while (canRead)
		{
			...
			var i = num;
			for (var length = text.Length; i < length; i++)
			{
				switch (text[i])
				{
				case ',':
