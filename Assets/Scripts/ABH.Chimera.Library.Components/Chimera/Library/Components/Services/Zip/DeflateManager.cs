...
namespace Chimera.Library.Components.Services.Zip
{
	internal sealed class DeflateManager
	{
		...
		{
			var num = this.heap[k];
			for (var i = k << 1; i <= this.heap_len; i <<= 1)
			{
			...
			for (var i = 0; i <= max_code; i++)
			{
				...
				if (++num3 >= num4 || num6 != num2)
				{
					...
					else if (num6 != 0)
					{
						if (num6 != num)
						{
							var array = this.bl_tree;
							var num7 = num6 * 2;
							array[num7] += 1;
						}
						var array2 = this.bl_tree;
						var num8 = InternalConstants.REP_3_6 * 2;
						array2[num8] += 1;
					}
					else if (num3 <= 10)
					{
						var array3 = this.bl_tree;
						var num9 = InternalConstants.REPZ_3_10 * 2;
						array3[num9] += 1;
					}
					else
					{
						var array4 = this.bl_tree;
						var num10 = InternalConstants.REPZ_11_138 * 2;
						array4[num10] += 1;
					}
					num3 = 0;
					num = num6;
		...
		internal bool _tr_tally(int dist, int lc)
		{
			...
			else
			{
				this.matches++;
				dist--;
				var array2 = this.dyn_ltree;
				var num2 = ((int)Tree.LengthCode[lc] + InternalConstants.LITERALS + 1) * 2;
				array2[num2] += 1;
				var array3 = this.dyn_dtree;
				var num3 = Tree.DistanceCode(dist) * 2;
				array3[num3] += 1;
			}
			if ((this.last_lit & 8191) == 0 && this.compressionLevel > CompressionLevel.Level2)
			{
				var num4 = this.last_lit << 3;
				var num5 = this.strstart - this.block_start;
				for (var i = 0; i < InternalConstants.D_CODES; i++)
				{
		...
		internal void send_compressed_block(short[] ltree, short[] dtree)
		{
			...
			if (this.last_lit != 0)
			{
				do
				{
					var num2 = this._distanceOffset + num * 2;
					var num3 = (((int)this.pending[num2] << 8) & 65280) | (int)(this.pending[num2 + 1] & byte.MaxValue);
					var num4 = (int)(this.pending[this._lengthOffset + num] & byte.MaxValue);
					num++;
					if (num3 == 0)
					{
					...
					{
						var num5 = (int)Tree.LengthCode[num4];
						this.send_code(num5 + InternalConstants.LITERALS + 1, ltree);
						var num6 = Tree.ExtraLengthBits[num5];
						if (num6 != 0)
						{
		...
		private void _fillWindow()
		{
			do
			{
				var num = this.window_size - this.lookahead - this.strstart;
				int num2;
				...
				else if (this.strstart >= this.w_size + this.w_size - DeflateManager.MIN_LOOKAHEAD)
				{
					...
					do
					{
						var num4 = (int)this.head[--num3] & 65535;
					...
					do
					{
						var num4 = (int)this.prev[--num3] & 65535;
		...
		internal BlockState DeflateSlow(FlushType flush)
		{
			...
			for (;;)
			{
				...
				{
					var num2 = this.strstart + this.lookahead - DeflateManager.MIN_MATCH;
					var flag = this._tr_tally(this.strstart - 1 - this.prev_match, this.prev_length - DeflateManager.MIN_MATCH);
				...
				{
					var flag = this._tr_tally(0, (int)(this.window[this.strstart - 1] & byte.MaxValue));
					if (flag)
					{
						this.flush_block_only(false);
					}
		...
		{
			var num = this.config.MaxChainLength;
			var num2 = this.strstart;
			var num3 = this.prev_length;
			var num4 = this.strstart > this.w_size - DeflateManager.MIN_LOOKAHEAD ? this.strstart - (this.w_size - DeflateManager.MIN_LOOKAHEAD) : 0;
			var niceLength = this.config.NiceLength;
			var num5 = this.w_mask;
			var num6 = this.strstart + DeflateManager.MAX_MATCH;
			var b = this.window[num2 + num3 - 1];
			var b2 = this.window[num2 + num3];
			...
			do
			{
				...
				if (this.window[num7 + num3] == b2 && this.window[num7 + num3 - 1] == b && this.window[num7] == this.window[num2] && this.window[++num7] == this.window[num2 + 1])
				{
					...
					var num8 = DeflateManager.MAX_MATCH - (num6 - num2);
		...
		internal int Deflate(FlushType flush)
		{
			...
			var num = this.last_flush;
			this.last_flush = (int)flush;
			if (this.status == DeflateManager.INIT_STATE)
			{
				var num2 = DeflateManager.Z_DEFLATED + (this.w_bits - 8 << 4) << 8;
				var num3 = ((this.compressionLevel - CompressionLevel.BestSpeed) & 255) >> 1;
				if (num3 > 3)
				{
					num3 = 3;
				}
			...
			{
				var blockState = this.DeflateFunction(flush);
